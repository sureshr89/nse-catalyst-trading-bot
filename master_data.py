"""Build persistent master datasets for the rolling six-month trading window."""
...
def _trade_merge(new):
    ...
    if "trade_id" in combined.columns:
        ...
        if fallback:
            combined = combined.drop_duplicates(subset=fallback, keep="last")
...
def _prune_to_last_six_months(path, date_columns):
    ...
    first = current - (MASTER_MONTHS - 1)
    allowed = {str(p) for p in pd.period_range(first, current, freq="M")}
    trimmed = frame.loc[months.isna() | months.isin(allowed)].copy()
...
def _today_rows(frame, date_columns, today):
    ...
    for column in date_columns:
        if column in frame.columns:
            values = pd.to_datetime(frame[column], errors="coerce")
            if values.notna().any():
                return frame.loc[values.dt.strftime("%Y-%m-%d").eq(today)].copy()
...
def _closed_unique(frame):
    ...
    if "trade_id" in closed.columns:
        ids = closed["trade_id"].astype(str).str.strip()
        has_id = ids.ne("") & ids.ne("nan")
        with_id = closed.loc[has_id].drop_duplicates(subset=["trade_id"], keep="last")
        without_id = closed.loc[~has_id].copy()
...
def build_master_data():
    ...
    if not gaps.empty and "Symbol" in gaps.columns:
        stock = gaps.copy().drop(columns=["TradeDate"], errors="ignore")
    ...
    if not trades.empty:
        t = trades.copy().drop(columns=["TradeDate"], errors="ignore")
    ...
    row = {
        ...
        "GapUps": int(gap_types.eq("GAP_UP").sum()),
        "GapDowns": int(gap_types.eq("GAP_DOWN").sum()),
        ...
        "MarketFilterPassed": int(diag.get("market_alignment_passed", 0) or 0),
        "StrategySetupPassed": int(diag.get("strategy_setup_passed", 0) or 0),
        "Nifty500ChangePct": float(diag.get("nifty500_change_pct", 0) or 0),
