...
SCANNER_DIAGNOSTICS_FILE = OUTPUT_DIR / "scanner_diagnostics.json"

_lock = threading.RLock()
_thread = None
_worker_lock_handle = None
_state = {
    "status": "STARTING", "message": "Paper bot is starting.", "last_cycle": None,
    "last_scan": None, "last_scan_completed": None, "scan_started_at": None,
    "scan_duration_seconds": None, "last_signal_count": 0, "last_scan_error": None,
    "scanner_status": "IDLE", "error": None, "worker_alive": False, "heartbeat": None,
        ...
        if bot is not None:
            try:
                ...
                payload.update({
                    ...
                    "available_capital": session.get("available_capital", 0.0),
                    "used_capital": session.get("used_capital", 0.0),
                    "session_pnl": session.get("total_pnl", 0.0),
                })
            except Exception:
                pass
            try:
                ...
                payload.update({
                    ...
                    "winning_trades": journal.get("winning_trades", 0),
                    "losing_trades": journal.get("losing_trades", 0),
                    "journal_pnl": journal.get("total_pnl", 0.0),
                })
            except Exception:
                pass
            try:
                payload["daily_pnl"] = bot.daily_pnl
                payload["cooldown_until"] = bot.cooldown_until.isoformat() if bot.cooldown_until else None
            except Exception:
                pass
...
def _prepare_pre_entry_candidates(bot):
    try:
        _write_status(bot, status="PREPARING", message="Preparing NIFTY 500 PDH/PDL and today's Open setups.", scanner_status="PREPARING")
        references = bot.scanner.prepare_reference_data()
        candidates = bot.scanner.prepare_opening_candidates()
        _persist_scanner_diagnostics(bot)
        if references.empty or candidates.empty:
            _write_status(bot, status="WAITING", message="NIFTY 500 setup preparation incomplete; retrying.", scanner_status="ERROR", error="PDH/PDL or opening setup coverage unavailable")
            return False
        _write_status(bot, status="WAITING", message=f"NIFTY 500 setups ready: {len(candidates)} stocks. Waiting for {TRADING_START} IST.", scanner_status="IDLE", error=None)
        return True
    except Exception as error:
        _write_status(bot, status="WAITING", message="Pre-entry preparation failed; worker will retry.", scanner_status="ERROR", error=f"{type(error).__name__}: {error}")
        return False
...
def _run_one_trading_day():
    ...
    bot = TradingBot()
    _write_status(bot, status="RUNNING", message="NIFTY 500 paper-trading bot is running.", error=None, scanner_status="IDLE", cycle_count=0, scan_count=0, worker_id=_worker_id())
    ...
    while True:
        ...
        if current < SQUARE_OFF_TIME:
            ...
            stamp = _iso_now()
            _write_status(bot, scanner_status="SCANNING", last_scan=stamp, scan_started_at=stamp, last_scan_error=None, scan_count=int(_state.get("scan_count", 0)) + 1)
            try:
                bot.run_cycle()
                _persist_scanner_diagnostics(bot)
                _write_status(bot, last_signal_count=int(getattr(bot.scanner.diagnostics, "get", lambda *_: 0)("final_signals", 0)), last_scan_error=None, error=None)
            except Exception as error:
                _persist_scanner_diagnostics(bot)
                message = f"Scanner/trading cycle error: {type(error).__name__}: {error}"
                _write_status(bot, last_scan_error=message, error=message, status="ERROR", scanner_status="ERROR")
                print(message)
                traceback.print_exc()
            finally:
                _write_status(bot, scanner_status="IDLE", last_scan_completed=_iso_now(), scan_duration_seconds=round(time.monotonic() - scan_started, 2))
        ...
        try:
            ...
            _write_status(bot, status="WAITING", message="Trading day complete. Waiting for the next Indian market session.", scanner_status="IDLE", error=None)
        return
...
def _run_bot():
    ...
    try:
        ...
        while True:
            now = _now()
            if now.weekday() >= 5:
                _write_status(status="WAITING", message="Weekend. Waiting for the next Indian market session.", scanner_status="IDLE", error=None)
                time.sleep(30)
                continue
            try:
                if now.strftime("%H:%M") < SQUARE_OFF_TIME:
                    ...
                    _write_status(status="WAITING", message="Market session finished. Waiting for the next Indian market session.", scanner_status="IDLE", error=None)
                    time.sleep(30)
            except Exception as error:
        ...
        try:
            _write_status(status="ERROR", message="Worker bootstrap failed; dashboard watchdog will restart it.", scanner_status="ERROR", error=f"{type(error).__name__}: {error}")
        except Exception:
            pass
    finally:
...
def start_bot():
    ...
    with _lock:
        ...
        _thread = threading.Thread(target=_run_bot, name="nifty500-paper-trading-bot", daemon=True)
        _thread.start()
    return get_status()
