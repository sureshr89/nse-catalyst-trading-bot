...
RR=1.25; MIN_RISK=1400.0; MAX_RISK=1500.0; CAPITAL_PER_TRADE=250000.0
...
def position_size(entry,stop_loss):
    try: entry,stop_loss=float(entry),float(stop_loss);rps=abs(entry-stop_loss)
    except (TypeError,ValueError): return None
    if not _finite_positive(entry) or not _finite_positive(rps):return None
    max_cap=int(CAPITAL_PER_TRADE//entry)
    if max_cap<1:return None
    # Choose the smallest quantity that reaches the minimum risk; reject if it crosses max risk.
...
def make_signal(strategy,side,symbol,entry,stop_loss,nifty500_change_pct,sector_alignment_pct,ad_ratio,ad_coverage,reason,previous_candle_open,previous_candle_close):
    if not market_gate(side,nifty500_change_pct,sector_alignment_pct,ad_ratio,ad_coverage) or not candle_gate(side,previous_candle_open,previous_candle_close):return None
    ...
    qty,rps,risk,capital=sizing;color="GREEN" if float(previous_candle_close)>float(previous_candle_open) else "RED"
    return TradeSignal(str(strategy).upper(),side,str(symbol).upper(),round(entry,4),round(stop_loss,4),round(target,4),round(rps,4),qty,round(risk,2),round(capital,2),RR,round(float(nifty500_change_pct),4),round(float(sector_alignment_pct),4),round(float(ad_ratio),4),round(float(previous_candle_open),4),round(float(previous_candle_close),4),color,reason,"Exit at SL or 1.25R target; force square-off at 15:00 IST")
...
def evaluate_s1(symbol,side,today_open,pdh,pdl,today_low,today_high,ltp,pdh_swept_down=False,pdl_swept_up=False,pdh_swept=False,pdl_swept=False,**g):
    ...
    if side=="BUY" and today_open>pdh and down_pdh and ltp>=today_open:return make_signal("S1",side,symbol,ltp,today_low,previous_candle_open=po,previous_candle_close=pc,reason="Open > PDH -> sweep below PDH -> reclaim Open",**g)
    if side=="SELL" and today_open<pdl and up_pdl and ltp<=today_open:return make_signal("S1",side,symbol,ltp,today_high,previous_candle_open=po,previous_candle_close=pc,reason="Open < PDL -> sweep above PDL -> reject Open",**g)
    return None

def evaluate_s2(symbol,side,pdh,pdl,pullback_low,pullback_high,ltp,breakout_seen=False,**g):
    g,po,pc=_candle_kwargs(g)
    if po is None or pc is None:return None
    if side=="BUY" and breakout_seen and ltp>=pdh and pullback_low is not None and pullback_low<=pdh:return make_signal("S2",side,symbol,ltp,pullback_low,previous_candle_open=po,previous_candle_close=pc,reason="Break PDH -> pullback to PDH -> reclaim",**g)
    if side=="SELL" and breakout_seen and ltp<=pdl and pullback_high is not None and pullback_high>=pdl:return make_signal("S2",side,symbol,ltp,pullback_high,previous_candle_open=po,previous_candle_close=pc,reason="Break PDL -> pullback to PDL -> fail below",**g)
    return None
...
def evaluate_s3(symbol,side,today_open,pdh,pdl,today_low,today_high,ltp,pdl_swept_down=False,pdh_swept_up=False,pdh_swept=False,pdl_swept=False,**g):
    ...
    if side=="BUY" and today_open>pdl and down_pdl and ltp>=today_open:return make_signal("S3",side,symbol,ltp,today_low,previous_candle_open=po,previous_candle_close=pc,reason="Open > PDL -> sweep below PDL -> reclaim Open",**g)
    if side=="SELL" and today_open<pdh and up_pdh and ltp<=today_open:return make_signal("S3",side,symbol,ltp,today_high,previous_candle_open=po,previous_candle_close=pc,reason="Open < PDH -> sweep above PDH -> reject Open",**g)
    return None

def evaluate_s4(symbol,side,today_high,today_low,prior_intraday_high,prior_intraday_low,ltp,**g):
    g,po,pc=_candle_kwargs(g)
    if po is None or pc is None:return None
    if side=="BUY" and prior_intraday_high is not None and ltp>float(prior_intraday_high):return make_signal("S4",side,symbol,ltp,float(prior_intraday_low) if prior_intraday_low is not None else float(today_low),previous_candle_open=po,previous_candle_close=pc,reason="LTP broke previously formed intraday High",**g)
    if side=="SELL" and prior_intraday_low is not None and ltp<float(prior_intraday_low):return make_signal("S4",side,symbol,ltp,float(prior_intraday_high) if prior_intraday_high is not None else float(today_high),previous_candle_open=po,previous_candle_close=pc,reason="LTP broke previously formed intraday Low",**g)
    return None
