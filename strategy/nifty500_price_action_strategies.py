...
The engine supplies one finalized Dhan snapshot collected over the configured
15-second window. No strategy performs an independent market-data request.
"""
from dataclasses import dataclass,asdict
from typing import Any,Dict
import math
from config.settings import MIN_DATA_COVERAGE_COUNT
RR=1.25;MIN_RISK=1400.0;MAX_RISK=1500.0;CAPITAL_PER_TRADE=250000.0;MIN_MARKET_COVERAGE=MIN_DATA_COVERAGE_COUNT
@dataclass(frozen=True)
class TradeSignal:
 strategy:str;side:str;symbol:str;entry:float;stop_loss:float;target:float;risk_per_share:float;quantity:int;actual_risk:float;capital_used:float;rr:float
 nifty500_change_pct:float;sector_alignment_pct:float;ad_ratio:float;previous_candle_open:float;previous_candle_close:float;previous_candle_color:str;entry_reason:str;exit_rules:str
...
def _c(g):
 g=dict(g);po=g.pop("previous_candle_open",None);pc=g.pop("previous_candle_close",None);allowed={"nifty500_change_pct","sector_alignment_pct","ad_ratio","ad_coverage","positive_sectors","negative_sectors"};return {k:v for k,v in g.items() if k in allowed},po,pc
