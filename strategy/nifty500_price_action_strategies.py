...
def position_size(entry, stop_loss):
    ...
    if max_capital_qty < 1: return None
    min_qty = max(1, int((MIN_RISK + rps - 1e-12) // rps))
    max_qty = min(max_capital_qty, int(MAX_RISK // rps))
...
def make_signal(strategy, side, symbol, entry, stop_loss, nifty500_change_pct, sector_alignment_pct, ad_ratio, ad_coverage, reason, previous_candle_open, previous_candle_close):
    ...
    return TradeSignal(
        strategy=str(strategy).upper(), side=side, symbol=str(symbol).upper(), entry=round(entry,4), stop_loss=round(stop_loss,4), target=round(target,4),
        risk_per_share=round(rps,4), quantity=qty, actual_risk=round(risk,2), capital_used=round(capital,2), rr=RR,
        nifty500_change_pct=round(float(nifty500_change_pct),4), sector_alignment_pct=round(float(sector_alignment_pct),4), ad_ratio=round(float(ad_ratio),4),
        previous_candle_open=round(float(previous_candle_open),4), previous_candle_close=round(float(previous_candle_close),4), previous_candle_color=color,
        entry_reason=reason, exit_rules="Exit at SL or 1.25R target; force square-off at 15:00 IST")


def evaluate_s1(symbol, side, today_open, pdh, pdl, today_low, today_high, ltp, pdh_swept_down=False, pdl_swept_up=False, **g):
    po, pc = g.get("previous_candle_open"), g.get("previous_candle_close")
    if po is None or pc is None: return None
    if side == "BUY" and today_open > pdh and pdh_swept_down and ltp >= today_open:
        return make_signal("S1", side, symbol, ltp, today_low, reason="Open > PDH -> sweep below PDH -> reclaim Open", previous_candle_open=po, previous_candle_close=pc, **g)
    if side == "SELL" and today_open < pdl and pdl_swept_up and ltp <= today_open:
        return make_signal("S1", side, symbol, ltp, today_high, reason="Open < PDL -> sweep above PDL -> reject Open", previous_candle_open=po, previous_candle_close=pc, **g)
    return None
...
def evaluate_s3(symbol, side, today_open, pdh, pdl, today_low, today_high, ltp, pdl_swept_down=False, pdh_swept_up=False, **g):
    po, pc = g.get("previous_candle_open"), g.get("previous_candle_close")
    if po is None or pc is None: return None
    if side == "BUY" and today_open > pdl and pdl_swept_down and ltp >= today_open:
        return make_signal("S3", side, symbol, ltp, today_low, reason="Open > PDL -> sweep below PDL -> reclaim Open", previous_candle_open=po, previous_candle_close=pc, **g)
    if side == "SELL" and today_open < pdh and pdh_swept_up and ltp <= today_open:
        return make_signal("S3", side, symbol, ltp, today_high, reason="Open < PDH -> sweep above PDH -> reject Open", previous_candle_open=po, previous_candle_close=pc, **g)
    return None
