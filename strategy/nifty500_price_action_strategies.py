...
def market_gate(side, nifty500_change_pct, sector_alignment_pct, ad_ratio, ad_coverage=500):
    try:
        change, sector, ad = float(nifty500_change_pct), float(sector_alignment_pct), float(ad_ratio)
    except (TypeError, ValueError):
        return False
...
def evaluate_s1(symbol, side, today_open, pdh, pdl, today_low, today_high, ltp,
    ...
    if side == "BUY" and today_open > pdh and down_pdh and ltp >= today_open:
        return make_signal("S1", side, symbol, ltp, today_low, previous_candle_open=po, previous_candle_close=pc,
                           reason="Open > PDH -> sweep below PDH -> reclaim Open", **g)
    if side == "SELL" and today_open < pdl and up_pdl and ltp <= today_open:
        return make_signal("S1", side, symbol, ltp, today_high, previous_candle_open=po, previous_candle_close=pc,
                           reason="Open < PDL -> sweep above PDL -> reject Open", **g)
    return None
...
def evaluate_s3(symbol, side, today_open, pdh, pdl, today_low, today_high, ltp,
    ...
    # Opposite-level sweep: BUY starts below PDL; SELL starts above PDH.
    if side == "BUY" and today_open < pdl and down_pdl and ltp >= today_open:
        return make_signal("S3", side, symbol, ltp, today_low, previous_candle_open=po, previous_candle_close=pc,
                           reason="Open < PDL -> sweep below PDL -> reclaim Open", **g)
    if side == "SELL" and today_open > pdh and up_pdh and ltp <= today_open:
        return make_signal("S3", side, symbol, ltp, today_high, previous_candle_open=po, previous_candle_close=pc,
                           reason="Open > PDH -> sweep above PDH -> reject Open", **g)
    return None
...
def evaluate_s4(symbol, side, today_high, today_low, prior_intraday_high, prior_intraday_low, ltp, **g):
    ...
    if side == "BUY" and prior_intraday_high is not None and ltp > float(prior_intraday_high):
        return make_signal("S4", side, symbol, ltp, float(prior_intraday_low) if prior_intraday_low is not None else float(today_low),
                           previous_candle_open=po, previous_candle_close=pc, reason="LTP broke previously formed intraday High", **g)
    if side == "SELL" and prior_intraday_low is not None and ltp < float(prior_intraday_low):
        return make_signal("S4", side, symbol, ltp, float(prior_intraday_high) if prior_intraday_high is not None else float(today_high),
                           previous_candle_open=po, previous_candle_close=pc, reason="LTP broke previously formed intraday Low", **g)
    return None
