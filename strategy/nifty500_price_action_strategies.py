...
def market_gate(side,nifty500_change_pct,sector_alignment_pct,ad_ratio,ad_coverage=500):
    ...
    return change>0 and sector>0 and ad>1 if side=="BUY" else change<0 and sector<0 and ad<1 if side=="SELL" else False
...
def position_size(entry,stop_loss):
    ...
    min_qty=max(1,int((MIN_RISK+rps-1e-12)//rps));max_qty=min(max_cap,int(MAX_RISK//rps))
...
def evaluate_s4(symbol,side,today_high,today_low,prior_intraday_high,prior_intraday_low,ltp,**g):
    po,pc=g.get("previous_candle_open"),g.get("previous_candle_close")
    if po is None or pc is None:return None
    if side=="BUY" and prior_intraday_high is not None and ltp>float(prior_intraday_high):return make_signal("S4",side,symbol,ltp,float(prior_intraday_low) if prior_intraday_low is not None else float(today_low),reason="LTP broke previously formed intraday High",previous_candle_open=po,previous_candle_close=pc,**g)
    if side=="SELL" and prior_intraday_low is not None and ltp<float(prior_intraday_low):return make_signal("S4",side,symbol,ltp,float(prior_intraday_high) if prior_intraday_high is not None else float(today_high),reason="LTP broke previously formed intraday Low",previous_candle_open=po,previous_candle_close=pc,**g)
    return None
