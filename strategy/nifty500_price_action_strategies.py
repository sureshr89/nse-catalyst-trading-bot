...
belongs to the engine; these functions validate the observed setup and calculate
a deterministic paper-trade signal.
"""
...
def market_gate(side, nifty500_change_pct, sector_alignment_pct, ad_ratio, ad_coverage=500):
    """Common directional gate; incomplete breadth never qualifies a trade."""
    try:
        change, sector, ad = map(float, (nifty500_change_pct, sector_alignment_pct, ad_ratio))
        coverage = int(ad_coverage)
    except (TypeError, ValueError):
        return False
    if coverage != 500 or not all(math.isfinite(x) for x in (change, sector, ad)):
        return False
    if side == "BUY":
...
def candle_gate(side, previous_candle_open, previous_candle_close):
    """Return candle direction for diagnostics; not used as an entry blocker."""
    try:
...
def make_signal(strategy, side, symbol, entry, stop_loss, nifty500_change_pct,
    ...
    return TradeSignal(str(strategy).upper(), str(side).upper(), str(symbol).upper(),
                       round(entry, 4), round(stop_loss, 4), round(target, 4), round(rps, 4), qty,
                       round(risk, 2), round(capital, 2), RR, round(float(nifty500_change_pct), 4),
                       round(float(sector_alignment_pct), 4), round(float(ad_ratio), 4),
                       round(po, 4), round(pc, 4), color, reason,
...
def evaluate_s4(symbol, side, today_high, today_low, prior_intraday_high, prior_intraday_low, ltp, **g):
    ...
    if side == "BUY" and prior_intraday_high is not None and ltp > prior_intraday_high:
        ...
        return make_signal("S4", side, symbol, ltp, sl, previous_candle_open=po, previous_candle_close=pc,
...
def evaluate_s5(symbol, side, pdh, pdl, ltp, **g):
    g, po, pc = _candle_kwargs(g)
    if po is None or pc is None: return None
    try: pdh, pdl, ltp = float(pdh), float(pdl), float(ltp)
    except (TypeError, ValueError): return None
