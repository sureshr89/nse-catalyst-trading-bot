...
def make_signal(strategy,side,symbol,entry,stop_loss,nifty500_change_pct,sector_alignment_pct,ad_ratio,ad_coverage,reason,previous_candle_open,previous_candle_close,positive_sectors=0,negative_sectors=0):
    ...
    return TradeSignal(str(strategy).upper(),str(side).upper(),str(symbol).upper(),round(entry,4),round(stop_loss,4),round(target,4),round(rps,4),qty,round(risk,2),round(capital,2),RR,round(float(nifty500_change_pct),4),round(float(sector_alignment_pct),4),round(float(ad_ratio),4),round(po,4),round(pc,4),color,reason,"Exit at SL or 1.25R target; force square-off at 15:00 IST")
...
def evaluate_s1(symbol,side,today_open,pdh,pdl,today_low,today_high,ltp,**g):
    ...
    if side=="BUY" and today_open>pdh and today_low<=pdh and ltp>today_open:
        return make_signal("S1",side,symbol,ltp,pdh,previous_candle_open=po,previous_candle_close=pc,reason="Open > PDH -> PDH swept/touched -> live LTP reclaimed above open",**g)
    if side=="SELL" and today_open<pdl and today_high>=pdl and ltp<today_open:
        return make_signal("S1",side,symbol,ltp,pdl,previous_candle_open=po,previous_candle_close=pc,reason="Open < PDL -> PDL swept/touched -> live LTP reclaimed below open",**g)
    return None
...
def evaluate_s2(symbol,side,pdh,pdl,pullback_low,pullback_high,ltp,breakout_seen=False,**g):
    ...
    if side=="BUY" and breakout_seen and ltp>=pdh and pullback_low is not None and pullback_low<=pdh:
        return make_signal("S2",side,symbol,ltp,pullback_low,previous_candle_open=po,previous_candle_close=pc,reason="Break PDH -> retest PDH -> live reclaim",**g)
    if side=="SELL" and breakout_seen and ltp<=pdl and pullback_high is not None and pullback_high>=pdl:
        return make_signal("S2",side,symbol,ltp,pullback_high,previous_candle_open=po,previous_candle_close=pc,reason="Break PDL -> retest PDL -> live failure",**g)
    return None
...
def evaluate_s3(symbol,side,today_open,pdh,pdl,today_low,today_high,ltp,**g):
    ...
    if side=="BUY" and inside and today_low<=pdl and ltp>today_open:
        return make_signal("S3",side,symbol,ltp,today_low,previous_candle_open=po,previous_candle_close=pc,reason="Open inside range -> sweep below PDL -> live reversal above open",**g)
    if side=="SELL" and inside and today_high>=pdh and ltp<today_open:
        return make_signal("S3",side,symbol,ltp,today_high,previous_candle_open=po,previous_candle_close=pc,reason="Open inside range -> sweep above PDH -> live reversal below open",**g)
    return None
...
def evaluate_s4(symbol,side,today_high,today_low,prior_intraday_high,prior_intraday_low,ltp,**g):
    ...
    if side=="BUY" and prior_intraday_high is not None and ltp>prior_intraday_high:
        return make_signal("S4",side,symbol,ltp,prior_intraday_low if prior_intraday_low else today_low,previous_candle_open=po,previous_candle_close=pc,reason="Live LTP broke previously formed intraday high",**g)
    if side=="SELL" and prior_intraday_low is not None and ltp<prior_intraday_low:
        return make_signal("S4",side,symbol,ltp,prior_intraday_high if prior_intraday_high else today_high,previous_candle_open=po,previous_candle_close=pc,reason="Live LTP broke previously formed intraday low",**g)
    return None
