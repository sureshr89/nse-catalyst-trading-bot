...
Completely isolated from Strategy 1 with its own ₹2.5 lakh paper capital,
journal, diagnostics and position state.
"""
...
class Strategy2Runtime:
    def __init__(self, scanner):
        self.scanner = scanner; self.strategy = GapExtensionReversalEngine(TRADING_START, LAST_ENTRY_TIME, 1.25); self.risk_engine = Strategy2RiskEngine(); self.paper_engine = Strategy2PaperTradeEngine(); self.journal = TradeJournal(str(STRATEGY2_TRADES), "outputs/strategy2_signals.csv"); self.processed = set(); self.last_signals = []; self.cooldown_until = None; self.daily_pnl = 0.0; self.diagnostics = {"strategy":"STRATEGY_2_GAP_EXTENSION_REVERSAL","signals":0,"candidates":0,"qualified":0,"buy_candidates":0,"sell_candidates":0,"buy_qualified":0,"sell_qualified":0,"rejections":{}}; self._restore_session(); self._restore_processed(); self._write_diagnostics()
    @staticmethod
    ...
    def _restore_session(self):
        ...
        try:
            ...
            if df.empty or not {"status","exit_time","pnl"}.issubset(df.columns):return
            closed=df[df["status"].astype(str).str.upper().eq("CLOSED")].copy()
            if closed.empty:return
            dates=closed["exit_time"].map(self._date_ist); pnl=pd.to_numeric(closed["pnl"],errors="coerce").fillna(0.0); self.daily_pnl=round(float(pnl[dates.eq(today)].sum()),2)
            if "exit_reason" not in closed.columns:return
            stop_rows=closed.loc[dates.eq(today)&closed["exit_reason"].astype(str).str.upper().eq("STOP_LOSS")]
            if stop_rows.empty:return
            last_stop=pd.to_datetime(stop_rows["exit_time"],errors="coerce").dropna().max()
            if pd.isna(last_stop):return
            if getattr(last_stop,"tzinfo",None) is None:last_stop=last_stop.tz_localize(INDIA_TZ)
            else:last_stop=last_stop.tz_convert(INDIA_TZ)
            end=last_stop.to_pydatetime()+timedelta(minutes=COOLDOWN_MINUTES)
            if end>self._now():self.cooldown_until=end.replace(tzinfo=None)
        except Exception as error:print(f"Strategy 2 session restore skipped: {type(error).__name__}: {error}")
    ...
    def _open_signal(self,signal,rank):
        ...
        trigger=trigger.tz_localize(INDIA_TZ) if getattr(trigger,"tzinfo",None) is None else trigger.tz_convert(INDIA_TZ);age=(self._now()-trigger).total_seconds()
        if age>MAX_TRIGGER_AGE_SECONDS:self._reject("stale_trigger");return False
        if age<-5:self._reject("future_trigger");return False
        candidate_id=f"S2|{self._now().date().isoformat()}|{signal['symbol']}|{signal['signal']}|{signal['trigger_time']}";signal.update({"candidate_id":candidate_id,"entry_time":signal["trigger_time"],"open_cross_level":signal["today_open"],"priority_rank":rank,"candidate_state":"QUALIFIED","nifty500_universe":True,"pdh_pdl_reached":False})
        if candidate_id in self.processed or self.journal.signal_exists(signal):self.processed.add(candidate_id);return False
        ...
        if not risk.get("approved"):
            self._reject("risk")
            if "CAPITAL" not in " ".join(map(str,risk.get("reasons",[]))).upper():self.processed.add(candidate_id)
            return False
        trade=dict(signal);trade.update(risk);trade["approved"]=True;result=self.paper_engine.open_trade(trade)
        if not result.get("opened"):self._reject("execution");self.processed.add(candidate_id) if "capital" not in str(result.get("reason","")).lower() else None;return False
        position=result.get("position")
        if position:self.journal.log_trade(position)
        self.processed.add(candidate_id);return True
    def scan(self):
    ...
    def run_cycle(self):self.process_positions();return self.scan()
    def square_off_all(self):
        for symbol in list(self.paper_engine.open_positions):
            candle=self.scanner.price_data.get_latest_available_1m(symbol)
            if candle is None:continue
            closed=self.paper_engine.close_position(symbol,float(candle["Close"]),candle["Datetime"],"SQUARE_OFF")
            if closed:self.daily_pnl=round(self.daily_pnl+float(closed.get("pnl",0) or 0),2);self.journal.log_trade(closed)
        self._write_diagnostics()
