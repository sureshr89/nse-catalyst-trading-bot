...
class Strategy2Runtime:
    def __init__(self, scanner):
        ...
        self.daily_pnl = 0.0
        self.diagnostics = {"strategy": "STRATEGY_2_GAP_EXTENSION_REVERSAL", "signals": 0, "candidates": 0, "qualified": 0, "buy_candidates": 0, "sell_candidates": 0, "buy_qualified": 0, "sell_qualified": 0, "rejections": {}}
    ...
    def _open_signal(self, signal, rank):
        ...
        if self.cooldown_until and self._now().replace(tzinfo=None) < self.cooldown_until:
            self._reject("cooldown"); return False
        ...
        if not risk.get("approved"):
            ...
            return False
        trade = dict(signal); trade.update(risk); trade["approved"] = True
        result = self.paper_engine.open_trade(trade)
        if not result.get("opened"):
            self._reject("execution")
            if "capital" not in str(result.get("reason", "")).lower(): self.processed.add(candidate_id)
            return False
