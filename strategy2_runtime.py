...
class Strategy2Runtime:
    def __init__(self, scanner):
        ...
        self.processed = set(); self.last_signals = []; self.cooldown_until = None; self.daily_pnl = 0.0
        self.diagnostics = {"strategy": self.strategy.strategy_id, "strategy_name": self.strategy.strategy_name, "strategy_version": self.strategy.strategy_version, "signals": 0, "candidates": 0, "qualified": 0, "buy_candidates": 0, "sell_candidates": 0, "buy_qualified": 0, "sell_qualified": 0, "rejections": {}}
    ...
    def _write_diagnostics(self):
        payload = dict(self.diagnostics); payload["timestamp"] = self._now().isoformat(timespec="seconds"); payload["open_positions"] = len(self.paper_engine.open_positions); payload["available_capital"] = self.paper_engine.available_capital; payload["used_capital"] = self.paper_engine.used_capital; payload["total_capital"] = self.paper_engine.total_capital; payload["daily_pnl"] = self.daily_pnl; payload["cooldown_active"] = bool(self.cooldown_until and self._now().replace(tzinfo=None) < self.cooldown_until)
        path = Path("outputs/strategy2_diagnostics.json"); path.parent.mkdir(parents=True, exist_ok=True); path.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    ...
    def scan(self):
        ...
        if candidates is None or candidates.empty: self.diagnostics["candidates"] = 0; self.last_signals = []; self._write_diagnostics(); return []
        rows = []
