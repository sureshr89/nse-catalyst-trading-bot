...
BASE_URL="https://api.dhan.co/v2"; MASTER_URL="https://images.dhan.co/api-data/api-scrip-master.csv"; CACHE_DIR=Path("data"); MASTER_CACHE=CACHE_DIR/"dhan_scrip_master.csv"; IST="Asia/Kolkata"; _LOCK=threading.RLock(); _QUOTE_CACHE={}; _QUOTE_CACHE_AT=0.0
...
def _post(path,payload,timeout=15):
    ...
    try:
        ...
        if r.status_code!=200:
            code=b.get("errorCode") or b.get("error_code") or b.get("code") if isinstance(b,dict) else None; msg=b.get("errorMessage") or b.get("error_message") or b.get("message") if isinstance(b,dict) else r.text[:300]
...
def load_instrument_master(force=False):
    ...
    try:
        r=requests.get(MASTER_URL,timeout=30);r.raise_for_status();x=pd.read_csv(StringIO(r.text),low_memory=False);x.to_csv(MASTER_CACHE,index=False);return x
    except Exception as exc:_set_status(ok=False,stage="INSTRUMENT_MASTER",message=f"{type(exc).__name__}: {exc}");return pd.DataFrame()
...
def market_quote(mapping,cache_seconds=10):
    ...
    ids=pd.to_numeric(mapping["SecurityId"],errors="coerce").dropna().astype(int).astype(str).tolist();response=_marketfeed("NSE_EQ",ids);data=response.get("data",{}).get("NSE_EQ",{}) if response else {};by_id=dict(zip(mapping["SecurityId"].astype(str),mapping["Symbol"].astype(str)));rows=[]
    for sid,item in data.items():
        if not isinstance(item,dict):continue
        o=item.get("ohlc") or {}
        try:
            ltp=float(item.get("last_price") or 0);nc=float(item.get("net_change") or 0);ohlc_close=float(o.get("close") or 0)
            # Dhan quote OHLC close is the completed/previous session close. Do not derive it as LTP-net_change.
