...
def load_instrument_master(force: bool = False) -> pd.DataFrame:
    """Load Dhan's public instrument master and retain NSE equity rows."""
...
def map_nifty500(symbols, force: bool = False) -> pd.DataFrame:
    ...
    The result contains Symbol, SecurityId, ExchangeSegment and Instrument.
    A caller must require exactly 500 valid rows before enabling signals.
    """
...
def market_quote(mapping: pd.DataFrame, cache_seconds: int = 10) -> pd.DataFrame:
    """Get LTP + today's OHLC + previous close for up to 1000 instruments."""
    global _QUOTE_CACHE_AT
    ...
    ids = pd.to_numeric(mapping["SecurityId"], errors="coerce").dropna().astype(int).tolist()
    payload = {"NSE_EQ": ids[:1000]}
    response = _post("/marketfeed/ohlc", payload)
...
def daily_history(security_id: str, from_date: str, to_date: str) -> pd.DataFrame:
    ...
    response = _post("/charts/historical", {
        "securityId": str(security_id),
...
def previous_day_references(mapping: pd.DataFrame, force: bool = False) -> pd.DataFrame:
    ...
    for _, item in mapping.iterrows():
        ...
        rows.append({
            ...
            "PDH": float(row["High"]),
            "PDL": float(row["Low"]),
            "PreviousDayClose": float(row["Close"]),
            "PreviousDayOpen": float(row["Open"]),
            "PreviousDayVolume": float(row.get("Volume", 0) or 0),
        })
