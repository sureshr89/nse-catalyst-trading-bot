...
class Nifty500Breadth:
 ...
 def _get_universe(self):
  ...
  if u is None or u.empty or "Symbol" not in u.columns:u=self.universe_engine.get_dataframe(refresh=True)
  ...
  if len(u)>=500:self._universe=u.head(500).reset_index(drop=True)
 ...
 def snapshot(self,force=False):
  ...
  if u.empty or "Symbol" not in u.columns:return self._fail("NIFTY_500_UNIVERSE_UNAVAILABLE",mode,now)
  symbols=u.Symbol.astype(str).str.upper().str.replace(".NS","",regex=False).drop_duplicates().tolist()
  if len(symbols)!=500:return self._fail(f"NIFTY_500_UNIVERSE_ONLY_{len(symbols)}/500",mode,now,len(symbols))
  if not dhan_configured():return self._fail("DHAN_NOT_CONFIGURED",mode,now)
  mapping=self._get_mapping(symbols)
  if len(mapping)!=500:return self._fail(f"DHAN_SECURITY_MAPPING_{len(mapping)}/500",mode,now,len(mapping),stage="MAPPING")
  ...
  if len(quotes)!=500:return self._fail(f"DHAN_MARKET_DATA_{len(quotes)}/500",mode,now,len(quotes),stage="QUOTE")
  ...
  if len(quotes)!=500:return self._fail(f"DHAN_VALID_PRICE_DATA_{len(quotes)}/500",mode,now,len(quotes),stage="PRICE")
  quotes["change_pct"]=(quotes.SessionClose-quotes.PreviousClose)/quotes.PreviousClose*100
  advances=int((quotes.change_pct>0).sum());declines=int((quotes.change_pct<0).sum());unchanged=int((quotes.change_pct==0).sum())
  # Never expose infinity. 0/0 means all 500 are unchanged; if there are advances
  # and no declines, infinity is mathematically possible but is not useful in the UI.
  ad_ratio=(float(advances/declines) if declines else (None if advances==0 else None))
  try:
  ...
  time.sleep(1.1);nifty=index_quote("NIFTY 500");prev_close=float(nifty.get("PreviousClose") or 0) if nifty else 0.0;live_ltp=float(nifty.get("LTP") or 0) if nifty else 0.0;day_close=float(nifty.get("Close") or 0) if nifty else 0.0
