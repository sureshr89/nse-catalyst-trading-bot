...
class Nifty500Breadth:
 ...
 def _get_universe(self):
  ...
  if len(u)!=REQUIRED or u["Symbol"].nunique()!=REQUIRED:return pd.DataFrame()
  if u["Sector"].astype(str).str.upper().isin({"UNKNOWN","NAN","NONE",""}).any():return pd.DataFrame()
 ...
 def _fail(self,reason,mode,now,evaluated=0,sector=None,stage=None):
  status=dhan_status();actual=status.get("message") or reason
  if stage:actual=f"{reason} | {actual}"
  return self._store(self._unknown(actual,mode,now,evaluated,sector))
 def snapshot(self,force=False):
  ...
  if u.empty or "Symbol" not in u.columns:return self._fail("NIFTY_500_UNIVERSE_NOT_EXACTLY_500",mode,now)
  ...
  if len(mapping)<MIN_DATA_COVERAGE_COUNT or len(mapping_symbols)<MIN_DATA_COVERAGE_COUNT:return self._fail(f"DHAN_SECURITY_MAPPING_BELOW_95PCT_{len(mapping)}/500",mode,now,len(mapping),stage="MAPPING")
  ...
  for attempt in range(1,QUOTE_RETRIES+1):
   ...
   if len(quotes)>=MIN_DATA_COVERAGE_COUNT and len(quote_symbols)>=MIN_DATA_COVERAGE_COUNT:break
  ...
  if len(quotes)<MIN_DATA_COVERAGE_COUNT or len(quote_symbols)<MIN_DATA_COVERAGE_COUNT:return self._fail(f"DHAN_MARKET_DATA_RETRY_FAILED_{len(quotes)}/500 ({last_quote_error})",mode,now,len(quotes),stage="QUOTE")
  ...
  if len(quotes)<MIN_DATA_COVERAGE_COUNT:return self._fail(f"DHAN_VALID_PRICE_DATA_BELOW_95PCT_{len(quotes)}/500",mode,now,len(quotes),stage="PRICE")
  ...
  advances=int((delta>tolerance).sum());declines=int((delta<-tolerance).sum());unchanged=int(len(quotes)-advances-declines);ad_ratio=(advances/declines if declines else None)
  try:
   sm=load_sector_map(u,refresh=True);sector=calculate_sector_alignment(quotes[["Symbol","change_pct"]],sm,"change_pct")
  except Exception as exc:sector={"available":False,"alignment_pct":None,"mapped":0,"priced":0,"sectors":0,"positive_sectors":0,"negative_sectors":0,"unchanged_sectors":0,"coverage":f"{len(quotes)}/500","error":str(exc)}
  if not sector.get("available") or int(sector.get("mapped",0))<MIN_DATA_COVERAGE_COUNT or int(sector.get("priced",0))<MIN_DATA_COVERAGE_COUNT:return self._fail(f"NIFTY_500_SECTOR_BELOW_95PCT_{sector.get('priced',0)}/500",mode,now,len(quotes),sector=sector,stage="SECTOR")
  nifty=None
  ...
  coverage=len(quotes)
  result={"universe":"NIFTY 500","total":REQUIRED,"evaluated":coverage,"advances":advances,"declines":declines,"unchanged":unchanged,"ad_ratio":ad_ratio,"direction":"BULLISH" if advances>declines else "BEARISH" if declines>advances else "NEUTRAL","complete":coverage>=MIN_DATA_COVERAGE_COUNT,"reason":"OK" if coverage>=MIN_DATA_COVERAGE_COUNT else "BELOW_95PCT","updated_at":f"{label} • market close 15:30 IST • refreshed {now.strftime('%H:%M:%S')} IST","nifty500_change_pct":nifty_change,"nifty500_ltp":live_ltp,"nifty500_previous_close":display_close if display_close>0 else None,"nifty500_reference_close":prev_close if prev_close>0 else None,"closed_session_label":label,"closed_session_basis":"Dhan completed-session close" if mode=="closed" else "Dhan live LTP","market_close_time":"15:30 IST","sector_alignment_pct":sector.get("alignment_pct"),"sector_complete":True,"sector_coverage":sector.get("coverage",f"{coverage}/500"),"sector_mapped":int(sector.get("mapped",0)),"sector_priced":int(sector.get("priced",0)),"sector_count":sector.get("sectors",0),"positive_sectors":sector.get("positive_sectors",0),"negative_sectors":sector.get("negative_sectors",0),"unchanged_sectors":sector.get("unchanged_sectors",0),"sector_error":sector.get("error"),"market_data_source":"DHAN","quote_rows":quotes[[c for c in ["Symbol","SecurityId","LTP","TodayOpen","TodayHigh","TodayLow","TodayClose","PreviousClose","NetChange","Volume","change_pct"] if c in quotes.columns]].copy(),"_cache_date":now.date(),"_cache_mode":mode}
