...
class Nifty500Breadth:
 ...
 def _get_universe(self):
  if self._universe is not None and len(self._universe)==500 and "Sector" in self._universe.columns and self._universe["Sector"].astype(str).str.upper().ne("UNKNOWN").sum()>=450:return self._universe
  u=self.universe_engine.get_dataframe(refresh=False)
  sector_ok=u is not None and not u.empty and "Sector" in u.columns and u["Sector"].astype(str).str.upper().ne("UNKNOWN").sum()>=450
  if u is None or u.empty or "Symbol" not in u.columns or not sector_ok:u=self.universe_engine.get_dataframe(refresh=True)
