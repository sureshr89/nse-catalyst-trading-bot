...
class Nifty500Breadth:
 ...
 def snapshot(self,force=False):
  ...
  except Exception as exc:sector={"available":False,"alignment_pct":None,"mapped":0,"priced":0,"sectors":0,"positive_sectors":0,"negative_sectors":0,"unchanged_sectors":0,"coverage":"0/500","error":str(exc)}
  if not sector.get("available") or int(sector.get("mapped",0))!=REQUIRED or int(sector.get("priced",0))!=REQUIRED:return self._fail(f"NIFTY_500_SECTOR_INCOMPLETE_{sector.get('priced',0)}/500",mode,now,len(quotes),sector=sector,stage="SECTOR")
  nifty=None
