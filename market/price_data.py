...
class PriceData:
    ...
    _multi_1m_cache = {}; _multi_1m_cache_at = {}
    _multi_daily_cache = {}; _multi_daily_cache_at = {}
    _live_price_cache = {}; _live_price_cache_at = {}

    def __init__(self):
        self.valid_intervals = {"1m", "5m", "1d"}
        self.max_workers = 5
        self._live_price_cache = self.__class__._live_price_cache
        self._live_price_cache_at = self.__class__._live_price_cache_at
    ...
    def _dhan_configured(self):
        return dhan_data.configured()
    ...
    def get_candles(self, symbol, interval="5m", period="1d"):
        ...
        if m is None or len(m) != 1:
            return pd.DataFrame()
        sid = str(m.iloc[0]["SecurityId"])
        now = datetime.now(INDIA_TZ); today = now.date()
        if interval == "1d":
    ...
    def get_multi_daily(self, symbols, period="10d"):
        """Fetch daily history for a full universe without exceeding Dhan's data rate.

        Dhan's current API documents five data requests per second. We therefore
        use five workers in paced batches. This is a once-per-session reference
        preparation path, not part of the 15-second live quote cycle.
        """
        ...
        def fetch_one(symbol):
            ...
            try:
                return symbol, dhan_data.daily_history(sid, start, end)
            except Exception:
                return symbol, pd.DataFrame()
    ...
    def get_latest_live_price(self, symbol, max_age_seconds=8):
        ...
        if not key or not self._dhan_configured():
            return None
        ...
        if mapping is None or getattr(mapping, "empty", True) or len(mapping) != 1:
            return None
    ...
    def _normalize_live_quote(frame, security_id):
        ...
        try:
            ...
            return None
        if any(pd.isna(v) or not float(v) > 0 for k, v in out.items() if k != "NetChange"):
            return None
        if pd.isna(out["NetChange"]) or not float(out["High"]) >= max(out["Open"], out["Low"], out["Close"]):
            return None
        if not float(out["Low"]) <= min(out["Open"], out["High"], out["Close"]):
            return None
        return out

    def get_latest_market_price(self, symbol):
        return self.get_latest_live_price(symbol, max_age_seconds=8)
