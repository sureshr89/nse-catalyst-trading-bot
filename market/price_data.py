...
class PriceData:
    ...
    def _to_ist(series):
        ...
        try:
            if getattr(values.dt,"tz",None) is None:return values.dt.tz_localize(INDIA_TZ)
    ...
    def _completed_1m(df):
        ...
        try:
            if getattr(timestamps.dt,"tz",None) is None:timestamps=timestamps.dt.tz_localize(INDIA_TZ)
            else:timestamps=timestamps.dt.tz_convert(INDIA_TZ)
            valid=timestamps.notna();current_minute=datetime.now(INDIA_TZ).replace(second=0,microsecond=0);valid &= timestamps<current_minute
            return data.loc[valid].copy().reset_index(drop=True)
        except Exception as error:
            print(f"1-minute completion validation failed: {type(error).__name__}: {error}");return pd.DataFrame()
    ...
    def get_latest_available_1m(self,symbol):
        try:
            data=self._clean_data(yf.download(tickers=self.yahoo_symbol(symbol),period="1d",interval="1m",auto_adjust=False,progress=False,threads=False,prepost=False,timeout=self.download_timeout));data=self._completed_1m(self._today_intraday(data));return None if data.empty else data.iloc[-1].to_dict()
        except Exception as error:print(f"Latest completed price failed for {symbol}: {error}");return None
    def get_latest_market_price(self,symbol):
        ticker=self.yahoo_symbol(symbol);now=datetime.now(INDIA_TZ)
        try:
            raw=self._clean_data(yf.download(tickers=ticker,period="1d",interval="1m",auto_adjust=False,progress=False,threads=False,prepost=False,timeout=self.download_timeout));today=self._completed_1m(self._today_intraday(raw))
            if not today.empty:
                latest=today.iloc[-1];candle_time=pd.Timestamp(latest["Datetime"]);candle_time=candle_time.tz_localize(INDIA_TZ) if candle_time.tzinfo is None else candle_time.tz_convert(INDIA_TZ);age_seconds=(now-candle_time.to_pydatetime()).total_seconds()
                if 0<=age_seconds<=120:return {"Close":float(latest["Close"]),"Datetime":candle_time.to_pydatetime(),"price_source":"recent_1m"}
                print(f"Current quote for {symbol} is stale ({age_seconds:.0f}s); square-off will retry")
        except Exception as error:print(f"Intraday market price failed for {symbol}: {type(error).__name__}: {error}")
        return None
    ...
    def _download_multi_batch(self,tickers,interval="1m",period="1d"):
        ...
        except Exception as error:print(f"Yahoo batch failed ({len(tickers)} tickers): {error}");return pd.DataFrame()
    ...
    def get_multi_1m(self,symbols):
        ...
        if non_empty:
            latest_common=min(pd.Timestamp(df["Datetime"].max()) for df in non_empty);latest_common=self._to_ist(pd.Series([latest_common])).iloc[0];now=datetime.now(INDIA_TZ);age=(now-latest_common.to_pydatetime()).total_seconds()
            if age<0 or age>120:
                print(f"Common 1m snapshot is stale ({age:.0f}s); rejecting batch")
                return {symbol:pd.DataFrame() for symbol in symbols}
            for symbol,df in list(result.items()):
                if df is not None and not df.empty:
                    ts=self._to_ist(df["Datetime"]);result[symbol]=df.loc[ts<=latest_common].reset_index(drop=True)
        return result
    def get_index_1m(self,ticker="^NSEI"):
        ...
        except Exception as error:print("NIFTY market-index data failed:",error);return pd.DataFrame()
