"""Live market price data with bounded Yahoo requests and process-wide short caches."""
...
class PriceData:
    ...
    _multi_1m_cache_at = {}
    _multi_daily_cache = {}
    _multi_daily_cache_at = {}
    _index_cache = {}
    _index_cache_at = {}
    _index_change_cache = {}
    _index_change_cache_at = {}
    _live_price_cache = {}
    ...
    def _to_ist(series):
        ...
        try:
            return values.dt.tz_localize(INDIA_TZ) if getattr(values.dt, "tz", None) is None else values.dt.tz_convert(INDIA_TZ)
        except Exception:
            return values
    ...
    def _completed_1m(df):
        ...
        try:
            ...
            valid = timestamps.notna() & (timestamps < datetime.now(INDIA_TZ).replace(second=0, microsecond=0))
    ...
    def _download(self, **kwargs):
        ...
        try:
            ...
            print(f"Yahoo download failed: {type(error).__name__}: {error}")
            return pd.DataFrame()
    ...
    def get_latest_available_1m(self, symbol):
        """Return the latest completed 1-minute candle for strategy logic."""
        try:
    ...
    def get_latest_live_price(self, symbol, max_age_seconds=8):
        """Return the newest available market price, including the current 1-minute bar.

        This is deliberately separate from completed-candle strategy data. It is used
        only for live SL/TP monitoring, so an open position can exit as soon as the
        observed LTP crosses its stop or target instead of waiting for candle close.
        """
        key = str(symbol).strip().upper().replace(".NS", "")
        now = time.monotonic()
        with self._cache_lock:
            cached = self._live_price_cache.get(key)
            cached_at = self._live_price_cache_at.get(key, 0.0)
            if cached is not None and now - cached_at <= max_age_seconds:
                return dict(cached)
        try:
            raw = self._download(tickers=self.yahoo_symbol(key), period="1d", interval="1m", auto_adjust=False, progress=False, threads=False, prepost=False, timeout=self.download_timeout)
            data = self._clean_data(raw)
            data = self._today_intraday(data)
            if data.empty:
                return cached
            row = data.iloc[-1]
            ts = pd.Timestamp(row["Datetime"])
            ts = ts.tz_localize(INDIA_TZ) if ts.tzinfo is None else ts.tz_convert(INDIA_TZ)
            result = {"Close": float(row["Close"]), "Datetime": ts.to_pydatetime(), "Open": float(row["Open"]), "High": float(row["High"]), "Low": float(row["Low"]), "price_source": "live_1m_bar"}
            with self._cache_lock:
                self._live_price_cache[key] = dict(result)
    ...
    def get_latest_market_price(self, symbol):
        """Compatibility display method: use the newest available live price."""
        latest = self.get_latest_live_price(symbol)
        if latest:
            try:
                ts = pd.Timestamp(latest["Datetime"])
                ts = ts.tz_localize(INDIA_TZ) if ts.tzinfo is None else ts.tz_convert(INDIA_TZ)
                if 0 <= (datetime.now(INDIA_TZ) - ts.to_pydatetime()).total_seconds() <= 180:
                    return {"Close": float(latest["Close"]), "Datetime": ts.to_pydatetime(), "price_source": latest.get("price_source", "live_1m_bar")}
            except Exception:
                pass
        return None
        ...
        if isinstance(raw.columns, pd.MultiIndex):
            ...
            for symbol, ticker in zip(batch, tickers):
                try:
                    ...
                    cleaned = self._completed_1m(cleaned) if completed else cleaned
                    if today_only:
                        cleaned = self._today_intraday(cleaned)
                    if not cleaned.empty:
                        result[symbol] = cleaned
                except Exception:
                    continue
        elif len(batch) == 1:
            cleaned = self._clean_data(raw)
            cleaned = self._completed_1m(cleaned) if completed else cleaned
            if today_only:
                cleaned = self._today_intraday(cleaned)
            if not cleaned.empty:
    ...
    @classmethod
    def _copy_cache(cls, cache):
        return {k: v.copy() for k, v in cache.items()}

    def get_multi_daily(self, symbols, period="5d"):
        symbols = tuple(dict.fromkeys(str(s).upper().replace(".NS", "") for s in symbols if str(s).strip()))
        key = (symbols, period)
        now = time.monotonic()
        with self._cache_lock:
            cached = self._multi_daily_cache.get(key)
            if cached is not None and now - self._multi_daily_cache_at.get(key, 0) < 50:
                return self._copy_cache(cached)
        result = {}
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    result.update(self._extract_batch(futures[future], future.result(), False, False))
                except Exception as error:
                    print("Daily batch failed:", error)
        for s in symbols:
            result.setdefault(s, pd.DataFrame())
        with self._cache_lock:
            self._multi_daily_cache[key] = result
            self._multi_daily_cache_at[key] = time.monotonic()
        return self._copy_cache(result)
    ...
    def get_multi_1m(self, symbols):
        ...
        now = time.monotonic()
        with self._cache_lock:
            cached = self._multi_1m_cache.get(key)
            if cached is not None and now - self._multi_1m_cache_at.get(key, 0) < 50:
                return self._copy_cache(cached)
        result = {}
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    result.update(self._extract_batch(futures[future], future.result(), True, True))
                except Exception as error:
                    print("1m batch failed:", error)
        for s in symbols:
            result.setdefault(s, pd.DataFrame())
        with self._cache_lock:
            self._multi_1m_cache[key] = result
            self._multi_1m_cache_at[key] = time.monotonic()
        return self._copy_cache(result)

    def get_index_1m(self, ticker="^CRSLDX", max_age_seconds=20):
        now = time.monotonic()
        cached = self._index_cache.get(ticker)
        if cached is not None and now - self._index_cache_at.get(ticker, 0) < max_age_seconds:
            return cached.copy()
        try:
            ...
            if not data.empty:
                with self._cache_lock:
                    self._index_cache[ticker] = data.copy()
                    self._index_cache_at[ticker] = time.monotonic()
                return data
        except Exception:
            pass
    ...
    def get_index_change_pct(self, ticker="^CRSLDX", intraday=None, max_age_seconds=25):
        now = time.monotonic()
        cached = self._index_change_cache.get(ticker)
        if cached is not None and now - self._index_change_cache_at.get(ticker, 0) < max_age_seconds:
            return cached
        try:
            ...
            if prior.empty or intraday is None or intraday.empty:
                return cached
            value = (float(intraday.iloc[-1]["Close"]) / float(prior.iloc[-1]["Close"]) - 1) * 100
            with self._cache_lock:
                self._index_change_cache[ticker] = float(value)
                self._index_change_cache_at[ticker] = time.monotonic()
            return float(value)
        except Exception:
            return cached

    def today_only(self, df):
        return self._today_intraday(df)
