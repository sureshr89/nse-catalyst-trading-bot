...
class PriceData:
    ...
    def _dhan_live_quote(self,symbol):
        ...
        try:
            ...
            mapping=map_nifty500([str(symbol).upper().replace(".NS","")])
    ...
    def get_latest_live_price(self,symbol,max_age_seconds=8):
        key=str(symbol).strip().upper().replace(".NS","");now=time.monotonic()
        with self._cache_lock:
            cached=self._live_price_cache.get(key);cached_at=self._live_price_cache_at.get(key,0.0)
            if cached is not None and now-cached_at<=max_age_seconds:return dict(cached)
        dhan=self._dhan_live_quote(key)
        if dhan:
        ...
        try:
            ...
            row=data.iloc[-1];ts=pd.Timestamp(row["Datetime"]);ts=ts.tz_localize(INDIA_TZ) if ts.tzinfo is None else ts.tz_convert(INDIA_TZ);result={"Close":float(row["Close"]),"Datetime":ts.to_pydatetime(),"Open":float(row["Open"]),"High":float(row["High"]),"Low":float(row["Low"]),"price_source":"Yahoo fallback"}
            with self._cache_lock:self._live_price_cache[key]=dict(result);self._live_price_cache_at[key]=time.monotonic()
            return result
