...
class PriceData:
    ...
    _multi_1m_cache={}; _multi_1m_cache_at={}; _multi_daily_cache={}; _multi_daily_cache_at={}
    _index_cache={}; _index_cache_at={}; _index_change_cache={}; _index_change_cache_at={}
    def __init__(self):
        self.valid_intervals={"1m","5m","1d"}; self.download_timeout=10; self.batch_size=50; self.max_workers=2; self.batch_retries=1
    ...
    def _completed_1m(df):
        ...
        try:
            timestamps=timestamps.dt.tz_localize(INDIA_TZ) if getattr(timestamps.dt,"tz",None) is None else timestamps.dt.tz_convert(INDIA_TZ);valid=timestamps.notna()&(timestamps<datetime.now(INDIA_TZ).replace(second=0,microsecond=0));return data.loc[valid].copy().reset_index(drop=True)
    ...
    def _clean_data(self,df):
        ...
        data=data.dropna(subset=required);keep=required+(["Volume"] if "Volume" in data.columns else []);return data[keep].sort_values("Datetime").drop_duplicates("Datetime").reset_index(drop=True)
    @staticmethod
    ...
    def _download(self,**kwargs):
        ...
        except Exception as error:print(f"Yahoo download failed: {type(error).__name__}: {error}");return pd.DataFrame()
    def get_candles(self,symbol,interval="5m",period="1d"):
        ...
        data=self._clean_data(self._download(tickers=self.yahoo_symbol(symbol),period=period,interval=interval,auto_adjust=False,progress=False,threads=False,prepost=False,timeout=self.download_timeout));return self._completed_1m(data) if interval=="1m" else data
    ...
    def get_latest_available_1m(self,symbol):
        try:
            data=self._completed_1m(self._today_intraday(self._clean_data(self._download(tickers=self.yahoo_symbol(symbol),period="1d",interval="1m",auto_adjust=False,progress=False,threads=False,prepost=False,timeout=self.download_timeout))));return None if data.empty else data.iloc[-1].to_dict()
    ...
    def get_multi_daily(self,symbols,period="5d"):
        symbols=tuple(dict.fromkeys(str(s).upper().replace(".NS","") for s in symbols if str(s).strip()));key=(symbols,period);now=time.monotonic()
        ...
        with self._cache_lock:self._multi_daily_cache[key]=result;self._multi_daily_cache_at[key]=time.monotonic()
        return self._copy_cache(result)
    def get_multi_1m(self,symbols):
        symbols=tuple(dict.fromkeys(str(s).upper().replace(".NS","") for s in symbols if str(s).strip()));key=symbols;now=time.monotonic()
        with self._cache_lock:
        ...
        for s in symbols:result.setdefault(s,pd.DataFrame())
        with self._cache_lock:self._multi_1m_cache[key]=result;self._multi_1m_cache_at[key]=time.monotonic()
        return self._copy_cache(result)
    def get_index_1m(self,ticker="^CRSLDX",max_age_seconds=20):
        ...
        try:
            ...
            if not data.empty:
                with self._cache_lock:self._index_cache[ticker]=data.copy();self._index_cache_at[ticker]=time.monotonic()
                return data
        except Exception:pass
