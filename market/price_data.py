...
class PriceData:
    ...
    def _clean(df):
        ...
        x=df.copy();x["Datetime"]=pd.to_datetime(x["Datetime"],errors="coerce")
    ...
    def get_candles(self,symbol,interval="5m",period="1d"):
        ...
        if interval=="1d":
            days=max(10,int(str(period).rstrip("d")) if str(period).endswith("d") else 10);return daily_history(sid,(today-timedelta(days=days+5)).isoformat(),(today+timedelta(days=1)).isoformat())
