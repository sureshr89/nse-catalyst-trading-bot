...
class PriceData:
    ...
    def get_latest_market_price(self, symbol):
        ...
        try:
            ...
            if not today.empty:
                ...
                print(f"Current quote for {symbol} is stale ({age_seconds:.0f}s); retrying")
        except Exception as error:
    ...
    def _download_multi_batch(self, tickers, interval="1m", period="1d"):
        try:
            return yf.download(
                ...
                auto_adjust=False, progress=False, threads=False, prepost=False,
                group_by="ticker", timeout=self.download_timeout
            )
        except Exception as error:
    ...
    def get_multi_1m(self, symbols):
        ...
        if non_empty:
            ...
            now = datetime.now(INDIA_TZ)
            age = (now - latest_common.to_pydatetime()).total_seconds()
            if age < 0 or age > 120:
    ...
    def get_index_1m(self, ticker="^CRSLDX"):
        try:
            ...
            print("NIFTY 500 index data failed:", error)
            return pd.DataFrame()

    def get_index_change_pct(self, ticker="^CRSLDX"):
        """Return current NIFTY 500 percentage change versus the previous trading close."""
        try:
            data = self._clean_data(yf.download(
                tickers=ticker, period="5d", interval="1d", auto_adjust=False,
                progress=False, threads=False, prepost=False, timeout=self.download_timeout
            ))
            if data.empty:
                return None
            data = data.sort_values("Datetime").dropna(subset=["Close"])
            today = datetime.now(INDIA_TZ).date()
            prior = data[data["Datetime"].dt.date < today]
            if prior.empty:
                return None
            previous_close = float(prior.iloc[-1]["Close"])
            intraday = self.get_index_1m(ticker)
            if intraday.empty or previous_close <= 0:
                return None
            current_price = float(intraday.iloc[-1]["Close"])
            return (current_price / previous_close - 1.0) * 100.0
        except Exception as error:
            print(f"NIFTY 500 change calculation failed: {type(error).__name__}: {error}")
            return None
