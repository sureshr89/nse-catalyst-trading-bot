...
def _link(label, page, key):
    # Numeric width is not accepted by Streamlit page_link on the deployed version.
    # Let the surrounding horizontal container control the layout instead.
    st.page_link(page, label=label, key=key)


def _row(left, right):
    with st.container(horizontal=True, horizontal_alignment="distribute", vertical_alignment="center", gap="small", width="stretch"):
...
def render_nav(top_offset=0):
    ...
    st.markdown(
        ...
        [data-testid="stPageLink"] a { min-height:52px !important; border:1px solid #303A4B !important; border-radius:12px !important; background:#151B26 !important; padding:8px 10px !important; box-sizing:border-box !important; justify-content:center !important; }
        [data-testid="stPageLink"] a:hover { border-color:#59769F !important; background:#192233 !important; }
        @media (max-width:600px) { [data-testid="stPageLink"] a { min-height:50px !important; padding:7px 5px !important; font-size:.76rem !important; } }
        </style>
        """,
    ...
    _row(("🔵 STRATEGY 1", "pages/current_trading.py", "nav_home_s1"), ("🔴 STRATEGY 2", "pages/strategy2.py", "nav_home_s2"))
