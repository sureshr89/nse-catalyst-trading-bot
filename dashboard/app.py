...
# Streamlit Cloud can execute dashboard/app.py with the repository root absent
# from sys.path. Add it BEFORE importing dashboard.* or root-level modules.
...
def grid(items):
    st.markdown(
        ...
        + ''.join(f'<div class="metric-card"><small>{a}</small><b>{b}</b></div>' for a, b in items)
        + '</div>',
