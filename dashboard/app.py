...
STATUS_FILE=PROJECT_ROOT/"outputs"/"bot_status.json"; TRADES_FILE=PROJECT_ROOT/"outputs"/"trades.csv"; SIGNALS_FILE=PROJECT_ROOT/"outputs"/"signals.csv"; STATE_FILE=PROJECT_ROOT/"outputs"/"paper_engine_state.json"; BOT_RUNNER_FILE=PROJECT_ROOT/"bot_runner.py"; SETTINGS_FILE=PROJECT_ROOT/"config"/"settings.py"; INDIA_TZ=ZoneInfo("Asia/Kolkata")
...
worker_alive=bool(bot_status.get("worker_alive",False)) if worker_module is not None else False; status=str(bot_status.get("status","STARTING")).upper(); scanner_status=str(bot_status.get("scanner_status","IDLE")).upper(); state=read_json(STATE_FILE); open_positions=state.get("open_positions",{}) or {}; trades=read_csv(TRADES_FILE); signals=dedupe_signals(read_csv(SIGNALS_FILE))
st.markdown("""
<style>
...
.live-title{font-size:1.55rem;font-weight:800;letter-spacing:-.02em;margin:.1rem 0 .15rem;color:#f5f8fc}.live-subtitle{font-size:.78rem;color:#9fb0c7;margin-bottom:.7rem}.section-title{font-size:.92rem;font-weight:750;color:#dce6f3;margin:1rem 0 .45rem}
...
# Four buttons, two rows, so all are visible on phones.
n1,n2=st.columns(2,gap="small"); n3,n4=st.columns(2,gap="small")
n1.page_link("app.py",label="🟢 BOT STATUS",icon="🟢"); n2.page_link("pages/current_trading.py",label="📌 CURRENT TRADING",icon="📌"); n3.page_link("pages/analysis.py",label="📊 ANALYSIS",icon="📊"); n4.page_link("pages/downloads.py",label="⬇️ DOWNLOADS",icon="⬇️")
...
a,b,c=st.columns(3); a.metric("Total Scans",int(number(bot_status,"scan_count"))); b.metric("Unique Signals",len(signals) if not signals.empty else 0); c.metric("Cycle Count",int(number(bot_status,"cycle_count")))
...
if not signals.empty:
 display_cols=[c for c in ["timestamp","symbol","signal","entry","stop_loss","target","approved","reason"] if c in signals.columns]; st.dataframe(signals[display_cols].iloc[::-1].head(12),use_container_width=True,hide_index=True)
if bot_status.get("last_scan_error"):st.error(f"Last scanner error: {bot_status['last_scan_error']}")
with st.expander("SYSTEM DIAGNOSTICS",expanded=False):st.write({"Last bot cycle":fmt_time(bot_status.get("last_cycle")),"Heartbeat":fmt_time(bot_status.get("heartbeat")),"Worker":worker_alive,"Status":bot_status.get("message") or "—"})
with st.expander("STRATEGY CONFIGURATION",expanded=False):st.write(f"NIFTY 100 • Pre-09:45: liquidity + previous-day direction + today's gap • GAP_FAILURE_OPEN_RECLAIM • Entry {TRADING_START}–{LAST_ENTRY_TIME} • Square-off {SQUARE_OFF_TIME} • Risk ₹{MIN_REQUIRED_RISK:,.0f}–₹{MAX_RISK_PER_TRADE:,.0f} • R:R 1:{RISK_REWARD_RATIO:g}")
