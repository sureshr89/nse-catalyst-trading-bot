...
status=get_status();render_nav();st.title("📈 NSE Catalyst Trading Bot");st.caption("NIFTY 500 • PDH/PDL + Today's Open Return • Paper Trading")
status_value=str(status.get("status","UNKNOWN"));message=str(status.get("message","No status message available."))
...
if available_capital is None:available_capital=getattr(settings,"TOTAL_CAPITAL",0)
cards([("Bot Status",status_value),("Open Positions",int(status.get("open_positions",0) or 0)),("Available Capital",f"₹{float(available_capital or 0):,.0f}"),("Daily P&L",f"₹{float(status.get('daily_pnl',0) or 0):,.0f}")])
st.divider();left,right=st.columns(2,gap="large")
with left:
...
st.caption("Paper trading only. Live order execution is disabled.");render_daily_footer()
