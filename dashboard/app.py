...
def closed_today(df):
    ...
    dates=pd.to_datetime(out[col],errors="coerce",utc=True).dt.tz_convert("Asia/Kolkata").dt.date
...
pnl=float(closed["pnl"].sum()) if not closed.empty and "pnl" in closed.columns else 0.0
...
st.subheader("LIVE STATUS");grid([("India Time",datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%H:%M:%S")),("Bot",bot),("Worker","ALIVE" if worker else "OFFLINE"),("Scanner",status.get("scanner_status","IDLE")),("Open Positions",len(state.get("open_positions",{}) or {})),("Daily P&L",f"₹{today_pnl:,.2f}")])
...
st.subheader("TODAY'S TRADING");grid([("Closed Trades",len(today_closed)),("Wins / Losses",f"{wins} / {losses}"),("Win Rate",f"{wins/len(closed)*100:.1f}%" if len(closed) else "0.0%"),("Realized P&L",f"₹{pnl:,.2f}")])
