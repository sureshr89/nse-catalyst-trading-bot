...
if market_change >= NIFTY_THRESHOLD:
    permission, permission_note = "🟢 BUY ONLY", "NIFTY 500 ≥ +0.25%"
elif market_change <= -NIFTY_THRESHOLD:
    permission, permission_note = "🔴 SELL ONLY", "NIFTY 500 ≤ −0.25%"
else:
    permission, permission_note = "⚪ WAIT", "NIFTY 500 inside −0.25% to +0.25%"
...
if not signals.empty:
    ...
    if "approved" in display.columns:
        display = display[display["approved"].astype(str).str.lower().isin(["true", "1", "yes"])].copy()
    cols = [c for c in ["symbol", "signal", "entry_time", "entry", "stop_loss", "target", "nifty500_change_pct"] if c in display.columns]
    if display.empty or not cols:
        st.info("No approved qualifying signals today.")
    else:
        st.dataframe(display[cols].tail(20).iloc[::-1], width="stretch", hide_index=True, height=300)
else:
    st.info("No approved qualifying signals today.")
...
st.caption(f"Market-data coverage: {float(diag.get('market_data_coverage', 0) or 0) * 100:.1f}% • Auto-refresh: 5 seconds")
render_daily_footer()
