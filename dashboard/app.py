...
with c1:
    st.markdown("### 🟢 BUY")
    grid([
        ("Market", "NIFTY 500 ≥ +0.25%"),
        ("Opening", "Today's Open > PDH"),
        ("Reaction", "Price moves below PDH"),
        ("Alignment", "Industry/Sector + Stock bullish"),
    ])
with c2:
    st.markdown("### 🔴 SELL")
    grid([
        ("Market", "NIFTY 500 ≤ −0.25%"),
        ("Opening", "Today's Open < PDL"),
        ("Reaction", "Price moves above PDL"),
        ("Alignment", "Industry/Sector + Stock bearish"),
    ])
...
if not signals.empty:
    ...
    st.info("No qualifying signal yet. A row appears only after the configured strategy and alignment checks pass.")

if positions:
    st.subheader("📍 OPEN POSITIONS")
    rows = []
    for symbol, position in positions.items():
        rows.append({"Stock": symbol, "Side": str(position.get("signal", "")).upper(), "Entry": position.get("entry", "—"), "SL": position.get("stop_loss", "—"), "Target": position.get("target", "—"), "Qty": position.get("quantity", "—")})
