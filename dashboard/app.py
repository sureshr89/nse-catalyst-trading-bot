"""NSE Catalyst Trading Bot - live operations dashboard."""
...
TOTAL_CAPITAL=250000; MAX_RISK_PER_TRADE=1500; MIN_REQUIRED_RISK=1400; RISK_REWARD_RATIO=1.25; MAX_OPEN_POSITIONS=2; PAPER_TRADING=True; LIVE_TRADING=False; TRADING_START="09:45"; LAST_ENTRY_TIME="14:00"; SQUARE_OFF_TIME="15:00"; SCAN_INTERVAL_SECONDS=30
try:
    ...
    TOTAL_CAPITAL=int(settings.TOTAL_CAPITAL); MAX_RISK_PER_TRADE=float(settings.MAX_RISK_PER_TRADE); MIN_REQUIRED_RISK=float(settings.MIN_REQUIRED_RISK); RISK_REWARD_RATIO=float(settings.RISK_REWARD_RATIO); MAX_OPEN_POSITIONS=int(settings.MAX_OPEN_POSITIONS); PAPER_TRADING=bool(settings.PAPER_TRADING); LIVE_TRADING=bool(settings.LIVE_TRADING); TRADING_START=str(settings.TRADING_START); LAST_ENTRY_TIME=str(settings.LAST_ENTRY_TIME); SQUARE_OFF_TIME=str(settings.SQUARE_OFF_TIME); SCAN_INTERVAL_SECONDS=int(settings.SCAN_INTERVAL_SECONDS)
...
def dedupe_signals(df):
    ...
    out=df.copy(); dates=pd.to_datetime(out.get("timestamp",pd.Series(index=out.index)),errors="coerce").dt.strftime("%Y-%m-%d").fillna("")
...
def load_worker():
    ...
    spec=importlib.util.spec_from_file_location("nse_paper_bot_runner_final_v3",BOT_RUNNER_FILE)
    if spec is None or spec.loader is None:raise ImportError("Could not load bot_runner.py")
    module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module); starter=getattr(module,"ensure_bot_running",None)
...
status=str(bot_status.get("status","STARTING")).upper(); scanner_status=str(bot_status.get("scanner_status","IDLE")).upper(); state=read_json(STATE_FILE); open_positions=state.get("open_positions",{}) or {}; trades=read_csv(TRADES_FILE); signals=dedupe_signals(read_csv(SIGNALS_FILE))

st.markdown("""
<style>
.block-container{max-width:1420px;padding:.65rem .65rem 2rem}
[data-testid="stAppViewContainer"]{background:#0b1220}
[data-testid="stSidebar"],[data-testid="stSidebarCollapsedControl"]{display:none!important}
[data-testid="stMetric"]{background:#111b2d;border:1px solid #26344d;border-radius:10px;padding:.45rem .55rem;min-height:64px}
[data-testid="stMetricLabel"]{font-size:.64rem!important;line-height:1.1!important;color:#9fb0c7!important}
[data-testid="stMetricValue"]{font-size:1rem!important;line-height:1.2!important;font-weight:700!important;color:#f4f7fb!important}
.live-title{font-size:1.45rem;font-weight:800;margin:.1rem 0 .15rem;color:#f5f8fc}.live-subtitle{font-size:.72rem;color:#9fb0c7;margin-bottom:.7rem}.section-title{font-size:.9rem;font-weight:750;color:#dce6f3;margin:.9rem 0 .4rem}
...
@media(max-width:768px){.block-container{padding:.45rem .4rem 1.3rem}.live-title{font-size:1.15rem}.live-subtitle{font-size:.65rem;line-height:1.4}.section-title{font-size:.82rem}[data-testid="stMetric"]{min-height:54px;padding:.3rem .38rem}[data-testid="stMetricValue"]{font-size:.84rem!important}[data-testid="stMetricLabel"]{font-size:.55rem!important}}
</style>
...
a,b=st.columns(2); a.metric("India Time",now.strftime("%H:%M:%S")); b.metric("Bot",status)
a,b=st.columns(2); a.metric("Worker","ALIVE" if worker_alive else "OFFLINE"); b.metric("Scanner",scanner_status)
a,b=st.columns(2); a.metric("Open Positions",len(open_positions)); b.metric("Daily P&L",fmt_money(number(bot_status,"daily_pnl")))

st.markdown('<div class="section-title">CAPITAL & RISK</div>',unsafe_allow_html=True)
a,b=st.columns(2); a.metric("Starting Capital",fmt_money(TOTAL_CAPITAL)); b.metric("Available",fmt_money(number(bot_status,"available_capital",TOTAL_CAPITAL)))
a,b=st.columns(2); a.metric("Used",fmt_money(number(bot_status,"used_capital"))); b.metric("Risk / Trade",f"₹{MIN_REQUIRED_RISK:,.0f}–₹{MAX_RISK_PER_TRADE:,.0f}")
a,b=st.columns(2); a.metric("R:R",f"1:{RISK_REWARD_RATIO:g}"); b.metric("Max Positions",MAX_OPEN_POSITIONS)

st.markdown('<div class="section-title">OPEN POSITIONS</div>',unsafe_allow_html=True)
if open_positions:
    rows=[]
    for symbol,p in open_positions.items():
        entry=float(p.get("entry",0) or 0); stop=float(p.get("stop_loss",0) or 0); qty=int(float(p.get("quantity",0) or 0)); rows.append({"Stock":symbol,"Side":str(p.get("signal","")).upper(),"Entry":entry,"SL":stop,"Target":float(p.get("target",0) or 0),"Qty":qty,"Risk":round(abs(entry-stop)*qty,2),"R:R":p.get("risk_reward",RISK_REWARD_RATIO),"Entry Time":fmt_time(p.get("entry_time"))})
...
a,b=st.columns(2); a.metric("Closed Trades",len(closed)); b.metric("Wins / Losses",f"{wins} / {losses}")
a,b=st.columns(2); a.metric("Win Rate",f"{wins/len(closed)*100:.1f}%" if len(closed) else "0.0%"); b.metric("Realized P&L",fmt_money(float(closed["pnl"].sum()) if not closed.empty else 0))
if not closed.empty:
    cols=[c for c in ["trade_id","symbol","signal","entry_time","entry","exit_time","exit_price","exit_reason","quantity","pnl"] if c in closed.columns]; st.dataframe(closed[cols].iloc[::-1].head(12),width="stretch",hide_index=True)
...
a,b=st.columns(2); a.metric("Total Scans",int(number(bot_status,"scan_count"))); b.metric("Unique Signals",len(signals) if not signals.empty else 0)
a,b=st.columns(2); a.metric("Cycle Count",int(number(bot_status,"cycle_count"))); b.metric("Last Scan",fmt_time(bot_status.get("last_scan")))
a,b=st.columns(2); a.metric("Scan Duration",f'{number(bot_status,"scan_duration_seconds"):.2f}s'); b.metric("Last Completed",fmt_time(bot_status.get("last_scan_completed")))
if not signals.empty:
    display_cols=[c for c in ["timestamp","symbol","signal","entry","stop_loss","target","approved","reason"] if c in signals.columns]; st.dataframe(signals[display_cols].iloc[::-1].head(12),width="stretch",hide_index=True)
