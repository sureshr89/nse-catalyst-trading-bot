...
def cards(items):
    ...
    for label, value in items: html += f"<div class='metric-card'><small>{label}</small><b>{value}</b></div>"
    html += "</div>"
...
strategy_rows = [("Universe", "NIFTY 500"),("BUY", "Today's Open > PDH → price moves below PDH → return through Today's Open"),("SELL", "Today's Open < PDL → price moves above PDL → return through Today's Open"),("Market filter", "BUY ≥ +0.25% • SELL ≤ −0.25%"),("Alignment", "NIFTY 500 + Industry/Sector + Stock must support the same direction"),("Risk", "BUY SL = PDH • SELL SL = PDL • Target = 1.25 × entry-to-SL risk")]
...
if not gaps.empty and "GapType" in gaps.columns:
    ...
    with c1:
        st.markdown("### 🟢 BUY WATCHLIST"); st.caption("Today's Open above PDH — candidates requiring the remaining strategy and alignment checks")
        if ups.empty: st.info("No BUY candidates currently.")
        else:
            cols = [c for c in ["Symbol", "Industry", "TodayOpen", "PDH", "GapPercent"] if c in ups.columns]; view = ups[cols].head(30).copy()
            for col in ["TodayOpen", "PDH"]:
                if col in view.columns: view[col] = view[col].map(price)
    ...
    with c2:
        st.markdown("### 🔴 SELL WATCHLIST"); st.caption("Today's Open below PDL — candidates requiring the remaining strategy and alignment checks")
        if downs.empty: st.info("No SELL candidates currently.")
        else:
            cols = [c for c in ["Symbol", "Industry", "TodayOpen", "PDL", "GapPercent"] if c in downs.columns]; view = downs[cols].head(30).copy()
            for col in ["TodayOpen", "PDL"]:
                if col in view.columns: view[col] = view[col].map(price)
            if "GapPercent" in view.columns: view["GapPercent"] = view["GapPercent"].map(pct)
            st.dataframe(view, width="stretch", hide_index=True, height=360)
else: st.info("Candidates will populate automatically when today's market data is available.")
...
if not signals.empty:
    ...
    if approved.empty: st.info("No qualifying signals yet. Rejected or incomplete scanner records are not shown here.")
    else:
        recent = approved.tail(20).copy(); preferred = ["symbol", "signal", "entry_time", "entry", "stop_loss", "target", "nifty500_change_pct", "sector_direction", "stock_direction"]; cols = [c for c in preferred if c in recent.columns]
        if cols: st.dataframe(recent[cols].iloc[::-1], width="stretch", hide_index=True, height=260)
        else: st.info("No displayable qualifying signals yet.")
else: st.info("No qualifying signals yet. The bot will show a stock here only after every configured condition is satisfied.")
st.subheader("📍 Open Positions")
if positions:
    price_data = PriceData(); rows = []
    for symbol, position in positions.items():
        try:
            latest = price_data.get_latest_market_price(symbol); ltp = latest.get("Close") if latest else None
