...
def aline(label,value,detail):
    st.markdown(f"<div class='s1-analysis-row'><span>{label}</span><strong>{value}</strong><small>{detail}</small></div>",unsafe_allow_html=True)
...
cards([("STATUS","🟢 LIVE" if worker_ok else "🔴 STALE"),("NIFTY 500",pct(nifty_change)),("OPEN TRADES",len(positions)),("DAILY P&L",money(status.get("daily_pnl",status.get("session_pnl",0)))),("LAST SCAN",f"{scan_age}s" if scan_age is not None else "—")])
...
with st.expander("1. Before Trade",expanded=True):
    buy_ok=nifty_change is not None and float(nifty_change)>0; sell_ok=nifty_change is not None and float(nifty_change)<0
    aline("BUY market gate","PASS ✓" if buy_ok else "WAIT",f"NIFTY 500 {pct(nifty_change)} • requires > 0%"); aline("SELL market gate","PASS ✓" if sell_ok else "WAIT",f"NIFTY 500 {pct(nifty_change)} • requires < 0%"); aline("A/D Ratio",">1 BUY / <1 SELL","Full NIFTY 500 live A/D will be supplied by Dhan"); aline("BUY setup","Open > PDH → Low < PDH → return to Open","Immediate BUY; no candle close"); aline("SELL setup","Open < PDL → High > PDL → return to Open","Immediate SELL; no candle close")
with st.expander("2. Entry Confirmation",expanded=True):
    st.markdown("**BUY:** Open > PDH → Low < PDH → live price returns to Today's Open → NIFTY 500 > 0% + A/D > 1 → BUY immediately.")
    st.markdown("**SELL:** Open < PDL → High > PDL → live price returns to Today's Open → NIFTY 500 < 0% + A/D < 1 → SELL immediately.")
    st.caption("SL uses only Today's Low/High recorded before or at entry. No future value is used.")
with st.expander("3. Risk & Target",expanded=True):
    st.markdown("**SL:** Today's Low (BUY) / Today's High (SELL) • **Target:** 1.25R • **Actual risk:** ₹1,400–₹1,500"); st.caption("If no quantity produces ₹1,400–₹1,500 actual risk, reject the trade.")
