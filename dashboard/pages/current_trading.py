...
if not gaps.empty and "GapType" in gaps.columns:
    ...
    downs = g[g["GapType"].eq("GAP_DOWN")].sort_values("GapPercent", ascending=True)
    a,b = st.columns(2)
    with a:
        st.markdown("**🟢 Gap Ups**")
...
if isinstance(diag, dict) and diag:
    grid([
        ("NIFTY 500 Scanned", diag.get("stocks_scanned", 0)), ("Gap Data Ready", diag.get("gap_data_count", 0)),
        ("Gap Ups", diag.get("gap_up_count", 0)), ("Gap Downs", diag.get("gap_down_count", 0)),
        ("Liquidity Passed", diag.get("liquidity_passed", 0)), ("PDH / PDL Open Setup", diag.get("opening_setup_passed", 0)),
        ("NIFTY Market Alignment", diag.get("market_alignment_passed", 0)), ("Sector Alignment", diag.get("sector_alignment_passed", 0)),
        ("Strategy Setup", diag.get("strategy_setup_passed", 0)), ("Stock Alignment", diag.get("stock_alignment_passed", 0)),
        ("FINAL SIGNALS", diag.get("final_signals", 0)),
    ])
    rejection_labels = [("PDH / PDL not reached", "pdh_pdl_not_reached"), ("No Open Cross", "no_open_cross"), ("Sector Alignment", "sector_alignment"), ("Stock Alignment", "stock_alignment"), ("Strategy Setup", "strategy_setup"), ("NIFTY Alignment", "market_alignment"), ("Opening Setup", "opening_setup"), ("Liquidity", "liquidity"), ("Missing Data", "missing_data")]
    ...
    if ranked:
        st.subheader("Top Rejection Reasons"); grid(ranked)
else:
