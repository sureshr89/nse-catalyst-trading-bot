...
status=read(ROOT/"outputs/bot_status.json","json"); state=read(ROOT/"outputs/paper_engine_state.json","json"); trades=read(ROOT/"outputs/trades.csv","csv"); diag=read(ROOT/"outputs/scanner_diagnostics.json","json"); gaps=read(ROOT/"outputs/gap_analysis.csv","csv")
...
if not gaps.empty and "GapType" in gaps.columns:
    g=gaps.copy(); g["GapPercent"]=pd.to_numeric(g["GapPercent"],errors="coerce"); ups=g[g["GapType"].eq("GAP_UP")].sort_values("GapPercent",ascending=False); downs=g[g["GapType"].eq("GAP_DOWN")].sort_values("GapPercent")
    a,b=st.columns(2)
    with a: st.markdown("**🟢 Gap Ups**"); st.dataframe(ups[[c for c in ["Symbol","PreviousClose","TodayOpen","GapPercent","PDH","PDL"] if c in ups.columns]].head(25),width="stretch",hide_index=True,height=320)
    with b: st.markdown("**🔴 Gap Downs**"); st.dataframe(downs[[c for c in ["Symbol","PreviousClose","TodayOpen","GapPercent","PDH","PDL"] if c in downs.columns]].head(25),width="stretch",hide_index=True,height=320)
else: st.info("Gap board will be prepared automatically during the pre-entry phase and completed before 09:45 IST.")
...
if isinstance(diag,dict) and diag:
    ...
    ranked=sorted(((label,int((diag.get("rejections",{}) or {}).get(key,0) or 0)) for label,key in labels),key=lambda x:x[1],reverse=True); ranked=[x for x in ranked if x[1]>0]
    if ranked: st.subheader("Top Rejection Reasons"); grid(ranked)
else: st.info("Scanner diagnostics will appear after the next cycle.")

st.subheader("Latest Closed Trade")
if not closed.empty:
    t=closed.iloc[-1]; grid([("Stock",t.get("symbol","—")),("Side",t.get("signal","—")),("Entry",t.get("entry","—")),("Exit",t.get("exit_price","—")),("P&L",f"₹{float(t.get('pnl',0) or 0):,.2f}"),("Exit Reason",t.get("exit_reason","—")),("PDH",t.get("pdh","—")),("PDL",t.get("pdl","—")),("Open",t.get("today_open","—")),("Gap %",t.get("gap_percent","—")),("Setup",t.get("setup_type","—")),("Sector",t.get("sector","—")),("Market",t.get("market_direction","—"))])
