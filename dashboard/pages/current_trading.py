...
if pos:
    ...
    for symbol,p in pos.items():
        rows.append({
            "Stock":symbol,"Side":str(p.get("signal","")).upper(),"Entry":p.get("entry"),
            "SL":p.get("stop_loss"),"Target":p.get("target"),"Qty":p.get("quantity"),
            "Risk":p.get("actual_risk",p.get("risk")),"R:R":p.get("rr",p.get("risk_reward",1.25)),
            "Entry Time":p.get("entry_time"),"Setup":p.get("setup_type","GAP_FAILURE_OPEN_RECLAIM"),
            "PDC":p.get("pdc"),"Today Open":p.get("today_open"),"Today Low":p.get("today_low"),"Today High":p.get("today_high"),
        })
