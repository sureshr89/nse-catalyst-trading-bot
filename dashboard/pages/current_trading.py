...
status=read(ROOT/"outputs/bot_status.json"); state=read(ROOT/"outputs/paper_engine_state.json"); diag=read(ROOT/"outputs/scanner_diagnostics.json")
gaps=read(ROOT/"outputs/gap_analysis.csv","csv"); signals=read(ROOT/"outputs/signals.csv","csv")
...
st.caption("Live strategy command center • today's candidates, approved signals and open positions")
cards([("NIFTY 500",pct(market_change)),("Market Permission",permission),("Entry Window",window),("Open Positions",len(positions))])
st.caption(f"Entries {ENTRY_START}–{ENTRY_END} IST • Updated {now.strftime('%H:%M:%S')} IST")
if status.get("error"):st.warning(str(status["error"]))

st.subheader("⚡ Strategy State")
st.dataframe(pd.DataFrame([
    ("Universe","NIFTY 500"),
    ("BUY","Open above PDH → price moves below PDH → return above Today's Open"),
    ("SELL","Open below PDL → price moves above PDL → return below Today's Open"),
    ("Market filter","BUY ≥ +0.25% • SELL ≤ −0.25% NIFTY 500"),
    ("Risk","SL = PDH/PDL • Target = 1.25 × entry-to-SL risk"),
...
if signals.empty:st.info("No approved signals yet.")
else:
    ...
    if approved.empty or not cols:st.info("No approved signals yet.")
