...
nifty_value, live_change = live_nifty500()
if live_change is not None:
    market_change = live_change
else:
    raw = diag.get("nifty500_change_pct")
    market_change = None if raw in (None, "") else float(raw)
...
cards([
    ("NIFTY 500 VALUE", price(nifty_value) if nifty_value is not None else "Unavailable"),
    ("NIFTY 500 CHANGE", pct(market_change) if market_change is not None else "Unavailable"),
...
st.dataframe(pd.DataFrame([
    ...
    ("News", "POSITIVE for BUY / NEGATIVE for SELL; NEUTRAL or unusable news rejects"),
    ("Risk", "₹1,400–₹1,500 per trade • SL PDH/PDL • Target 1.25R • Max 2"),
], columns=["Condition","Current Rule"]), width="stretch", hide_index=True)

st.subheader("📰 News Gate")
if not news.empty:
    today_news = news[news["TradeDate"].astype(str).eq(now.strftime("%Y-%m-%d"))].copy() if "TradeDate" in news.columns else news.iloc[0:0]
    sentiment = today_news.get("news_sentiment", pd.Series(dtype=str)).astype(str).str.upper()
    a,b,c,d = st.columns(4); a.metric("Decisions",len(today_news)); b.metric("🟢 Positive",int(sentiment.eq("POSITIVE").sum())); c.metric("🔴 Negative",int(sentiment.eq("NEGATIVE").sum())); d.metric("⚪ Neutral",int(sentiment.eq("NEUTRAL").sum()))
...
if not gaps.empty and "GapType" in gaps.columns:
    ...
    board["GapPriority"]=board["GapPercent"].abs() if "GapPercent" in board.columns else 0
