...
st.caption("Closed Strategy 2 trades only • isolated ₹2,50,000 capital • Gap-Up Extension Reversal SELL")
...
st.dataframe(pd.DataFrame([
    ...
    ("Trigger", "First completed 1-minute CLOSE below Today's Open after price extended above Open"),
    ("SELL", "SELL only"),
    ("SL / Target", "Today's High at trigger / PDH"),
    ("Ranking", "Largest opening GAP % from Previous Day Close"),
...
with tabs[1]:
    if df.empty: st.info("No completed trades yet.")
        ...
        with b:
            roll = df[["Trade #", "pnl"]].copy(); roll["Rolling Avg"] = roll["pnl"].rolling(5, min_periods=1).mean()
...
with tabs[3]:
    if df.empty: st.info("No GAP results yet.")
        ...
        with b: chart(px.scatter(gap, x="Gap Magnitude %", y="pnl", hover_data=["symbol"], title="GAP vs P&L"), "s2_gap_pnl")
with tabs[4]:
    if df.empty: st.info("No risk results yet.")
    else:
        a, b = st.columns(2)
        with a: chart(px.histogram(df, x="rr", nbins=12, title="Recorded Risk:Reward"), "s2_rr")
        with b: chart(px.scatter(df, x="risk_per_share", y="pnl", hover_data=["symbol"], title="Risk per Share vs P&L"), "s2_risk")
with tabs[5]:
