...
count = len(df); wins = int((df["pnl"] > 0).sum()) if count else 0; losses = int((df["pnl"] < 0).sum()) if count else 0
...
win_rate = wins / count * 100 if count else 0.0; profit_factor = gross_profit / gross_loss if gross_loss else 0.0
max_dd = abs(float(df["Drawdown"].min())) if count else 0.0

st.title("📊 Strategy 2 — Complete Analysis")
st.caption("Closed Strategy 2 trades only • isolated ₹2,50,000 capital • Gap Extension Reversal BUY + SELL")
html = '<div class="analysis-kpi-grid">'
for label, value in [("Starting Capital", f"₹{STARTING_CAPITAL:,.0f}"),("Closed Trades", count),("Wins / Losses", f"{wins} / {losses}"),("Net P&L", f"₹{net:,.2f}"),("Equity", f"₹{STARTING_CAPITAL + net:,.2f}"),("Win Rate", f"{win_rate:.1f}%"),("Profit Factor", f"{profit_factor:.2f}"),("Max Drawdown", f"₹{max_dd:,.2f}")]:
    html += f'<div class="analysis-kpi"><span>{label}</span><strong>{value}</strong></div>'
html += "</div>"; st.markdown(html, unsafe_allow_html=True)

st.subheader("⚡ Rules Analysed")
st.dataframe(pd.DataFrame([
    ("SELL", "Open > PDH → extension above Open after 09:45 → first completed 1m CLOSE below Open"),
    ("SELL SL / Target", "Today's High at trigger / PDH"),
    ("BUY", "Open < PDL → extension below Open after 09:45 → first completed 1m CLOSE above Open"),
    ("BUY SL / Target", "Today's Low at trigger / PDL"),
    ("Ranking", "Largest absolute opening GAP % from Previous Day Close"),
    ("Entry", "09:45–14:00 IST"),
...
with tabs[0]:
    if df.empty: st.info("No completed Strategy 2 trades yet.")
    else:
        a,b=st.columns(2)
...
with tabs[2]:
    if df.empty or "symbol" not in df.columns: st.info("No stock-level results yet.")
    else:
        ...
        chart(px.bar(stock.head(20),x="symbol",y="PnL",text="Trades",title="Stocks by P&L"),"s2_stocks"); st.dataframe(stock,use_container_width=True,hide_index=True)
with tabs[3]:
    if df.empty: st.info("No GAP results yet.")
    else:
...
with tabs[4]:
    if df.empty: st.info("No risk results yet.")
    else:
        a,b=st.columns(2)
...
with tabs[5]:
    if df.empty or "entry_time" not in df.columns: st.info("No timing results yet.")
    else:
        ...
        timing=pd.DataFrame({"Entry Minute":dates.dt.hour*60+dates.dt.minute,"P&L":df["pnl"]}); chart(px.scatter(timing,x="Entry Minute",y="P&L",title="Entry Time vs P&L"),"s2_timing")
with tabs[6]:
    if df.empty: st.info("No completed trades yet.")
    else: st.dataframe(df.tail(200).iloc[::-1],use_container_width=True,hide_index=True,height=450)
