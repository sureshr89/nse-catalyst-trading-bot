    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        for name, (frame, cols) in sheets.items():
            ...
            if name == "Gap Board" and not data.empty:
                keep = [
                    c for c in [
                        "TradeDate", "Symbol", "PreviousClose", "TodayOpen", "Gap", "GapPercent",
                        "GapType", "PDH", "PDL", "GapFromPreviousClose", "GapPercentFromPreviousClose",
                        "PreviousDayTurnover", "DataSnapshotIST"
            ...
            for col in ws.columns:
                ...
                ws.column_dimensions[col[0].column_letter].width = min(max(max((len(v) for v in values), default=10) + 2, 10), 32)
            for cell in ws[1]:
...
st.download_button(
    ...
    data=monthly_excel(selected),
    file_name=f"NSE_CATALYST_MASTER_TRADING_DATA_{selected}.xlsx",
...
st.download_button(
    "⬇️ BOT STATUS JSON",
    data=json_bytes("bot_status.json", {"status": "WAITING", "worker_alive": False}),
    file_name="nifty500_bot_status.json",
