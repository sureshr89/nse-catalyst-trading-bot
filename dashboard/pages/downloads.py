...
def monthly_excel(month):
    ...
    with pd.ExcelWriter(output,engine="openpyxl") as writer:
        ...
        pd.DataFrame([["Month",month],["Strategy","NIFTY 500 + Industry/Sector + Stock alignment"],["BUY","Open above PDH → price moves below PDH → price returns above Today's Open → NIFTY 500 ≥ +0.25%"],["SELL","Open below PDL → price moves above PDL → price returns below Today's Open → NIFTY 500 ≤ −0.25%"],["Alignment","NIFTY 500, Industry/Sector and Stock must support the same direction"],["Risk","BUY SL = PDH; SELL SL = PDL; Target = 1.25 × entry-to-SL risk"],["Generated",pd.Timestamp.now(tz="Asia/Kolkata").strftime("%Y-%m-%d %H:%M:%S IST")]],columns=["Field","Value"]).to_excel(writer,sheet_name="README",index=False)
...
st.caption("Download your trading records, daily market data, gap board and strategy results.")
...
if not gaps.empty and "GapType" in gaps.columns:
    ...
    with a:
        ...
        st.dataframe(g[g["GapType"].eq("GAP_UP")].sort_values("GapPercent",ascending=False).head(30),width="stretch",hide_index=True,height=350)
    with b:
        st.markdown("**🔴 Gap Down — Open < PDL**")
        st.dataframe(g[g["GapType"].eq("GAP_DOWN")].sort_values("GapPercent").head(30),width="stretch",hide_index=True,height=350)
else:
    st.info("Gap board will appear after current market data is available.")

st.caption("Downloads are generated from the bot's recorded paper-trading and market-data files. No live orders are placed from this page.")
render_daily_footer()
