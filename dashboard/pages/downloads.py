...
st.caption("NIFTY 500 trading records, scanner diagnostics and sector classification.")
st.subheader("Trading Data")
trades_data = csv_bytes("trades.csv", ["status", "symbol", "entry_time", "exit_time", "pnl", "sector"])
signals_data = csv_bytes("signals.csv", ["timestamp", "symbol", "signal", "entry", "sector"])
...
st.subheader("🏭 NIFTY 500 Sector-wise Stock Classification")
try:
...
if not mapping.empty:
    ...
    st.download_button("⬇️ NIFTY 500 SECTOR-WISE STOCK LIST CSV", data=mapping.to_csv(index=False).encode("utf-8"), file_name="nifty500_sector_wise_stock_list.csv", mime="text/csv", key="download_sector_stock_list", width="stretch")
    st.download_button("⬇️ NIFTY 500 SECTOR SUMMARY CSV", data=summary.to_csv(index=False).encode("utf-8"), file_name="nifty500_sector_summary.csv", mime="text/csv", key="download_sector_summary", width="stretch")
    ...
    st.warning("NIFTY 500 sector mapping is not available yet. The page will retry on the next refresh.")
