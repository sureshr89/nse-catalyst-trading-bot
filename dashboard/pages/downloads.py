...
def monthly_excel(month):
    ...
    sheets={"Daily Stock Inputs":(daily,["TradeDate","DataSnapshotIST"]),"All Trades":(trades,["TradeDate","entry_time","exit_time"]),"Daily Summary":(summary,["TradeDate"]),"Gap Board":(daily,["TradeDate"]),"Signals":(signals,["timestamp","entry_time"])}
    with pd.ExcelWriter(output,engine="openpyxl") as writer:
        for name,(frame,cols) in sheets.items():
            data=month_filter(frame,month,cols)
            if name=="Gap Board" and not data.empty:
                keep=[c for c in ["TradeDate","Symbol","PreviousClose","TodayOpen","Gap","GapPercent","GapType","PDH","PDL","GapFromPreviousClose","GapPercentFromPreviousClose","PreviousDayTurnover","LiquidityQualified","DataSnapshotIST"] if c in data.columns];data=data[keep]
        ...
        pd.DataFrame([["Month",month],["Strategy","NIFTY 500 direct 1-minute PDH/PDL + Today's Open reversal"],["BUY","Open > PDH → price below PDH → return to Open → NIFTY 500 ≥ +0.25%"],["SELL","Open < PDL → price above PDL → return to Open → NIFTY 500 ≤ −0.25%"],["Risk","BUY SL = PDH; SELL SL = PDL; Target = 1.25 × entry-to-SL risk"],["Generated",pd.Timestamp.now(tz="Asia/Kolkata").strftime("%Y-%m-%d %H:%M:%S IST")]],columns=["Field","Value"]).to_excel(writer,sheet_name="README",index=False)
...
st.title("⬇️ Downloads");st.caption("NIFTY 500 1-minute price strategy records, PDH/PDL gap board, trades and diagnostics.")
trades=canonical_trades(read_csv("trades.csv"));signals=read_csv("signals.csv");gaps=read_csv("gap_analysis.csv");months_list=months()
st.subheader("⭐ Master Trading Data — Last 6 Months")
rows=[]
master=canonical_trades(read_csv("MASTER_TRADES.csv"));daily=read_csv("MASTER_DAILY_STOCK_DATA.csv")
for month in months_list:
    rows.append({"Month":pd.Timestamp(month+"-01").strftime("%B %Y"),"File":f"NSE_CATALYST_MASTER_TRADING_DATA_{month}.xlsx","Records":len(month_filter(master,month,["TradeDate","entry_time","exit_time"]))+len(month_filter(daily,month,["TradeDate"]))})
st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
selected=st.selectbox("📅 Select a month",months_list,format_func=lambda x:pd.Timestamp(x+"-01").strftime("%B %Y"));st.download_button("⬇️ DOWNLOAD MASTER EXCEL",data=monthly_excel(selected),file_name=f"NSE_CATALYST_MASTER_TRADING_DATA_{selected}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",width="stretch")
...
st.download_button("⬇️ SCANNER DIAGNOSTICS JSON",data=json_bytes("scanner_diagnostics.json",{"stocks_scanned":0,"market_data_coverage":0,"nifty500_change_pct":0,"final_signals":0}),file_name="nifty500_scanner_diagnostics.json",mime="application/json",width="stretch")
...
else:st.info("Gap board will appear after current 1-minute market data is available.")
render_daily_footer()
