...
def signal_quality(row):
    ...
    score, reasons = 0, []
    if required and str(row.get("market_direction", "")).upper() == required:
        score += 30; reasons.append("NIFTY 500 aligned")
    if required and str(row.get("sector_direction", "")).upper() == required:
        score += 30; reasons.append("Sector aligned")
    if required and str(row.get("stock_direction", row.get("stock_today_direction", ""))).upper() == required:
        score += 20; reasons.append("Stock aligned")
    try:
        gap = abs(float(row.get("gap_percent", 0) or 0))
        if gap > 0:
            score += 10; reasons.append(f"Gap {gap:.2f}%")
    except Exception:
        pass
    try:
        ...
        if not pd.isna(entry):
            ...
            if 585 <= minute <= 615:
                score += 10; reasons.append("09:45–10:15 entry")
            elif 615 < minute <= 660:
                score += 5; reasons.append("10:15–11:00 entry")
    except Exception:
        pass
...
def empty_chart(title, key, height=310):
    fig = go.Figure(); fig.update_layout(height=height, template="plotly_dark", title=title, margin=dict(l=8,r=8,t=45,b=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", xaxis=dict(visible=False), yaxis=dict(visible=False))
    fig.add_annotation(text="No completed paper-trade data yet", x=.5, y=.5, xref="paper", yref="paper", showarrow=False); chart(fig,key,height)
...
tabs=st.tabs(["📌 Overview","💰 P&L","⏱️ Time","🎯 Setup","🏆 Stocks","🌐 Market & Sectors","⚖️ Risk / Reward","📋 Trades"])
...
with tabs[0]:
    ...
    if not actual.empty:
        ...
        st.subheader("Why the Latest Trade Was Taken")
        latest=actual.iloc[-1]; st.write(f"**{latest.get('symbol','—')} {latest.get('signal','—')} — Quality {latest.get('Quality Score',0)}/100**"); st.write(f"• {latest.get('Why This Trade','Recorded setup context only')}")
...
with tabs[5]:
    st.subheader("NIFTY 500 / Sector / Stock Alignment")
    a,b,c=st.columns(3)
    with a: grouped_chart(actual,"market_direction","P&L by NIFTY 500 Direction","market_dir")
    with b: grouped_chart(actual,"sector_direction","P&L by Sector Direction","market_sector_dir")
    with c: grouped_chart(actual,"stock_today_direction","P&L by Stock Direction","market_stock")
    st.markdown("### All Sectors")
    if not actual.empty and "sector" in actual.columns:
        sector=actual.assign(sector=actual["sector"].fillna("Unknown").replace("","Unknown")).groupby("sector",as_index=False).agg(Trades=("pnl","size"),Wins=("pnl",lambda x:(x>0).sum()),Win_Rate=("pnl",lambda x:(x>0).mean()*100),Net_PnL=("pnl","sum")).sort_values("Net_PnL",ascending=False)
        st.dataframe(sector,use_container_width=True,hide_index=True,height=min(520,max(220,35+len(sector)*35)))
    elif not actual.empty and "sector_direction" in actual.columns:
        st.info("Trade records contain sector direction but not sector names; all available sector-direction groups are shown above. Future trades will populate named-sector analysis when the journal provides the sector field.")
    else: st.info("No sector trade data yet.")
    if not actual.empty and "sector" in actual.columns:
        st.download_button("⬇️ DOWNLOAD SECTOR ANALYSIS CSV",data=sector.to_csv(index=False).encode("utf-8"),file_name="nifty500_sector_analysis.csv",mime="text/csv",width="stretch")
