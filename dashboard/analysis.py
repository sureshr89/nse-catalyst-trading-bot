...
def empty_chart(title, key, height=330):
    fig = go.Figure(); fig.update_layout(height=height, template="plotly_dark", title=title, margin=dict(l=10, r=10, t=55, b=15), font=dict(family="Inter, sans-serif"), xaxis=dict(visible=False), yaxis=dict(visible=False))
    fig.add_annotation(text="No completed paper-trade data yet", x=.5, y=.5, xref="paper", yref="paper", showarrow=False, font=dict(size=14)); chart(fig, key, height)
...
c1, c2, c3, c4 = st.columns(4); c1.metric("Starting Capital", f"₹{STARTING_CAPITAL:,.0f}"); c2.metric("Closed Trades", s["Trades"]); c3.metric("Net P&L", f"₹{s['P&L']:,.2f}"); c4.metric("Current Equity", f"₹{STARTING_CAPITAL+s['P&L']:,.2f}")
c1, c2, c3, c4 = st.columns(4); c1.metric("Win Rate", f"{s['Win Rate %']:.1f}%"); c2.metric("Wins", s["Wins"]); c3.metric("Losses", s["Losses"]); c4.metric("Profit Factor", f"{s['Profit Factor']:.2f}")

st.subheader("Performance Charts")
if actual.empty:
    a, b = st.columns(2)
    with a: empty_chart("Cumulative P&L", "cumulative_pnl")
    with b: empty_chart("P&L per Trade", "trade_pnl")
    a, b = st.columns(2)
    with a: empty_chart("Drawdown", "drawdown")
    with b: empty_chart("Win / Loss / Flat Mix", "result_mix")
else:
    time_col = next((c for c in ["exit_time", "entry_time"] if c in actual.columns), None)
    if time_col:
        actual["_time"] = pd.to_datetime(actual[time_col], errors="coerce"); actual = actual.sort_values("_time")
    actual["Trade #"] = range(1, len(actual)+1); actual["Cumulative P&L"] = actual["pnl"].cumsum(); actual["Drawdown"] = actual["Cumulative P&L"]-actual["Cumulative P&L"].cummax()
    a, b = st.columns(2)
...
a, b = st.columns(2)
if not actual.empty and "_time" in actual.columns and actual["_time"].notna().any():
    daily = actual.dropna(subset=["_time"]).copy(); daily["Date"] = daily["_time"].dt.strftime("%d %b"); daily = daily.groupby("Date", sort=False, as_index=False)["pnl"].sum()
...
if not actual.empty and "entry_time" in actual.columns and "exit_time" in actual.columns:
    entry_t = pd.to_datetime(actual["entry_time"], errors="coerce"); exit_t = pd.to_datetime(actual["exit_time"], errors="coerce"); actual["Duration (min)"] = (exit_t-entry_t).dt.total_seconds()/60; duration = actual.dropna(subset=["Duration (min)"])
    with b:
        if duration.empty: empty_chart("Trade Duration", "duration")
        else: chart(px.bar(duration, x="Trade #", y="Duration (min)", title="Trade Duration"), "duration")
else:
...
analysis_cols = ["market_direction", "sector_direction", "stock_today_direction", "exit_reason"]
for start in range(0, len(analysis_cols), 2):
    row = st.columns(2)
    for j, column in enumerate(analysis_cols[start:start+2]):
        with row[j]:
            if not actual.empty and column in actual.columns:
                ...
                empty_chart(f"P&L by {column.replace('_', ' ').title()}", f"group_{column}", 320)
...
else: st.dataframe(signals.iloc[::-1], width="stretch", hide_index=True, height=420)
