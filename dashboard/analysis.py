...
ROOT=Path(__file__).resolve().parent.parent;TRADES=ROOT/"outputs"/"trades.csv";SIGNALS=ROOT/"outputs"/"signals.csv";STARTING_CAPITAL=250000.0;IST=ZoneInfo("Asia/Kolkata")
st.set_page_config(page_title="NSE Catalyst | Analysis",page_icon="📊",layout="wide");st.markdown(load_css(),unsafe_allow_html=True)
def read_csv(path):
    try:return pd.read_csv(path)
    except (FileNotFoundError,pd.errors.EmptyDataError,OSError):return pd.DataFrame()
def to_ist(series):
    """Parse timestamps consistently as IST; naive timestamps are already treated as IST."""
    parsed=pd.to_datetime(series,errors="coerce")
    if getattr(parsed.dt,"tz",None) is None:return parsed.dt.tz_localize(IST,ambiguous="NaT",nonexistent="NaT")
...
def prepare(df):
    ...
    df["Result"]=df["pnl"].apply(lambda x:"WIN" if x>0 else "LOSS" if x<0 else "FLAT");return df
...
def chart(fig,key,height=310):
    fig.update_layout(height=height,margin=dict(l=8,r=8,t=45,b=10),template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",font=dict(family="Inter, sans-serif",size=12),hovermode=False);fig.update_xaxes(fixedrange=True);fig.update_yaxes(fixedrange=True);st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False,"scrollZoom":False,"doubleClick":False,"staticPlot":True,"responsive":True},key=key)
...
trades=prepare(read_csv(TRADES));actual=trades[trades["status"].astype(str).str.upper().eq("CLOSED")].copy() if not trades.empty and "status" in trades.columns else pd.DataFrame();count,wins,losses,win_rate,pnl_total,profit_factor=stats(actual)
if not actual.empty:
    tc=next((c for c in ["exit_time","entry_time"] if c in actual.columns),None)
    if tc:actual["_time"]=to_ist(actual[tc]);actual=actual.sort_values("_time")
...
st.title("📊 NIFTY 500 Strategy Analysis");st.caption("PDH/PDL reaction → today's Open 1-minute reversal • Closed paper trades only • No live orders")
kpis=[("Starting Capital",f"₹{STARTING_CAPITAL:,.0f}"),("Closed Trades",count),("Net P&L",f"₹{pnl_total:,.2f}"),("Current Equity",f"₹{STARTING_CAPITAL+pnl_total:,.2f}"),("Win Rate",f"{win_rate:.1f}%"),("Wins",wins),("Losses",losses),("Profit Factor",f"{profit_factor:.2f}")];st.markdown('<div class="analysis-kpi-grid">'+''.join(f'<div class="analysis-kpi"><span>{a}</span><strong>{b}</strong></div>' for a,b in kpis)+'</div>',unsafe_allow_html=True)
...
with tabs[0]:
    ...
    a,b=st.columns(2)
    with a:empty_chart("Win / Loss / Flat Mix","ov_mix") if actual.empty else chart(px.pie(actual["Result"].value_counts().rename_axis("Result").reset_index(name="Trades"),names="Result",values="Trades",title="Win / Loss / Flat Mix"),"ov_mix")
    ...
    if not actual.empty:
        st.subheader("Signal Quality — Research Only");st.caption("Descriptive score only; it never approves, rejects, or changes a trade.");st.dataframe(actual[[c for c in ["symbol","signal","entry_time","Quality Score","Why This Trade","pnl"] if c in actual.columns]].iloc[::-1].head(50),use_container_width=True,hide_index=True);st.subheader("Why the Latest Trade Was Taken");latest=actual.iloc[-1];st.write(f"**{latest.get('symbol','—')} {latest.get('signal','—')} — Quality {latest.get('Quality Score',0)}/100**");st.write(f"• {latest.get('Why This Trade','Recorded setup context only')}");st.write(f"• Entry ₹{float(latest.get('entry',0) or 0):,.2f} | SL ₹{float(latest.get('stop_loss',0) or 0):,.2f} | Target ₹{float(latest.get('target',0) or 0):,.2f}")
with tabs[1]:
...
with tabs[2]:
    ...
    if not actual.empty and "entry_time" in actual.columns and "exit_time" in actual.columns:
        x=actual.copy();x["Duration (min)"]=(to_ist(x["exit_time"])-to_ist(x["entry_time"])).dt.total_seconds()/60;x=x.dropna(subset=["Duration (min)"])
    ...
    if not actual.empty and "entry_time" in actual.columns:
        t=actual.copy();t["Entry Window"]=to_ist(t["entry_time"]).dt.floor("30min").dt.strftime("%H:%M");g=t.groupby("Entry Window",dropna=False).agg(Trades=("pnl","size"),Win_Rate=("pnl",lambda x:(x>0).mean()*100),Net_PnL=("pnl","sum")).reset_index();st.dataframe(g,use_container_width=True,hide_index=True)
with tabs[3]:
...
with tabs[5]:
    st.subheader("NIFTY 500 Market Alignment");a,b=st.columns(2)
    ...
    if not actual.empty:st.dataframe(actual[[c for c in ["symbol","signal","market_direction","stock_today_direction","entry_time","pnl"] if c in actual.columns]].iloc[::-1].head(100),use_container_width=True,hide_index=True)
with tabs[6]:
