...
if not trades.empty:
    trades["strategy"]=trades.get("strategy","").map(normalize_strategy)
...
full_breadth=coverage=="500/500"; sector_ok=bool(diag.get("sector_available")) and sector_map=="500/500" and sector_priced=="500/500"
buy=bool(diag.get("buy_alignment")); sell=bool(diag.get("sell_alignment")); bias="🟢 BUY" if buy else "🔴 SELL" if sell else "⚪ NO TRADE"
...
st.markdown("<div style='display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:9px'>"+"".join([
 k("NIFTY 500",pct(nifty),"good"),k("SECTOR ALIGNMENT",pct(sector)),k("NIFTY 500 A/D",f"{float(ad):.2f}" if full_breadth and ad is not None else "UNAVAILABLE"),k("A/D COVERAGE",coverage),k("SECTOR COVERAGE",sector_priced),k("MASTER BIAS",bias)
...
for col,s in zip(cols,STRATEGIES):
    with col:
        ...
        if not td.empty and "entry_time" in td:
            dt=pd.to_datetime(td["entry_time"],errors="coerce");today_td=td[dt.dt.date==now.date()]
        wins=int((today_td["pnl"]>0).sum()) if not today_td.empty and "pnl" in today_td else 0; losses=int((today_td["pnl"]<0).sum()) if not today_td.empty and "pnl" in today_td else 0; pnl=float(today_td["pnl"].sum()) if not today_td.empty and "pnl" in today_td else 0
        locked=len(today_td)>=MAX_TRADES or pnl<=-DAILY_LOSS; status="🔒 LOCKED" if locked else "🟢 ALIGNED" if (buy or sell) else "⚪ WAITING"
...
if not trades.empty and "pnl" in trades:
    ...
    seq=trades.copy();seq["entry_time"]=pd.to_datetime(seq.get("entry_time"),errors="coerce");seq=seq.dropna(subset=["entry_time"]).sort_values("entry_time");seq["Cumulative P&L"]=seq.groupby("strategy")["pnl"].cumsum()
...
try:
    ...
    path=build_journal();df=pd.read_csv(path);st.dataframe(df.tail(25),use_container_width=True,hide_index=True);st.download_button("⬇️ Download Master Strategy Journal CSV",path.read_bytes(),"strategy_journal_master.csv","text/csv")
