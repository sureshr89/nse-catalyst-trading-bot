...
def _aligned_buy(snap, idx):
    """TEST ONLY: require NIFTY positive + A/D > 1 + positive sectors > negative sectors."""
...
def _update_test_trade(rows, now):
    ...
    reason = None
    if ltp <= state["sl"]: reason = "SL"
    elif ltp >= state["target"]: reason = "TARGET"
    elif now.time() >= FORCE_EXIT: reason = "2:45 PM TIME EXIT"
    if reason:
...
def _render_test_trade(rows, snap, idx):
    ...
    if state.get("status") in {"OPEN", "CLOSED"}:
        ...
        pnl = state.get("pnl")
        a,b,c,d = st.columns(4); a.metric("Stock / Side", f"{state.get('symbol','—')} / BUY"); b.metric("Entry", f"₹{_fmt(state.get('entry'))}"); c.metric("Current / Exit", f"₹{_fmt(price)}"); d.metric("P&L", f"₹{_fmt(pnl) if pnl is not None else '—'}")
        a,b,c,d = st.columns(4); a.metric("Entry Time", state.get("entry_time", "—")[11:19] if state.get("entry_time") else "—"); b.metric("SL", f"₹{_fmt(state.get('sl'))}"); c.metric("Target", f"₹{_fmt(state.get('target'))}"); d.metric("Exit Time", state.get("exit_time", "—")[11:19] if state.get("exit_time") else "—")
...
def render_test_tab():
    ...
    try:
        ...
        snap = BREADTH.snapshot(force=False); q = snap.get("quote_rows")
        ...
        for col,(label,ok) in zip(cols,checks):
            with col: st.markdown(f'<div class="test-card"><div class="test-label">{label}</div><div class="test-status {"pass" if ok else "wait"}">{"PASS" if ok else "WAIT"}</div></div>',unsafe_allow_html=True)
        st.markdown("### 2. NIFTY 500 live index")
        if idx:
            ltp=float(idx.get("LTP") or 0); prev=float(idx.get("PreviousClose") or 0); net=float(idx.get("NetChange") or (ltp-prev)); pct=(net/prev*100) if prev else 0
            a,b,c,d=st.columns(4); a.metric("LTP",f"₹{ltp:,.2f}"); b.metric("Previous Close",f"₹{prev:,.2f}"); c.metric("Change",f"{net:+,.2f}"); d.metric("Change %",f"{pct:+.2f}%")
        st.markdown("### 3. Breadth & sectors")
        a,b,c,d,e=st.columns(5); a.metric("Advances",int(snap.get("advances",0) or 0)); b.metric("Declines",int(snap.get("declines",0) or 0)); c.metric("Unchanged",int(snap.get("unchanged",0) or 0)); d.metric("A / D",_fmt(snap.get("ad_ratio"),2) if snap.get("ad_ratio") is not None else "—"); e.metric("Sectors + / −",f"{int(snap.get('positive_sectors',0) or 0)} / {int(snap.get('negative_sectors',0) or 0)}")
