...
def _state(today):
    ...
    if not s or s.get("date")!=today:
        s=_load(today);st.session_state["nse_test_trade"]=s
    return s

def _alignment(snap,idx):
    if not idx or snap.get("ad_ratio") is None:return None
    try:
        net=float(idx.get("NetChange") or 0);ad=float(snap.get("ad_ratio") or 0)
        pos=int(snap.get("positive_sectors",0) or 0);neg=int(snap.get("negative_sectors",0) or 0)
...
def _candidate(rows,side):
    ...
    w=w.dropna(subset=["Symbol","LTP","TodayOpen"])
    w=w[w["LTP"]>w["TodayOpen"]] if side=="BUY" else w[w["LTP"]<w["TodayOpen"]]
    if w.empty:return None
    r=w.sort_values("Symbol").iloc[0]
...
def _update(rows,now,s):
    ...
    try:
        ...
        if not r.empty:
            ltp=float(r.iloc[0]["LTP"]);s["last_ltp"]=ltp;s["last_update_time"]=now.isoformat()
            s["pnl_live"]=ltp-float(s["entry"]) if s.get("side")=="BUY" else float(s["entry"])-ltp
    except Exception:pass
...
def _live_test(rows,snap,idx):
    ...
    if s.get("status")=="WAITING" and ENTRY_START<=now.time()<FORCE_EXIT:
        ...
        st.info("Waiting for 09:15 AM IST." if now.time()<ENTRY_START else "Today's entry window is closed; no late entry will be created.")
    ...
    if s.get("status") in {"OPEN","CLOSED"}:
        ...
        html="<div class='test-grid'>"+_card("STATUS","CLOSED" if closed else "OPEN • LIVE")+_card("STOCK / SIDE",f"{s.get('symbol','—')} / {s.get('side','—')}")+_card("ENTRY",f"₹{_fmt(s.get('entry'))}")+_card("LIVE / EXIT",f"₹{_fmt(price)}")+_card("LIVE P&L",f"₹{_fmt(pnl)}")+_card("ENTRY TIME",et[11:19] if et else "—")+_card("LAST UPDATE",ut[11:19] if ut else "—")+_card("EXIT TIME",xt[11:19] if xt else "—")+"</div>"
        st.markdown(html,unsafe_allow_html=True)
        st.success(f"Exited at 2:45 PM IST • ₹{_fmt(s.get('exit'))} • P&L ₹{_fmt(s.get('pnl'))}") if closed else st.info(f"LIVE: last Dhan quote update {ut[11:19] if ut else '—'} • next refresh in 15 seconds")

def render_test_tab():
    try:
        from market.nifty500_breadth import BREADTH
        from market.dhan_data import index_quote
        snap=BREADTH.snapshot(force=False);q=snap.get("quote_rows");rows=q.copy() if isinstance(q,pd.DataFrame) else pd.DataFrame();idx=index_quote("NIFTY 500")
        _live_test(rows,snap,idx)
