...
try:
 ...
 live_market=BREADTH.snapshot(force=False);dhan_ok=dhan_configured();api_status=dhan_status()
...
# CLOSED DATA: use the verified NIFTY-500 breadth snapshot directly after market close.
# This avoids a second Dhan request and guarantees the same 500-stock dataset feeds the cards.
closed_df=pd.DataFrame();closed_market={}
try:
 from market.closed_session import load_saved
 if now.time()>=dt_time(15,30):
  closed_market=dict(live_market) if live_market.get("evaluated",0)>0 else {}
  if not closed_market.get("complete"):
   closed_df, saved=load_saved(now.date())
   if saved: closed_market=saved
 else:
  closed_df,saved=load_saved(now.date())
  if saved: closed_market=saved
except Exception as exc: closed_market={"complete":False,"reason":f"Closed data: {type(exc).__name__}: {exc}"}
market=live_market
def num(x,d=None):
 try:return float(x)
 except:return d
...
pc=closed_market.get("nifty500500_close",closed_market.get("nifty500_previous_close"));ca=closed_market.get("ad_ratio");cov=closed_market.get("sector_coverage",f"{closed_market.get('evaluated',len(closed_df))}/500")
if closed_market.get("complete") and closed_market.get("evaluated",0)>=450: cov=f"{closed_market.get('evaluated',0)}/500"
if pc is None and not closed_df.empty and "Close" in closed_df.columns: pc=num(pd.to_numeric(closed_df["Close"],errors="coerce").mean())
st.markdown("<div class='grid4'>"+"".join([card("NIFTY 500 CLOSE",f"{pc:,.2f}" if pc is not None else "—"),card("A/D RATIO",f"{ca:.2f}" if ca is not None else "—"),card("ADVANCES / DECLINES",f"{closed_market.get('advances','—')} / {closed_market.get('declines','—')}"),card("SECTOR ALIGNMENT",pct(closed_market.get('sector_alignment_pct'))),card("POSITIVE SECTORS",closed_market.get('positive_sectors','—')),card("NEGATIVE SECTORS",closed_market.get('negative_sectors','—')),card("500-STOCK COVERAGE",cov),card("DATA SOURCE / TIME",f"Dhan • {closed_market.get('closed_session_label',closed_market.get('session_date','—'))} • 15:30 IST")])+"</div>",unsafe_allow_html=True)
if not closed_market.get("complete"):
 st.caption(f"Closed-session diagnostic: {closed_market.get('reason','Dhan closed-session data not available yet')}")
...
st.markdown("<div class='sec'>🔎 What Happened Yesterday?</div>",unsafe_allow_html=True);st.markdown(f"<div class='status'><b>{closed_market.get('closed_session_label',closed_market.get('session_date','Latest completed session'))}</b> • Close: {f'{pc:,.2f}' if pc is not None else '—'} • A/D: {f'{ca:.2f}' if ca is not None else '—'} • Advances/Declines: {closed_market.get('advances','—')}/{closed_market.get('declines','—')} • Sector alignment: {pct(closed_market.get('sector_alignment_pct'))}</div>",unsafe_allow_html=True)
st.markdown("<div class='sec'>🧠 Daily Analysis & Journal</div>",unsafe_allow_html=True);st.info("Closed-session data is kept separate from live data and used here when verified.")
...
if dc and not today.empty:dt=pd.to_datetime(today[dc],errors="coerce");today=today[dt.dt.date==now.date()]
