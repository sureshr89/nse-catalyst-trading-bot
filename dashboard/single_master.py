...
def metric_card(label, value, emphasis=""):
    ...
    return (
        ...
        f'padding:12px 13px;min-height:82px;box-sizing:border-box;box-shadow:0 2px 8px rgba(0,0,0,.18);font-family:Arial,sans-serif;">'
        f'<div style="font-size:12px;font-weight:800;color:#9fb0c5;text-transform:uppercase;letter-spacing:.04em;">{label}</div>'
        f'<div style="font-size:20px;font-weight:900;color:{color};margin-top:8px;line-height:1.15;">{value}</div></div>'
    )
...
def strategy_card(strategy, state, state_color, cells):
    ...
    details = "".join(
        f'<div style="background:#111f32;border:1px solid #1e314b;border-radius:8px;padding:9px 10px;min-width:0;font-family:Arial,sans-serif;">'
        f'<div style="font-size:11px;font-weight:850;color:#9fb0c5;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{label}</div>'
        f'<div style="font-size:14px;font-weight:900;color:#f8fafc;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{value or "—"}</div></div>'
        for label, value in cells
    )
    return (
        f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;border-top:3px solid {accent};padding:13px 14px;margin:9px 0;box-sizing:border-box;box-shadow:0 3px 10px rgba(0,0,0,.22);font-family:Arial,sans-serif;">'
        f'<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;">'
        f'<div style="display:flex;align-items:center;gap:9px;min-width:0;"><span style="font-size:22px;">{icon}</span><div>'
        f'<div style="font-size:19px;font-weight:950;color:{accent};line-height:1;">{strategy}</div>'
        f'<div style="font-size:11px;font-weight:800;color:#9fb0c5;letter-spacing:.04em;margin-top:5px;">{subtitle}</div></div></div>'
        f'<span style="font-size:11px;font-weight:900;color:{state_color};background:#111f32;border:1px solid #2a405d;border-radius:999px;padding:7px 10px;white-space:nowrap;">{state}</span></div>'
...
def cumulative_chart_frames(trades_all):
    ...
    strategy_data = [{"Strategy": s, "Cumulative P&L": performance_stats(work, s)["pnl"]} for s in STRATEGIES]
    return daily, pd.DataFrame(strategy_data).set_index("Strategy")

st.markdown("""<style>
.stApp{background:#05080d!important;color:#f8fafc!important;font-family:Arial,sans-serif!important}
.main,.block-container{background:#05080d!important}
.block-container{max-width:1450px;padding:.7rem .8rem 2rem}
.section-title{font-size:21px;font-weight:950;color:#f8fafc;margin:18px 0 9px;font-family:Arial,sans-serif}
.chart-note{font-size:13px;color:#9fb0c5;margin:0 0 8px;font-family:Arial,sans-serif}
.stMarkdown,.stText,.stCaption{font-family:Arial,sans-serif!important}
@media (max-width:700px){.block-container{padding:.55rem .55rem 1.5rem}.section-title{font-size:19px}}
...
def live_dashboard():
    ...
    st.markdown('<div style="font-size:30px;font-weight:950;color:#f8fafc;font-family:Arial,sans-serif;">📊 NSE Catalyst — Master Dashboard</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="font-size:13px;color:#9fb0c5;margin-bottom:12px;font-family:Arial,sans-serif;">NIFTY 500 • PAPER TRADING ONLY • Dhan • {now.strftime("%d %b %Y %H:%M:%S")} IST • auto refresh 15s</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="margin:8px 0;padding:11px 13px;background:{bg};border:1px solid {border};border-radius:11px;color:{text};font-size:13px;font-family:Arial,sans-serif;">{status}</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="font-size:13px;color:#9fb0c5;margin-bottom:9px;font-family:Arial,sans-serif;">Today overall: one combined summary across S1–S5.</div>', unsafe_allow_html=True)
    today_pnl_emphasis = "buy" if ts["pnl"] > 0 else "sell" if ts["pnl"] < 0 else ""
    st.markdown(metric_grid([metric_card("OPEN", ts["open"]), metric_card("TRADES", ts["trades"]), metric_card("WINS", ts["wins"], "buy"), metric_card("LOSSES", ts["losses"], "sell"), metric_card("WIN RATE", f'{ts["win_rate"]:.1f}%'), metric_card("P&L", f'₹{ts["pnl"]:,.2f}', today_pnl_emphasis)]), unsafe_allow_html=True)
    ...
    if not strategy_frame.empty:
        st.markdown('<div style="font-size:15px;font-weight:850;color:#f8fafc;font-family:Arial,sans-serif;margin:6px 0;">Cumulative P&L by Strategy</div>', unsafe_allow_html=True)
        st.bar_chart(strategy_frame, use_container_width=True, height=260)
    if not daily_frame.empty:
        st.markdown('<div style="font-size:15px;font-weight:850;color:#f8fafc;font-family:Arial,sans-serif;margin:6px 0;">Daily P&L — spikes and drawdowns</div>', unsafe_allow_html=True)
        st.bar_chart(daily_frame[["Daily P&L"]], use_container_width=True, height=260)
        st.markdown('<div style="font-size:15px;font-weight:850;color:#f8fafc;font-family:Arial,sans-serif;margin:6px 0;">Cumulative P&L curve</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;padding:14px;font-size:16px;font-weight:850;color:#f8fafc;font-family:Arial,sans-serif;">💡 {tips[now.date().toordinal() % len(tips)]}</div>', unsafe_allow_html=True)
