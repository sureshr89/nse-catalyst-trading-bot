...
def live_dashboard():
    ...
    try:
        ...
        market={"complete":False,"sector_complete":False,"evaluated":0,"sector_priced":0,"nifty500_change_pct":None,"sector_alignment_pct":None,"ad_ratio":None,"advances":0,"declines":0,"unchanged":0,"positive_sectors":0,"negative_sectors":0,"reason":str(exc),"quote_rows":pd.DataFrame()};dhan_ok=False;api_status={"ok":False,"received":0,"requested":0,"message":str(exc)}
    ...
    cards=[("NIFTY 500",pct(n) if complete else "WAITING"),("ADVANCES",advances if complete else "WAITING"),("DECLINES",declines if complete else "WAITING"),("UNCHANGED",unchanged if complete else "WAITING"),("A/D RATIO",fmt(ad) if complete and ad is not None else "WAITING"),("SECTORS + / −",f"{positive_sectors} / {negative_sectors}" if sector_complete else "WAITING")]
    st.markdown('<div class="grid6">'+''.join(card(l,v) for l,v in cards)+'</div>',unsafe_allow_html=True)
    cards2=[("STOCK DATA",f"{evaln}/500"),("SECTOR DATA",f"{sp}/500"),("MASTER BIAS",bias)]
