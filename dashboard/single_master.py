...
.title{font-size:clamp(1.5rem,4vw,2.4rem);font-weight:900;color:#f5f7fb}
.sub{font-size:.75rem;color:#9fb1ca;margin-bottom:10px}
.sec{font-size:1.1rem;font-weight:900;margin:15px 0 8px;color:#fff}
.grid6{display:grid;grid-template-columns:repeat(6,1fr);gap:7px}
.card{background:#101b2b;border:1px solid #294367;border-radius:11px;padding:10px;min-height:60px}
.label{font-size:.56rem;font-weight:850;color:#9fb1ca;text-transform:uppercase}.value{font-size:.95rem;font-weight:850;margin-top:4px}
.status{background:#101b2b;border:1px solid #294367;border-radius:11px;padding:9px;margin:7px 0;color:#d9e3f1;font-size:.76rem}
.strategy{background:#0b1422;border:1px solid #294367;border-radius:12px;padding:10px;margin:7px 0}
.strategy-title{font-weight:900;font-size:.88rem;margin-bottom:7px}.state{float:right;font-weight:900;font-size:.68rem;padding:4px 7px;border-radius:7px;background:#162943}
...
.tip{background:#101b2b;border:1px solid #294367;border-radius:11px;padding:13px;font-weight:700}
@media(max-width:900px){.grid6{grid-template-columns:repeat(3,1fr)}.trade-grid{grid-template-columns:repeat(4,1fr)}}
@media(max-width:600px){.grid6{grid-template-columns:repeat(2,1fr)}.trade-grid{grid-template-columns:repeat(2,1fr)}}
</style>
...
def live_dashboard():
    ...
    try:
        ...
        dhan_ok = False
        api_status = {"ok": False, "received": 0, "requested": 0, "message": str(exc)}

    trades_all = read_csv("trades.csv")
    signals_all = read_csv("signals.csv")

    # SCREEN = current day only.  Never invent a trade: it comes from the real journal or signal ledger.
    today = now.date()
    trades_today = trades_all.copy()
    if not trades_today.empty:
        date_col = next((c for c in ["exit_time", "entry_time", "market_entry_time", "trigger_entry_time"] if c in trades_today.columns), None)
        if date_col:
            d = pd.to_datetime(trades_today[date_col], errors="coerce", utc=True)
            # Compare in India time when timestamps contain a timezone; naive values are treated as local.
            try:
    ...
    if not signals_today.empty:
        date_col = next((c for c in ["timestamp", "entry_time", "logged_at"] if c in signals_today.columns), None)
        if date_col:
            d = pd.to_datetime(signals_today[date_col], errors="coerce", utc=True)
            try:
                d = d.dt.tz_convert(IST)
            except Exception:
                pass
    ...
    # One cumulative CSV: actual persistent trade journal only, preserving every column already written by the engine.
    master_csv = trades_all.copy()
    if master_csv.empty:
        master_csv = pd.DataFrame(columns=list(trades_all.columns))
    ...
    for strategy in strategies:
        ...
        sg = pd.DataFrame()
        if not trades_today.empty:
            cols = [c for c in ["strategy", "strategy_name", "signal", "setup_type"] if c in trades_today.columns]
            if cols:
                mask = pd.Series(False, index=trades_today.index)
                for c in cols:
                    mask |= trades_today[c].astype(str).str.upper().str.strip().eq(strategy)
                    mask |= trades_today[c].astype(str).str.upper().str.startswith(strategy + " ")
                tr = trades_today[mask]
        if not signals_today.empty:
            cols = [c for c in ["strategy", "strategy_name", "signal", "setup_type"] if c in signals_today.columns]
            if cols:
                mask = pd.Series(False, index=signals_today.index)
                for c in cols:
                    mask |= signals_today[c].astype(str).str.upper().str.strip().eq(strategy)
                    mask |= signals_today[c].astype(str).str.upper().str.startswith(strategy + " ")
                sg = signals_today[mask]

        row = tr.iloc[-1] if not tr.empty else None
        signal_row = sg.iloc[-1] if not sg.empty else None
        if row is not None:
            raw_status = str(first(row, "status", default="OPEN")).upper()
            state = "CLOSED" if raw_status == "CLOSED" or first(row,"exit_time") not in {"", None} else "TRADE OPEN"
        ...
        state_color = "#67e8a5" if state == "CLOSED" else "#5ec8ff" if state == "SIGNAL" else "#ffd166" if state == "TRADE OPEN" else "#9fb1ca"
        st.markdown(f'<div class="strategy"><span class="state" style="color:{state_color}">{state}</span><div class="strategy-title">{strategy}</div><div class="trade-grid">' + ''.join([
            f'<div class="trade-cell"><div class="trade-label">Stock</div><div class="trade-value">{stock or "—"}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">BUY / SELL</div><div class="trade-value">{side or "—"}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Signal Time</div><div class="trade-value">{signal_time or "—"}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Entry</div><div class="trade-value">{fmt(entry)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Stop Loss</div><div class="trade-value">{fmt(sl)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Target</div><div class="trade-value">{fmt(target)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Exit</div><div class="trade-value">{fmt(exit_price)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">P&L</div><div class="trade-value">{fmt(pnl)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Risk / Reward</div><div class="trade-value">{fmt(rr)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Quantity</div><div class="trade-value">{fmt(qty)}</div></div>',
            f'<div class="trade-cell"><div class="trade-label">Exit Reason</div><div class="trade-value">{exit_reason or "—"}</div></div>',
    ...
    st.caption(f"Cumulative journal: {len(master_csv)} trade record(s). All original journal columns are preserved.")

    st.markdown('<div class="sec">🧠 TODAY'S JOURNAL</div>', unsafe_allow_html=True)
    pnl = pd.to_numeric(trades_today.get("pnl", pd.Series(dtype=float)), errors="coerce").fillna(0)
    win_rate = f"{(pnl > 0).mean()*100:.1f}%" if len(pnl) else "—"
    cards2 = [("TRADES",len(trades_today)),("WINS",int((pnl>0).sum())),("LOSSES",int((pnl<0).sum())),("WIN RATE",win_rate),("TODAY P&L",f"₹{pnl.sum():,.0f}")]
    st.markdown('<div class="grid6">'+''.join(card(l,v) for l,v in cards2)+'</div>',unsafe_allow_html=True)
    if not trades_today.empty:
        st.dataframe(trades_today, width="stretch", hide_index=True)
    else:
        st.info("No completed/open trade journal records today.")
