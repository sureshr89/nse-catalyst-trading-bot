...
PALETTE = {"S1": ("#22c55e", "↩️", "PDH/PDL SWEEP REVERSAL"), "S2": ("#38bdf8", "🔁", "BREAKOUT + RETEST"), "S3": ("#f59e0b", "🎯", "INSIDE RANGE REVERSAL"), "S4": ("#a78bfa", "⚡", "INTRADAY BREAKOUT"), "S5": ("#fb7185", "🚀", "PDH/PDL BREAKOUT")}
...
def metric_card(label,value,emphasis=""):
    ...
    return f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;padding:12px 13px;min-height:76px;box-sizing:border-box;box-shadow:0 2px 8px rgba(0,0,0,.18);"><div style="font-size:10px;font-weight:800;color:#8ea2bc;text-transform:uppercase;letter-spacing:.04em;">{label}</div><div style="font-size:18px;font-weight:900;color:{color};margin-top:7px;line-height:1.15;">{value}</div></div>'
...
def strategy_card(strategy,state,state_color,cells):
    ...
    details="".join(f'<div style="background:#111f32;border:1px solid #1e314b;border-radius:8px;padding:8px 9px;min-width:0;"><div style="font-size:8px;font-weight:850;color:#8ea2bc;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{label}</div><div style="font-size:12px;font-weight:900;color:#f8fafc;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{value or "—"}</div></div>' for label,value in cells)
    return f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;border-top:3px solid {accent};padding:12px 13px;margin:9px 0;box-sizing:border-box;box-shadow:0 3px 10px rgba(0,0,0,.22);"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px;"><div style="display:flex;align-items:center;gap:8px;min-width:0;"><span style="font-size:20px;">{icon}</span><div><div style="font-size:17px;font-weight:950;color:{accent};line-height:1;">{strategy}</div><div style="font-size:9px;font-weight:800;color:#8ea2bc;letter-spacing:.04em;margin-top:4px;">{subtitle}</div></div></div><span style="font-size:9px;font-weight:900;color:{state_color};background:#111f32;border:1px solid #2a405d;border-radius:999px;padding:6px 9px;white-space:nowrap;">{state}</span></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(125px,1fr));gap:6px;">{details}</div></div>'
...
def performance_card(strategy,stats,cumulative=False):
    accent,icon,subtitle=PALETTE[strategy]; pnl=stats["pnl"]; pnl_color="#22c55e" if pnl>0 else "#fb7185" if pnl<0 else "#cbd5e1"; label="CUMULATIVE" if cumulative else "TODAY"
    cells=[("OPEN",stats["open"],"#f8fafc"),("TRADES",stats["trades"],"#f8fafc"),("WINS",stats["wins"],"#22c55e"),("LOSSES",stats["losses"],"#fb7185"),("WIN RATE",f'{stats["win_rate"]:.1f}%',"#f8fafc"),(f'{label} P&L',f'₹{pnl:,.2f}',pnl_color)]
    body="".join(f'<div style="background:#111f32;border:1px solid #1e314b;border-radius:8px;padding:7px;"><div style="color:#8ea2bc;font-size:8px;font-weight:800;">{k}</div><div style="font-size:13px;font-weight:900;color:{c};margin-top:3px;">{v}</div></div>' for k,v,c in cells)
    return f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;border-top:3px solid {accent};padding:12px;box-sizing:border-box;box-shadow:0 3px 10px rgba(0,0,0,.22);"><div style="display:flex;align-items:center;justify-content:space-between;gap:7px;margin-bottom:9px;"><div style="font-size:18px;font-weight:950;color:{accent};">{icon} {strategy}</div><div style="font-size:8px;font-weight:850;color:#8ea2bc;text-align:right;">{subtitle}</div></div><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;">{body}</div></div>'

def performance_summary(stats_map):
    total={"trades":sum(v["trades"] for v in stats_map.values()),"open":sum(v["open"] for v in stats_map.values()),"wins":sum(v["wins"] for v in stats_map.values()),"losses":sum(v["losses"] for v in stats_map.values()),"pnl":sum(v["pnl"] for v in stats_map.values())}
    decided=total["wins"]+total["losses"]; total["win_rate"]=total["wins"]/decided*100 if decided else 0.0
    return total

st.markdown("""<style>.stApp{background:#05080d!important;color:#f8fafc!important}.main,.block-container{background:#05080d!important}.block-container{max-width:1450px;padding:.7rem .8rem 2rem}.section-title{font-size:20px;font-weight:950;color:#f8fafc;margin:18px 0 9px}.performance-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.performance-summary{margin-top:10px;padding:14px 16px;background:#0d1726;border:1px solid #243752;border-radius:12px;display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px;box-shadow:0 3px 10px rgba(0,0,0,.22)}.summary-item{min-width:0;text-align:center;padding:2px 4px}.summary-label{font-size:9px;font-weight:850;color:#8ea2bc;letter-spacing:.04em}.summary-value{font-size:18px;font-weight:950;color:#f8fafc;margin-top:4px}.summary-wins{color:#22c55e!important}.summary-losses{color:#fb7185!important}.summary-pnl{color:#22c55e!important}@media (max-width:1100px){.performance-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.performance-summary{grid-template-columns:repeat(3,minmax(0,1fr))}}@media (max-width:700px){.block-container{padding:.55rem .55rem 1.5rem}.performance-grid{grid-template-columns:1fr}.performance-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}</style>""",unsafe_allow_html=True)
...
def live_dashboard():
    ...
    st.markdown('<div style="font-size:30px;font-weight:950;color:#f8fafc;">📊 NSE Catalyst — Master Dashboard</div>',unsafe_allow_html=True); st.markdown(f'<div style="font-size:12px;color:#8ea2bc;margin-bottom:12px;">NIFTY 500 • PAPER TRADING ONLY • Dhan • {now.strftime("%d %b %Y %H:%M:%S")} IST • auto refresh 15s</div>',unsafe_allow_html=True); st.markdown('<div class="section-title">🎯 MARKET ALIGNMENT</div>',unsafe_allow_html=True)
    index_display=pct(n)
    ...
    bg="#052e1b" if status_ok else "#3a1115"; border="#166534" if status_ok else "#991b1b"; text="#86efac" if status_ok else "#fda4af"; st.markdown(f'<div style="margin:8px 0;padding:11px 13px;background:{bg};border:1px solid {border};border-radius:11px;color:{text};font-size:13px;">{status}</div>',unsafe_allow_html=True)
    ...
    for strategy in STRATEGIES:
        ...
        state_color="#22c55e" if state=="CLOSED" else "#38bdf8" if state=="SIGNAL" else "#f59e0b" if state=="TRADE OPEN" else "#94a3b8"; st.markdown(strategy_card(strategy,state,state_color,cells),unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">📅 TODAY — ALL POSITIONS</div>',unsafe_allow_html=True); st.markdown('<div style="font-size:11px;color:#8ea2bc;margin-bottom:9px;">Today overall: one combined summary across S1–S5.</div>',unsafe_allow_html=True)
    ts=performance_summary(today_stats); st.markdown(f'<div class="performance-summary"><div class="summary-item"><div class="summary-label">OPEN</div><div class="summary-value">{ts["open"]}</div></div><div class="summary-item"><div class="summary-label">TRADES</div><div class="summary-value">{ts["trades"]}</div></div><div class="summary-item"><div class="summary-label">WINS</div><div class="summary-value summary-wins">{ts["wins"]}</div></div><div class="summary-item"><div class="summary-label">LOSSES</div><div class="summary-value summary-losses">{ts["losses"]}</div></div><div class="summary-item"><div class="summary-label">WIN RATE</div><div class="summary-value">{ts["win_rate"]:.1f}%</div></div><div class="summary-item"><div class="summary-label">P&amp;L</div><div class="summary-value summary-pnl">₹{ts["pnl"]:,.2f}</div></div></div>',unsafe_allow_html=True)
    st.markdown('<div class="section-title">📈 CUMULATIVE — ALL DAYS</div>',unsafe_allow_html=True); st.markdown('<div style="font-size:11px;color:#8ea2bc;margin-bottom:9px;">Complete historical S1–S5 performance across all available trading days.</div>',unsafe_allow_html=True); st.markdown('<div class="performance-grid">'+''.join(performance_card(s,cumulative_stats[s],cumulative=True) for s in STRATEGIES)+'</div>',unsafe_allow_html=True)
    st.markdown('<div class="section-title">📥 DOWNLOAD</div>',unsafe_allow_html=True); st.download_button("⬇️ Download Master CSV",trades_all.to_csv(index=False).encode("utf-8"),"nse_catalyst_master.csv","text/csv",use_container_width=True,key="master_csv"); st.markdown('<div class="section-title">💡 DAILY TRADING TIP</div>',unsafe_allow_html=True); tips=["Follow the setup, not the emotion.","Protect capital first; profits come second.","Wait for confirmation before entering.","One disciplined trade is better than many emotional trades.","Never chase a missed entry."]; st.markdown(f'<div style="background:#0d1726;border:1px solid #243752;border-radius:12px;padding:14px;font-size:16px;font-weight:850;color:#f8fafc;">💡 {tips[now.date().toordinal()%len(tips)]}</div>',unsafe_allow_html=True)
