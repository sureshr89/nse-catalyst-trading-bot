...
def total_performance(stats_map): return {"trades":sum(v["trades"] for v in stats_map.values()),"open":sum(v["open"] for v in stats_map.values()),"wins":sum(v["wins"] for v in stats_map.values()),"losses":sum(v["losses"] for v in stats_map.values()),"pnl":sum(v["pnl"] for v in stats_map.values())}

st.markdown("""<style>.stApp{background:#ffffff!important;color:#172033!important}.block-container{max-width:1450px;padding:.7rem .8rem 2rem}.section-title{font-size:20px;font-weight:950;color:#172033;margin:18px 0 9px}.performance-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.performance-total{margin-top:9px;padding:10px 12px;border-top:2px solid #d7e4f2;border-bottom:1px solid #d7e4f2;color:#475569;font-size:13px;font-weight:900;display:flex;gap:20px;flex-wrap:wrap}.performance-total b{color:#172033}@media (max-width:1100px){.performance-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media (max-width:700px){.block-container{padding:.55rem .55rem 1.5rem}.performance-grid{grid-template-columns:1fr}}</style>""",unsafe_allow_html=True)
...
def live_dashboard():
    ...
    st.markdown('<div style="font-size:30px;font-weight:950;color:#172033;">📊 NSE Catalyst — Master Dashboard</div>',unsafe_allow_html=True); st.markdown(f'<div style="font-size:12px;color:#64748b;margin-bottom:12px;">NIFTY 500 • PAPER TRADING ONLY • Dhan • {now.strftime("%d %b %Y %H:%M:%S")} IST • auto refresh 15s</div>',unsafe_allow_html=True); st.markdown('<div class="section-title">🎯 MARKET ALIGNMENT</div>',unsafe_allow_html=True)
    index_display=pct(n)
    ...
    bg="#ecfdf5" if status_ok else "#fef2f2"; border="#86efac" if status_ok else "#fecaca"; text="#166534" if status_ok else "#991b1b"; st.markdown(f'<div style="margin:8px 0;padding:11px 13px;background:{bg};border:1px solid {border};border-radius:11px;color:{text};font-size:13px;">{status}</div>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">📅 TODAY — ALL POSITIONS</div>',unsafe_allow_html=True); st.markdown('<div style="font-size:11px;color:#64748b;margin-bottom:9px;">S1–S5: open positions, trades, wins, losses, win rate and today P&amp;L.</div>',unsafe_allow_html=True); st.markdown('<div class="performance-grid">'+''.join(performance_card(s,today_stats[s]) for s in STRATEGIES)+'</div>',unsafe_allow_html=True)
    tt=total_performance(today_stats); twr=tt["wins"]/(tt["wins"]+tt["losses"])*100 if tt["wins"]+tt["losses"] else 0; tpnl_color="#15803d" if tt["pnl"]>0 else "#dc2626" if tt["pnl"]<0 else "#475569"; st.markdown(f'<div class="performance-total"><span>OPEN <b>{tt["open"]}</b></span><span>TRADES <b>{tt["trades"]}</b></span><span>WINS <b>{tt["wins"]}</b></span><span>LOSSES <b>{tt["losses"]}</b></span><span>WIN RATE <b>{twr:.1f}%</b></span><span>P&amp;L <b style="color:{tpnl_color};">₹{tt["pnl"]:,.2f}</b></span></div>',unsafe_allow_html=True)
    st.markdown('<div class="section-title">📈 CUMULATIVE — ALL DAYS</div>',unsafe_allow_html=True); st.markdown('<div style="font-size:11px;color:#64748b;margin-bottom:9px;">Complete historical S1–S5 performance across all available trading days.</div>',unsafe_allow_html=True); st.markdown('<div class="performance-grid">'+''.join(performance_card(s,cumulative_stats[s],cumulative=True) for s in STRATEGIES)+'</div>',unsafe_allow_html=True)
    ct=total_performance(cumulative_stats); cwr=ct["wins"]/(ct["wins"]+ct["losses"])*100 if ct["wins"]+ct["losses"] else 0; cpnl_color="#15803d" if ct["pnl"]>0 else "#dc2626" if ct["pnl"]<0 else "#475569"; st.markdown(f'<div class="performance-total"><span>OPEN <b>{ct["open"]}</b></span><span>TRADES <b>{ct["trades"]}</b></span><span>WINS <b>{ct["wins"]}</b></span><span>LOSSES <b>{ct["losses"]}</b></span><span>WIN RATE <b>{cwr:.1f}%</b></span><span>P&amp;L <b style="color:{cpnl_color};">₹{ct["pnl"]:,.2f}</b></span></div>',unsafe_allow_html=True)
    st.markdown('<div class="section-title">📥 DOWNLOAD</div>',unsafe_allow_html=True); st.download_button("⬇️ Download Master CSV",trades_all.to_csv(index=False).encode("utf-8"),"nse_catalyst_master.csv","text/csv",use_container_width=True,key="master_csv"); st.markdown('<div class="section-title">💡 DAILY TRADING TIP</div>',unsafe_allow_html=True); tips=["Follow the setup, not the emotion.","Protect capital first; profits come second.","Wait for confirmation before entering.","One disciplined trade is better than many emotional trades.","Never chase a missed entry."]; st.markdown(f'<div style="background:#f8fbff;border:1px solid #d7e4f2;border-radius:12px;padding:14px;font-size:16px;font-weight:850;color:#172033;">💡 {tips[now.date().toordinal()%len(tips)]}</div>',unsafe_allow_html=True)
