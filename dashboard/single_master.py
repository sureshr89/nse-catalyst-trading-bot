...
try:
 ...
 if now.time().replace(tzinfo=None)>=datetime.strptime("15:30","%H:%M").time(): closed_df,closed_market=build_closed_snapshot(force=False)
 else: closed_df,closed_market=latest_saved_before(now.date())
except Exception as exc:
 closed_df=pd.DataFrame();closed_market={"complete":False,"reason":f"Closed table: {type(exc).__name__}: {exc}"}
# IMPORTANT: keep the original live master cards. Closed session is displayed only in its own section.
market=live_market
...
# Separate closed-session section — never replaces the live cards above.
st.markdown("<div class='sec'>📚 Previous / Latest Closed Session</div>",unsafe_allow_html=True)
pc=closed_market.get("nifty500_close");ca=closed_market.get("ad_ratio");cov=closed_market.get("coverage",f"{len(closed_df)}/500")
...
st.markdown("<div class='sec'>🔎 What Happened Yesterday?</div>",unsafe_allow_html=True);st.markdown(f"<div class='status'><b>{closed_market.get('session_date','Latest completed session')}</b> • Close: {f'{pc:,.2f}' if pc is not None else '—'} • A/D: {f'{ca:.2f}' if ca is not None else '—'} • Advances/Declines: {closed_market.get('advances','—')}/{closed_market.get('declines','—')} • Sector alignment: {pct(closed_market.get('sector_alignment_pct'))}</div>",unsafe_allow_html=True)
