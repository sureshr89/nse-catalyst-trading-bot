...
st.markdown(
    ...
    .card{min-height:61px}
    .label{font-size:.56rem;font-weight:850;color:#9fb1ca;text-transform:uppercase}
    ...
    .strategy b{font-size:.9rem}
    .strategy span{color:#9fb1ca;font-size:.76rem}
...
def live_dashboard():
    # IMPORTANT: everything below is recalculated inside the 15-second fragment.
    # This makes Dhan quotes, breadth, sectors, A/D, journal values and clock refresh together.
    ...
    if not trades.empty:
        t = trades.copy()
        t.insert(0, "RecordType", "TRADE")
        master_parts.append(t)
    if not signals.empty:
        s = signals.copy()
        s.insert(0, "RecordType", "SIGNAL")
        master_parts.append(s)
    master_csv = (
        pd.concat(master_parts, ignore_index=True, sort=False)
        if master_parts
        else pd.DataFrame({"RecordType": [], "Status": []})
    )
    ...
    buy = bool(
        market.get("complete")
        and market.get("sector_complete")
        and num(n, 0) > 0
        and num(sec, 0) > 0
        and num(ad, 0) > 1
    )
    ...
    st.markdown(
        f'<div class="sub">NIFTY 500 • PAPER TRADING ONLY • Dhan data • App time {now.strftime("%d %b %Y %H:%M:%S")} IST</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        + "".join(
            [
                card("NIFTY 500", pct(n)),
                card("SECTORS", pct(sec)),
                card("A/D RATIO", f"{ad:.2f}" if ad is not None else "WAITING"),
                card("BREADTH", f"{evaln}/500"),
                card("SECTOR DATA", f"{sp}/500"),
                card("MASTER BIAS", bias),
            ]
        )
        + "</div>",
        unsafe_allow_html=True,
    ...
    st.markdown(
        f'<div class="status"><b>Dhan: {"CONNECTED" if dhan_ok else "WAITING"}</b> • '
        f'API: {"PASS" if api_status.get("ok") else "WAIT/ERROR"} • '
        f'Quotes {api_status.get("received",0)}/{api_status.get("requested",0)} • '
        f'Auto refresh: 15s</div>',
        unsafe_allow_html=True,
    )

    # Restore the five strategy cards before the download section.
    st.markdown('<div class="sec">⚡ S1–S5 STRATEGIES</div>', unsafe_allow_html=True)
    strategies = [
    ...
    for code, name, desc in strategies:
        st.markdown(
            f'<div class="strategy"><b>{code} • {name}</b><br><span>{desc}</span></div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        + "".join(
            [
                card("TRADES", len(today)),
                card("WINS", int((pnl > 0).sum())),
                card("LOSSES", int((pnl < 0).sum())),
                card("WIN RATE", win_rate),
                card("TODAY P&L", f"₹{pnl.sum():,.0f}"),
    ...
    st.markdown(
        f'<div class="tip">💡 {tips[now.date().toordinal() % len(tips)]}</div>',
        unsafe_allow_html=True,
    )
