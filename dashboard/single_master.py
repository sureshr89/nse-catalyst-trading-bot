...
st.markdown(f'<div class="sub">NIFTY 500 • PAPER TRADING ONLY • Dhan data • no automatic screen refresh • App time {now.strftime("%d %b %Y %H:%M:%S")} IST</div>',unsafe_allow_html=True)

# Running-status strip: visible time, Dhan state and last verified price update.
last_time=market.get("last_quote_time") or (api_status.get("updated_at") or "—")
status_word="RUNNING • Dhan PASS" if dhan_ok and api_status.get("ok") else "RUNNING • WAITING FOR DATA"
st.markdown(f'<div class="status"><b>🟢 {status_word}</b> • App time: {now.strftime("%H:%M:%S")} IST • Last Dhan data: {last_time} • Market session: {market.get("closed_session_label","—")} • NSE close: 15:30 IST</div>',unsafe_allow_html=True)
...
# Completed session — no duplicate old table.
st.markdown('<div class="sec">🔎 What Happened Yesterday?</div>',unsafe_allow_html=True)
close=num(market.get("nifty500_previous_close")); close_text=f"{close:,.2f}" if close is not None else "—"
st.markdown(f'<div class="grid4">'+''.join([card("NIFTY 500 CLOSE",close_text),card("A/D RATIO",f"{ad:.2f}" if ad is not None else "—"),card("ADVANCES / DECLINES",f"{market.get("advances","—")} / {market.get("declines","—")}"),card("SECTOR ALIGNMENT",pct(sec))])+'</div>',unsafe_allow_html=True)
st.markdown(f'<div class="grid4">'+''.join([card("POSITIVE SECTORS",market.get("positive_sectors","—")),card("NEGATIVE SECTORS",market.get("negative_sectors","—")),card("500-STOCK COVERAGE",f"{evaln}/500"),card("DATA SOURCE / TIME",f"Dhan • {market.get("closed_session_label","—")} • 15:30 IST")])+'</div>',unsafe_allow_html=True)
if not market.get("sector_complete") and market.get("sector_error"):
    st.warning(f"Sector calculation waiting: {market.get('sector_error')}")

# Verified Dhan quote table and full CSV download.
...
if quotes.empty:
    ...
    show=[c for c in ["Symbol","SecurityId","LTP","TodayOpen","TodayHigh","TodayLow","TodayClose","PreviousClose","NetChange","Volume","change_pct"] if c in quotes.columns]
    st.caption(f"Verified price rows: {len(quotes)}/500 • Last data: {last_time}")
    st.dataframe(quotes[show].head(20),width="stretch",hide_index=True)
    st.download_button("⬇️ Download all 500 Dhan Quotes CSV",quotes.to_csv(index=False).encode(),f"nifty500_dhan_quotes_{now.date()}.csv","text/csv")

# Master journal download.
st.markdown('<div class="sec">📘 Master Journal — Download</div>',unsafe_allow_html=True)
if master_journal.empty:
    st.info("Master Journal will appear when the verified 500-stock Dhan dataset is available.")
else:
    st.caption(f"Master Journal ready: {len(master_journal)}/500 verified Dhan rows • session {market.get('closed_session_label','Current session')}")
...
# Keep the journal/P&L area; remove the duplicate S1–S5 status-card table.
...
st.info("Verified Dhan session data is the source for the master journal. No artificial values are generated.")
