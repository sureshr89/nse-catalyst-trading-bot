...
def render_enhancements():
    ...
    st.markdown("""<style>
    .stApp{background:#000!important}.block-container{max-width:760px!important;padding:.45rem .55rem 1rem!important}.hero{background:linear-gradient(135deg,#020406,#07151F,#062B32);color:#fff;border-radius:18px;padding:15px;margin-bottom:10px;border:1px solid #17313A}.hero h1{color:#fff!important;font-size:1.32rem!important;margin:0!important}.hero small{color:#d9f4f6}.box{background:#0B0F14;border:1px solid #26313D;border-radius:15px;padding:11px;margin:7px 0}.title{font-size:.68rem;font-weight:850;color:#AAB6C3;text-transform:uppercase}.big{font-size:1.1rem;font-weight:900}.green{color:#20E38A}.red{color:#FF5C67}.amber{color:#FFD166}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:8px}.cell{background:#111820;border:1px solid #202A34;border-radius:10px;padding:7px}.lab{font-size:.57rem;color:#AAB6C3}.val{font-size:.8rem;font-weight:850;color:#F5F7FB}.strat{border-left:4px solid #00D9FF}.shead{display:flex;justify-content:space-between;gap:5px;font-size:.87rem;font-weight:900;color:#F5F7FB}.pill{font-size:.57rem;background:#092C32;color:#5DE7F5;padding:4px 7px;border-radius:20px;white-space:nowrap}.summary{display:grid;grid-template-columns:repeat(2,1fr);gap:7px}.tip{background:#0B2023;border:1px solid #164B50;border-radius:14px;padding:11px;font-size:.76rem;color:#D8F7FA}.quote-head{color:#F5F7FB!important;font-weight:900;font-size:1.05rem;margin:14px 0 7px}.quote-note{color:#AAB6C3;font-size:.72rem;margin-bottom:7px}[data-testid="stDataFrame"]{border:1px solid #26313D;border-radius:12px;overflow:hidden}.stDownloadButton button{background:#08242A!important;color:#5DE7F5!important;border:1px solid #00D9FF!important;font-weight:800}@media(max-width:480px){.block-container{padding:.3rem .4rem!important}.grid{grid-template-columns:repeat(2,1fr)}.hero h1{font-size:1.14rem!important}}
    </style>""",unsafe_allow_html=True)
    st.markdown("<div class='hero'><h1>📊 NSE CATALYST</h1><small>Paper Trading • ₹2.5L / Strategy • 1 Trade / Strategy / Day</small></div>",unsafe_allow_html=True)

    # Only this fragment reruns. The rest of the dashboard remains static.
    ...
    def live_market():
        ...
        st.markdown(f"<div class='box'><div class='title'>LIVE MARKET • DHAN</div><div class='big'>🕒 {now.strftime('%d %b %Y • %H:%M:%S')} IST</div></div>",unsafe_allow_html=True)
        ...
        qt=_quote_table(q);st.markdown(f"<div class='quote-head'>📈 LIVE STOCK QUOTES ({len(qt)})</div><div class='quote-note'>Latest Dhan snapshot • updates every 25 seconds</div>",unsafe_allow_html=True)
        if not qt.empty:
        ...
        else:st.warning("No live stock quotes received from Dhan yet. The next 25-second refresh will retry.")
    live_market()
    ...
    else:st.caption("Cumulative file will be available after the daily archive is created. Use Live Quotes CSV above to download current stock data now.")
