"""NSE Catalyst - static dashboard with a small live-data fragment."""
...
def render_enhancements():
 ...
 st.markdown("<div class='hero'><h1>📊 NSE CATALYST</h1><small>Paper Trading • ₹2.5L / Strategy • 1 Trade / Strategy / Day</small><small>Only clock + market prices refresh every 15 seconds</small></div>",unsafe_allow_html=True)
 ...
 else:st.caption("Cumulative file will be available after the daily archive is created.")
 st.markdown("### 💡 DAILY TRADING QUOTE")
 quotes=["The best trade is the trade that follows your rules.","Protect capital first; profits come second.","Patience is a trading edge.","Do not chase the market. Wait for your setup.","One disciplined trade is better than five emotional trades."]
 st.markdown(f"<div class='tip'>💡 {quotes[now.date().toordinal()%len(quotes)]}</div>",unsafe_allow_html=True)
