...
def _sector_pct(q, u):
    if q.empty or u.empty:
        return None
    sq = next((c for c in ["Symbol", "SEM_TRADING_SYMBOL", "TradingSymbol"] if c in q.columns), None)
    su = next((c for c in ["Symbol", "SEM_TRADING_SYMBOL", "TradingSymbol"] if c in u.columns), None)
    sc = next((c for c in ["Sector", "sector", "Industry"] if c in u.columns), None)
    if not sq or not su or not sc:
        return None
    m = u[[su, sc]].copy()
    m.columns = ["Symbol", "Sector"]
    x = q.copy()
    x["Symbol"] = x[sq].astype(str).str.upper().str.replace(".NS", "", regex=False)
    ...
    if "change_pct" not in x.columns:
        return None
    x = x.merge(m, on="Symbol", how="inner")
    return float(pd.to_numeric(x["change_pct"], errors="coerce").mean()) if not x.empty else None


def _archive(q, now):
    if now.hour < 16 or len(q) < 500:
        return
    ...
    try:
        ...
        if not old.empty and "Date" in old:
            old = old[old.Date.astype(str) != day]
...
def _quote_table(q):
    ...
    if "CHANGE %" not in out.columns and {"LTP", "PREV CLOSE"}.issubset(out.columns):
        p = pd.to_numeric(out["PREV CLOSE"], errors="coerce")
        out["CHANGE %"] = (pd.to_numeric(out["LTP"], errors="coerce") - p) / p * 100
...
def render_enhancements():
    ...
    .block-container{max-width:760px!important;padding:.45rem .55rem 1rem!important}
    .hero{background:linear-gradient(135deg,#020406,#07151F,#062B32);color:#fff;border-radius:18px;padding:15px;margin-bottom:8px;border:1px solid #17313A}
    .hero h1{color:#fff!important;font-size:1.32rem!important;margin:0!important}.hero small{display:block;color:#d9f4f6;margin-top:3px}
    .box{background:#0B0F14;border:1px solid #26313D;border-radius:15px;padding:11px;margin:7px 0}.title{font-size:.68rem;font-weight:850;color:#AAB6C3;text-transform:uppercase}.big{font-size:1.1rem;font-weight:900}
    .green{color:#20E38A}.red{color:#FF5C67}.amber{color:#FFD166}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:8px}
    .cell{background:#111820;border:1px solid #202A34;border-radius:10px;padding:7px}.lab{font-size:.57rem;color:#AAB6C3}.val{font-size:.8rem;font-weight:850;color:#F5F7FB}
    .strat{border-left:4px solid #00D9FF}.shead{display:flex;justify-content:space-between;gap:5px;font-size:.87rem;font-weight:900;color:#F5F7FB}.pill{font-size:.57rem;background:#092C32;color:#5DE7F5;padding:4px 7px;border-radius:20px;white-space:nowrap}
    .summary{display:grid;grid-template-columns:repeat(2,1fr);gap:7px}.tip{background:#0B2023;border:1px solid #164B50;border-radius:14px;padding:12px;font-size:.8rem;color:#D8F7FA;margin:7px 0}.quote{background:#0B1118;border:1px solid #26313D;border-radius:14px;padding:12px;margin:7px 0;color:#E8EEF5}.live-line{font-size:.86rem;font-weight:800;color:#5DE7F5;background:#07151F;border:1px solid #17313A;border-radius:12px;padding:9px 11px;margin-bottom:7px}
    .stDownloadButton button{background:#08242A!important;color:#5DE7F5!important;border:1px solid #00D9FF!important;font-weight:800}
    ...
    st.markdown("<div class='hero'><h1>📊 NSE CATALYST</h1><small>Paper Trading • ₹2.5L / Strategy • 1 Trade / Strategy / Day</small><small>🔄 Only live clock + market data update every 15 seconds • Screen does not refresh</small></div>", unsafe_allow_html=True)

    # ONLY THIS BLOCK reruns. The browser page, cards and scroll position are not refreshed.
    ...
    def live_block():
        ...
        try:
            ...
            live = BREADTH.snapshot(force=True)
            u = StockUniverse().get_dataframe(refresh=False)
        except Exception as e:
            live = {"quote_rows": pd.DataFrame(), "reason": str(e)}
            u = pd.DataFrame()

        q = live.get("quote_rows", pd.DataFrame())
        q = q if isinstance(q, pd.DataFrame) else pd.DataFrame(q)
        chg, ad, cov = live.get("nifty500_change_pct"), live.get("ad_ratio"), len(q)
        sp = live.get("sector_alignment_pct")
        if sp is None:
            sp = _sector_pct(q, u)
        ...
        state = "🟢 BUY ALIGNED" if buy else "🔴 SELL ALIGNED" if sell else "⚪ WAIT — NO ENTRY"
        cl = "green" if buy else "red" if sell else "amber"
        ...
        # Strategy cards are live-data cards; the fragment updates their live status without reloading the screen.
        ...
        # Download button is always visible when Dhan has returned any current data.
        qt = _quote_table(q)
        ...
        if not qt.empty:
            ...
            st.info("Waiting for Dhan stock data. The download button will appear automatically when quotes arrive.")
        ...
        if not master.empty:
            ...
            st.caption("Master cumulative CSV is created after the daily archive is available.")

        # End-of-page tips, tricks, rules and quotations.
        st.markdown("### 💡 DAILY TRADING TIPS • TRICKS • QUOTES")
        tips = [
            "💡 Tip: Follow the setup, not the emotion.",
            "🎯 Rule: Do not chase a missed entry. Wait for the next valid setup.",
            "🛡️ Risk: Protect capital first; profits come second.",
            "🧠 Psychology: One disciplined trade is better than five emotional trades.",
            "📌 Reminder: If confirmation is missing, WAIT is a valid decision.",
            "📖 Quote: Patience is a trading edge. Let price come to your level."
        ]
        idx = now.second // 15
        st.markdown(f"<div class='tip'>{tips[idx % len(tips)]}</div>", unsafe_allow_html=True)
        for text in tips:
