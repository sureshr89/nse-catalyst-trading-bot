...
def _dhan_market_quote(mapping, cache_seconds=QUOTE_CACHE_SECONDS):
    """Fetch one complete Dhan quote snapshot and reject partial/stale-invalid rows."""
    ...
    clean = clean.dropna(subset=["SecurityId"]).drop_duplicates("SecurityId")
    if len(clean) != len(mapping) or clean["Symbol"].duplicated().any():
        return pd.DataFrame()
    ids = clean["SecurityId"].astype(int).astype(str).tolist()
    expected_ids, expected_symbols = set(ids), set(clean["Symbol"])
    ...
    for sid, item in data.items():
        ...
        try:
            ...
            net = float(item.get("net_change") or 0)
            ...
            values = [ltp, op, hi, lo, close, vol, previous_close]
            if not all(pd.notna(v) for v in values) or ltp <= 0 or previous_close <= 0:
                continue
            if hi < max(op, lo, ltp) or lo > min(op, hi, ltp):
                continue
            rows.append({"Symbol": by_id[str(sid)], "SecurityId": str(sid), "LTP": ltp,
                         "TodayOpen": op, "TodayHigh": hi, "TodayLow": lo,
                         "TodayClose": close, "PreviousClose": previous_close,
    ...
    if result.empty:
        return result
    verified = (len(result) == len(expected_ids)
                and set(result["SecurityId"].astype(str)) == expected_ids
                and set(result["Symbol"].astype(str).str.upper()) == expected_symbols
...
def _blocked(reason):
    return {"intraday": {}, "prices": pd.DataFrame(), "sector": {}, "nifty_change": None,
            "ad_ratio": None, "ad_complete": False, "buy_alignment": False,
            "sell_alignment": False, "dhan_quotes": {}, "block_reason": reason}
...
def install(MasterEngine):
    ...
    def aligned_snapshot(self):
        ...
        if not verified.get("complete") or not verified.get("sector_complete"):
            ...
            self.diagnostics.update({"market_data_source": "DHAN_VERIFICATION_FAILED",
                                     "market_data_coverage": f"{verified.get('evaluated', 0)}/500",
                                     "ad_coverage": f"{verified.get('evaluated', 0)}/500",
                                     "sector_available": False, "market_snapshot": "BLOCKED",
        ...
        if not isinstance(quotes, pd.DataFrame) or len(quotes) != 500:
            ...
            self.diagnostics.update({"market_data_source": "DHAN_VERIFICATION_FAILED",
                                     "market_data_coverage": reason, "market_snapshot": "BLOCKED",
                                     "market_gate": "NO_ALIGNMENT"})
            self.diagnostics["rejections"]["market_data"] = reason
            snap = _blocked(reason); self.last_snapshot = snap; return snap
        quotes = quotes.copy()
        required_price_cols = {"Symbol", "LTP", "PreviousClose", "NetChange", "change_pct"}
        if not required_price_cols.issubset(quotes.columns):
            reason = "DHAN_QUOTE_SCHEMA_INVALID"
            self.diagnostics["rejections"]["market_data"] = reason
            snap = _blocked(reason); self.last_snapshot = snap; return snap
        ...
        if prices["LTP"].isna().any() or prices["PreviousClose"].isna().any() or (prices[["LTP", "PreviousClose"]] <= 0).any().any():
            reason = "DHAN_PRICE_VALIDATION_FAILED"
            self.diagnostics["rejections"]["market_data"] = reason
            snap = _blocked(reason); self.last_snapshot = snap; return snap
        sector = {"available": True, "mapped": int(verified.get("sector_mapped", 0) or 0),
        ...
        snap = {"intraday": intraday, "prices": prices, "sector": sector,
                ...
                "ad_complete": True, "buy_alignment": buy, "sell_alignment": sell,
                "dhan_quotes": dhan_quotes, "verified": verified}
        self.last_snapshot = snap
        self.diagnostics.update({"market_data_source": "DHAN_VERIFIED_500", "market_data_coverage": "500/500",
                                 "nifty500_change_pct": nifty_change, "sector_change_pct": sector_change,
                                 "sector_available": True, "sector_mapping": f"{sector['mapped']}/500",
                                 "sector_priced": f"{sector['priced']}/500", "sector_count": sector["sectors"],
                                 "positive_sectors": sector["positive_sectors"], "negative_sectors": sector["negative_sectors"],
                                 "unchanged_sectors": sector["unchanged_sectors"], "ad_ratio": ad_ratio,
                                 "ad_advances": int(verified.get("advances", 0) or 0),
                                 "ad_declines": int(verified.get("declines", 0) or 0), "ad_coverage": "500/500",
                                 "buy_alignment": buy, "sell_alignment": sell, "market_snapshot": "PASS",
