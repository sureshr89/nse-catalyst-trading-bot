...
def run_cycle(engine):
    """Run one safe market-data/strategy/paper-execution cycle.

    No broker order endpoint is called. Every accepted signal goes only to the
    configured PaperTradeEngine, which independently revalidates risk and timing.
    """
    now = engine.now()
    ...
    # First manage already-open paper positions from the same verified quote snapshot.
    ...
    # Force paper square-off at the configured time; never carry a paper position overnight.
    ...
    for _, ref in engine.references.iterrows():
        ...
        if not symbol:
            continue
        if engine.paper_engine.has_open_position(symbol):
            continue
        quote = snap.get("dhan_quotes", {}).get(symbol, {})
        if not quote or not _candidate(quote, ref, side):
            continue

        # Only shortlisted stocks request the 1-minute history needed for the
        # previous-candle/S2/S4 contracts, avoiding 475 historical calls every cycle.
        try:
        ...
        stock_signals = engine._evaluate_stock(symbol, ref, local_snap)
