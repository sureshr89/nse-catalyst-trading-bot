...
def _try_stock(engine, symbol, ref, snap, side, now):
    """Evaluate cheap snapshot-only strategies first.

    S1/S3/S5 do not require a completed intraday candle. Only if those produce
    no signal do we spend a historical-data request on S2/S4.
    """
...
def run_cycle(engine):
    ...
    for _, ref in engine.references.iterrows():
        ...
        if not symbol or engine.paper_engine.has_open_position(symbol):
            continue
        quote = snap.get("dhan_quotes", {}).get(symbol, {})
