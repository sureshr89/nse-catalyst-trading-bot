...
class MasterEngine:
    ...
    def _empty_diagnostics(self):
        return {
            "timestamp": None, "strategy": "S1-S5", "strategy_version": "2026.08.20.v4",
            ...
            "sector_mapping": "0/500", "sector_priced": "0/500", "ad_ratio": None,
            "ad_advances": 0, "ad_declines": 0, "ad_coverage": "0/500",
    ...
    def _refresh_reference_data(self, force=False):
        ...
        if not force and self._session_date == today and not self.references.empty:
            return
        try:
            universe = self.universe_engine.get_dataframe(refresh=force)
        except Exception:
        ...
        if len(universe) < 500:
            self.diagnostics["rejections"]["universe"] = f"NIFTY500_UNIVERSE_INCOMPLETE_{len(universe)}/500"
        try:
            refs = ReferenceStore(universe).prepare()
        except Exception:
            refs = pd.DataFrame()
        self.references = refs if refs is not None else pd.DataFrame()
        if not self.references.empty and "Symbol" in self.references.columns:
            self.references["Symbol"] = self.references["Symbol"].astype(str).str.upper().str.strip()
        if len(self.references) < 500:
            self.diagnostics["rejections"]["reference"] = f"REFERENCE_DATA_INCOMPLETE_{len(self.references)}/500"
        try:
            self.sector_map = load_sector_map(universe, refresh=force)
        except Exception:
            self.sector_map = pd.DataFrame()
        if len(self.sector_map) < 500:
            self.diagnostics["rejections"]["sector_mapping"] = f"SECTOR_MAPPING_INCOMPLETE_{len(self.sector_map)}/500"
        self._session_date = today

    def prepare_reference_data(self, force=False):
        self._refresh_reference_data(force=force)
    ...
    def prepare_opening_candidates(self, force=False):
        ...
        for _, ref in self.references.iterrows():
            ...
            rows.append({
                "Symbol": symbol,
                "TodayOpen": float(d.iloc[0]["Open"]),
                "PDH": float(ref["PDH"]),
                "PDL": float(ref["PDL"]),
                "PreviousDayClose": float(ref["PreviousDayClose"]),
            })
        ...
        try:
            ...
            for _, row in trades.loc[dates.dt.date == self.now().date()].iterrows():
                ...
                if strategy in self.daily_counts:
                    ...
                    try:
                        self.daily_pnl_by_strategy[strategy] += float(row.get("pnl", 0) or 0)
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _market_snapshot(self):
        ...
        if self.references.empty:
            ...
            self.diagnostics["ad_coverage"] = "0/500"
        ...
        for _, ref in self.references.iterrows():
            ...
            change = ((close - pdc) / pdc * 100) if pdc else 0.0
        ...
        try:
            ...
            nifty_change = None
        if nifty_change is None and complete:
        ...
        self.last_snapshot = {"intraday": intraday, "prices": prices, "sector": sector, "nifty_change": nifty_change, "ad_ratio": ad_ratio, "ad_complete": complete, "buy_alignment": buy, "sell_alignment": sell, "dhan_quotes": {}}
    ...
    def _evaluate_stock(self, symbol, ref, d, snap):
        ...
        prev = d.iloc[-1]
        dhan = snap.get("dhan_quotes", {}).get(symbol, {})
        # Dhan is authoritative for current LTP/OHLC; completed 1-minute candle remains the confirmation layer.
        ...
        for side in ("BUY", "SELL"):
            if side == "BUY" and (not snap["buy_alignment"] or float(prev["Close"]) <= float(prev["Open"])):
                continue
            if side == "SELL" and (not snap["sell_alignment"] or float(prev["Close"]) >= float(prev["Open"])):
                continue
            common = {
                "nifty500_change_pct": snap["nifty_change"], "sector_alignment_pct": snap["sector"].get("alignment_pct"), "ad_ratio": snap["ad_ratio"], "ad_coverage": 500,
                "previous_candle_open": float(prev["Open"]), "previous_candle_close": float(prev["Close"]), "symbol": symbol, "side": side, "ltp": ltp,
                "today_open": today_open, "pdh": pdh, "pdl": pdl, "today_low": today_low, "today_high": today_high,
                "prior_intraday_high": prior_high, "prior_intraday_low": prior_low, "pullback_low": today_low, "pullback_high": today_high,
                "breakout_seen": False, "pdh_swept": False, "pdl_swept": False,
            }
            if len(d) >= 2:
                pre = d.iloc[:-1]
                # Sweep definitions are directional and explicit: PDH sweep = price traded below PDH after an open above it;
                # PDL sweep = price traded below PDL after an open above it. SELL reverses the same logic above the level.
                common["pdh_swept"] = bool((pre["Low"] < pdh).any() or (pre["High"] > pdh).any())
                common["pdl_swept"] = bool((pre["Low"] < pdl).any() or (pre["High"] > pdl).any())
                common["breakout_seen"] = bool((pre["High"] > pdh).any() if side == "BUY" else (pre["Low"] < pdl).any())
                common["pullback_low"] = float(pre["Low"].min())
                common["pullback_high"] = float(pre["High"].max())
            for strategy in STRATEGY_DEFINITIONS:
                try:
                    signal = evaluate(strategy, **common)
                except (TypeError, ValueError, KeyError):
                    signal = None
                if signal:
                    row = signal.to_dict()
                    row.update({"strategy_name": STRATEGY_DEFINITIONS[strategy]["name"], "today_open": today_open, "today_low": today_low, "today_high": today_high, "pdh": pdh, "pdl": pdl, "previous_day_close": pdc, "entry_time": self.now().isoformat(timespec="seconds"), "signal_status": "ELIGIBLE"})
                    out.append(row)
        return out
    ...
    def _write_signal_ledger(self, signals):
        ...
        try:
            old = pd.read_csv(SIGNAL_FILE) if SIGNAL_FILE.exists() else pd.DataFrame()
            key_cols = [c for c in ["strategy", "symbol", "side", "entry"] if c in new.columns]
            if key_cols and not old.empty:
                old_keys = set(old[key_cols].astype(str).agg("|".join, axis=1))
                new = new[~new[key_cols].astype(str).agg("|".join, axis=1).isin(old_keys)]
            if not new.empty:
                pd.concat([old, new], ignore_index=True).to_csv(SIGNAL_FILE, index=False)
        except Exception:
            pass
    ...
    def scan(self):
        snap = self._market_snapshot()
        signals = []
        if not snap.get("ad_complete"):
            self.diagnostics["rejections"]["breadth"] = "NIFTY500_BREADTH_INCOMPLETE"
        if not snap.get("sector", {}).get("available"):
            self.diagnostics["rejections"]["sector"] = "SECTOR_ALIGNMENT_UNAVAILABLE"
        if not (snap.get("buy_alignment") or snap.get("sell_alignment")):
            self.diagnostics["rejections"]["market_alignment"] = "NO_MASTER_ALIGNMENT"
        if snap.get("buy_alignment") or snap.get("sell_alignment"):
            for _, ref in self.references.iterrows():
                symbol = str(ref["Symbol"]).upper()
                signals.extend(self._evaluate_stock(symbol, ref, snap.get("intraday", {}).get(symbol), snap))
        self._write_signal_ledger(signals)

        selected = []
        used = set()
        # Deterministic selection: one candidate per strategy, ranked by setup priority then symbol.
        priority = {"S1": 5, "S2": 4, "S3": 4, "S4": 3, "S5": 2}
        signals.sort(key=lambda x: (-priority.get(str(x.get("strategy", "")), 0), str(x.get("symbol", ""))))
        for sig in signals:
            strategy = sig["strategy"]
            if strategy in used:
                continue
            if self.daily_counts[strategy] >= MAX_TRADES_PER_STRATEGY_PER_DAY:
                continue
            if self.daily_pnl_by_strategy[strategy] <= -DAILY_MAX_LOSS_PER_STRATEGY:
                continue
            selected.append(sig)
            used.add(strategy)
        self.last_signals = selected
        self.diagnostics["final_signals"] = len(selected)
        self.diagnostics["signals_by_strategy"] = {s: sum(x.get("strategy") == s for x in selected) for s in STRATEGY_DEFINITIONS}
        self._write_diagnostics()
        return selected
    ...
    def process_signals(self, signals):
        ...
        for sig in signals:
            strategy = sig["strategy"]
            if self.daily_counts[strategy] >= MAX_TRADES_PER_STRATEGY_PER_DAY or self.daily_pnl_by_strategy[strategy] <= -DAILY_MAX_LOSS_PER_STRATEGY:
                continue
    ...
    def run_cycle(self):
        ...
        if hhmm < TRADING_START or hhmm > LAST_ENTRY_TIME:
            return []
    ...
    def _dhan_live_map(self):
        try:
            ...
            if not configured():
                return {}
            snapshot = BREADTH.snapshot()
            rows = snapshot.get("quote_rows")
            if rows is None or not hasattr(rows, "to_dict"):
                return {}
    ...
    def process_positions(self):
        ...
        for symbol in list(self.paper_engine.open_positions):
            ...
            if symbol in dhan_map:
                row = dhan_map[symbol]
                try:
                    live = {"Close": float(row.get("LTP")), "Datetime": self.now(), "High": float(row.get("TodayHigh")), "Low": float(row.get("TodayLow")), "price_source": "Dhan"}
            ...
            if closed:
                ...
                self.daily_pnl_by_strategy[strategy] = round(self.daily_pnl_by_strategy.get(strategy, 0.0) + float(closed.get("pnl", 0) or 0), 2)
    ...
    def square_off_all(self):
        ...
        for symbol in list(self.paper_engine.open_positions):
            ...
            if symbol in dhan_map:
                try:
                    price = float(dhan_map[symbol].get("LTP"))
                except (TypeError, ValueError):
                    price = None
            if price is None:
                live = self.price_data.get_latest_live_price(symbol, max_age_seconds=8)
                price = (live or {}).get("Close")
            if price:
                out.append(self.paper_engine.close_position(symbol, price, now, "SQUARE_OFF_15:00"))
    ...
    def _write_diagnostics(self):
        ...
        payload["daily_counts"] = dict(self.daily_counts)
        payload["daily_pnl_by_strategy"] = dict(self.daily_pnl_by_strategy)
        payload["paper_mode"] = True
        payload["refresh_seconds"] = SCAN_INTERVAL_SECONDS
