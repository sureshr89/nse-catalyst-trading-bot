...
class MasterEngine:
    ...
    def __init__(self):
        self.price_data = PriceData(); self.universe_engine = StockUniverse(); self.paper_engine = PaperTradeEngine(); self.journal = TradeJournal()
        self.scanner = self; self.references = pd.DataFrame(); self.sector_map = pd.DataFrame(); self.last_snapshot = {}; self.last_signals = []
        self.diagnostics = self._empty_diagnostics(); self.daily_counts = {s: 0 for s in STRATEGY_DEFINITIONS}; self.daily_pnl_by_strategy = {s: 0.0 for s in STRATEGY_DEFINITIONS}; self.cooldown_until = None; self._session_date = None
    ...
    def _refresh_reference_data(self, force=False):
        ...
        if universe is None or universe.empty or "Symbol" not in universe.columns: raise RuntimeError("NIFTY 500 universe unavailable")
        ...
        universe=universe.head(500); refs=ReferenceStore(universe).prepare()
        if refs is None or refs.empty or len(refs)<500: raise RuntimeError(f"Reference data incomplete: {0 if refs is None else len(refs)}/500")
        refs["Symbol"]=refs["Symbol"].astype(str).str.upper().str.strip(); self.references=refs; self.sector_map=load_sector_map(universe,refresh=force)
        if len(self.sector_map)<500: raise RuntimeError(f"Sector mapping incomplete: {len(self.sector_map)}/500")
        self._session_date=today

    # Compatibility hooks used by the persistent worker's pre-market preparation.
    ...
    def _market_snapshot(self):
        self._refresh_reference_data(); symbols=self.references["Symbol"].drop_duplicates().tolist(); intraday=self.price_data.get_multi_1m(symbols); available=sum(1 for s in symbols if s in intraday and not intraday[s].empty); rows=[]
        ...
        prices=pd.DataFrame(rows); sector=calculate_sector_alignment(prices,self.sector_map); complete=len(prices)==500
        if complete:
