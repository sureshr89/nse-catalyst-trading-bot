...
class MasterEngine:
    """One worker, one snapshot, five strategies, one paper-trade journal."""
    def __init__(self):
    ...
    def _refresh_reference_data(self, force=False):
        ...
        if len(universe) < 500: raise RuntimeError(f"NIFTY 500 universe incomplete: {len(universe)}/500")
        universe = universe.head(500)
        self.references = ReferenceStore(universe).prepare()
        if self.references is None or self.references.empty or len(self.references) < 500: raise RuntimeError(f"Reference data incomplete: {0 if self.references is None else len(self.references)}/500")
        ...
        if len(self.sector_map) < 500: raise RuntimeError(f"Sector mapping incomplete: {len(self.sector_map)}/500")
    ...
    def _market_snapshot(self):
        ...
        for _, ref in self.references.iterrows():
            ...
            if d.empty: continue
            last = d.iloc[-1]; pdc = float(ref.get("PreviousDayClose", 0) or 0)
        ...
        if complete:
            ...
            advances = declines = 0; ad_ratio = None
        ...
        buy = bool(complete and sector.get("available") and nifty_change is not None and nifty_change > 0 and sector_change > 0 and ad_ratio > 1)
        sell = bool(complete and sector.get("available") and nifty_change is not None and nifty_change < 0 and sector_change < 0 and ad_ratio < 1)
        self.last_snapshot = {"intraday": intraday, "prices": prices, "sector": sector, "nifty_change": nifty_change, "ad_ratio": ad_ratio, "ad_complete": complete}
    ...
    def _previous_candle(d):
        return None if d is None or d.empty else d.iloc[-1]

    @staticmethod
    def _prior_range(d):
        if d is None or len(d) < 2: return None, None
        prior = d.iloc[:-1]; return float(prior["High"].max()), float(prior["Low"].min())

    def _evaluate_stock(self, symbol, ref, d, snap):
        prev = self._previous_candle(d)
        if prev is None: return []
        today_open = float(d.iloc[0]["Open"]); today_low = float(d["Low"].min()); today_high = float(d["High"].max()); ltp = float(prev["Close"])
        pdh = float(ref["PDH"]); pdl = float(ref["PDL"]); pdc = float(ref["PreviousDayClose"])
        prior_high, prior_low = self._prior_range(d)
        common = {"nifty500_change_pct": snap["nifty_change"], "sector_alignment_pct": snap["sector"].get("alignment_pct"), "ad_ratio": snap["ad_ratio"], "ad_coverage": 500, "previous_candle_open": float(prev["Open"]), "previous_candle_close": float(prev["Close"])}
        out=[]
        for side in ("BUY", "SELL"):
            if side == "BUY" and not snap["buy_alignment"]: continue
            if side == "SELL" and not snap["sell_alignment"]: continue
            if side == "BUY" and float(prev["Close"]) <= float(prev["Open"]): continue
            if side == "SELL" and float(prev["Close"]) >= float(prev["Open"]): continue
            ...
            if len(d) >= 2:
                pre=d.iloc[:-1]; kwargs["pdh_swept"]=bool((pre["Low"] < pdh).any()); kwargs["pdl_swept"]=bool((pre["High"] > pdl).any()); kwargs["breakout_seen"]=bool((pre["High"] > pdh).any() if side == "BUY" else (pre["Low"] < pdl).any()); kwargs["pullback_low"]=float(pre["Low"].min()); kwargs["pullback_high"]=float(pre["High"].max())
            for strategy in STRATEGY_DEFINITIONS:
                try: signal=evaluate(strategy, **kwargs)
    ...
    def process_signals(self, signals):
        ...
        for sig in signals:
            ...
            if position:
                self.daily_counts[s]+=1; self.journal.log_trade(position); opened.append(position)
        return opened
