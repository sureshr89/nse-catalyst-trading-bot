...
@media(max-width:768px){[data-testid="stPageLink"] a{min-height:42px!important;font-size:.60rem!important}}
</style>
...
a,b,c,d,e=st.columns(5); a.metric("Bot",s.get("status","UNKNOWN")); b.metric("Worker","ALIVE" if s.get("worker_alive") else "OFFLINE"); c.metric("Open Positions",len(pos)); d.metric("Available Capital",f"₹{float(s.get('available_capital',250000) or 0):,.2f}"); e.metric("Realized P&L",f"₹{realized_pnl:,.2f}")
