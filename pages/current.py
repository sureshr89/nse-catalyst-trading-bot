...
a,b,c,d,e=st.columns(5); a.metric("Bot",bot_status); b.metric("Worker","ALIVE" if worker_alive else "OFFLINE"); c.metric("Open Positions",len(pos)); d.metric("Available Capital",f"₹{float(s.get('available_capital',250000) or 0):,.0f}"); e.metric("Realized P&L",f"₹{realized_pnl:,.2f}")
