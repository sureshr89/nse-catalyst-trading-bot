...
def test_contract_rejects_legacy_strategy_alias_as_active_engine():
    for legacy in ("STRATEGY_1","STRATEGY_2","STRATEGY_3","STRATEGY_4","STRATEGY_5","OPEN_RETURN"):
        try:
            strategy_metadata(legacy)
        except ValueError:
            pass
        else:
            raise AssertionError(f"Legacy strategy alias must be rejected: {legacy}")
