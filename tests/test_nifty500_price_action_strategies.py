...
def test_master_market_gate_uses_zero_threshold_and_sector():
    assert market_gate("BUY",0.01,0.01,1.01,500); assert market_gate("SELL",-0.01,-0.01,0.99,500)
    assert not market_gate("BUY",0.0,0.1,1.2,500); assert not market_gate("SELL",-0.1,0.0,0.8,500); assert not market_gate("BUY",0.2,0.2,1.2,499)
def test_previous_candle_direction():
    assert candle_gate("BUY",100,101); assert candle_gate("SELL",101,100); assert not candle_gate("BUY",101,100); assert not candle_gate("SELL",100,101)
...
def test_s1_buy_and_sell():
    buy=evaluate_s1("ABC","BUY",110,100,90,98,110,100,True,False,**G_BUY); sell=evaluate_s1("XYZ","SELL",90,100,95,85,102,95,False,True,**G_SELL)
    assert buy and buy.stop_loss==98 and buy.previous_candle_color=="GREEN"; assert sell and sell.stop_loss==102 and sell.previous_candle_color=="RED"
...
def test_filters_block_missing_breadth_or_sector():
    bad=dict(G_BUY); bad["ad_coverage"]=499; assert evaluate_s5("ABC","BUY",100,90,101,**bad) is None
    bad=dict(G_BUY); bad["sector_alignment_pct"]=-0.1; assert evaluate_s5("ABC","BUY",100,90,101,**bad) is None
