...
def test_market_gate_buy_and_sell_are_plus_minus_025():
...
def test_previous_candle_direction_is_required():
    assert market_gate("BUY", 0.30, 1.2)
    assert market_gate("SELL", -0.30, 0.8)
    assert evaluate_s5("ABC", "BUY", 100, 90, 101, 0.30, 1.2, 100, 101) is not None
    assert evaluate_s5("XYZ", "SELL", 100, 90, 89, -0.30, 0.8, 100, 99) is not None
    assert evaluate_s5("ABC", "BUY", 100, 90, 101, 0.30, 1.2, 101, 100) is None
    assert evaluate_s5("XYZ", "SELL", 100, 90, 89, -0.30, 0.8, 99, 100) is None
...
def test_s1_buy_and_sell():
    buy=evaluate_s1("ABC","BUY",110,100,90,98,115,110,True,False,0.30,1.2,100,101)
    sell=evaluate_s1("XYZ","SELL",90,100,95,85,102,90,False,True,-0.30,0.8,100,99)
...
def test_s2_buy_and_sell():
    buy=evaluate_s2("ABC","BUY",100,90,99,120,101,True,0.30,1.3,100,101)
    sell=evaluate_s2("XYZ","SELL",100,90,80,91,89,True,-0.30,0.7,100,99)
    assert buy and buy.stop_loss==99
    assert sell and sell.stop_loss==91


def test_s3_buy_and_sell():
    buy=evaluate_s3("ABC","BUY",110,120,100,98,115,110,True,False,0.30,1.2,100,101)
    sell=evaluate_s3("XYZ","SELL",90,100,80,85,102,90,False,True,-0.30,0.8,100,99)
    assert buy and buy.stop_loss==98
    assert sell and sell.stop_loss==102


def test_s4_buy_and_sell_use_previous_levels_only():
    buy=evaluate_s4("ABC","BUY",120,98,115,105,116,0.30,1.2,100,101)
    sell=evaluate_s4("XYZ","SELL",102,80,95,85,84,-0.30,0.8,100,99)
    assert buy and buy.stop_loss==105
    assert sell and sell.stop_loss==95


def test_s5_buy_and_sell():
    buy=evaluate_s5("ABC","BUY",100,90,101,0.30,1.2,100,101)
    sell=evaluate_s5("XYZ","SELL",100,90,89,-0.30,0.8,100,99)
    assert buy and buy.stop_loss==100
    assert sell and sell.stop_loss==90


def test_no_signal_when_market_filter_fails():
    assert evaluate_s5("ABC","BUY",100,90,101,0.24,1.2,100,101) is None
    assert evaluate_s5("XYZ","SELL",100,90,89,-0.24,0.8,100,99) is None


def test_no_future_level_is_required_for_s4():
    assert evaluate_s4("ABC","BUY",120,98,115,105,115,0.30,1.2,100,101) is None
    assert evaluate_s4("XYZ","SELL",102,80,95,85,85,-0.30,0.8,100,99) is None
