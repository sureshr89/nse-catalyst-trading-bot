...
def test_live_price_uses_dhan_source_label():
    pd_obj=PriceData();pd_obj._live_price_cache.clear();pd_obj._live_price_cache_at.clear();mapping=pd.DataFrame([{"Symbol":"ABC","SecurityId":"123"}]);quote=pd.DataFrame([{ "Symbol":"ABC","LTP":110,"TodayOpen":108,"TodayHigh":112,"TodayLow":107,"PreviousClose":105,"NetChange":5}])
    with patch("market.dhan_data.map_nifty500",return_value=mapping),patch("market.dhan_data.market_quote",return_value=quote),patch("market.dhan_data.configured",return_value=True): result=pd_obj.get_latest_live_price("ABC")
    assert result["price_source"]=="DHAN_OHLC";assert result["Close"]==110.0
