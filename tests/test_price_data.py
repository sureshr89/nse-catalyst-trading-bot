...
def test_daily_period_parses_numeric_days():
    ...
    with patch("market.dhan_data.map_nifty500",return_value=mapping),patch("market.dhan_data.daily_history",fake_history),patch("market.dhan_data.configured",return_value=True):
...
def test_live_price_uses_dhan_source_label():
    ...
    with patch("market.dhan_data.configured",return_value=True) as configured, \
         patch("market.dhan_data.map_nifty500",return_value=mapping) as mapper, \
         patch("market.dhan_data.market_quote",return_value=quote) as market_quote:
