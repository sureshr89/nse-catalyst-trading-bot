...
def test_daily_period_parses_numeric_days():
    ...
    def fake_history(sid, start, end):
        calls["start"] = start; calls["end"] = end; return pd.DataFrame()
    ...
    start = datetime.fromisoformat(calls["start"]).date(); end = datetime.fromisoformat(calls["end"]).date()
...
def test_live_price_uses_dhan_source_label():
    pd_obj = PriceData(); mapping = pd.DataFrame([{"Symbol": "ABC", "SecurityId": "123"}])
