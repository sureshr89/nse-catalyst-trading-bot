...
def test_position_size_rejects_impossible_risk_band():
    # Risk/share is too small to reach the minimum risk with the capital cap.
    assert position_size(100.0, 99.99) is None
    # Risk/share is so large that even the smallest affordable position
    # exceeds the maximum allowed risk.
    assert position_size(100000.0, 1.0) is None
