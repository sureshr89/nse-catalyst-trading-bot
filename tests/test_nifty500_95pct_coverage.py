...
def test_master_market_snapshot_accepts_490_verified_quotes(monkeypatch):
    ...
    engine.references = pd.DataFrame({
        ...
        "PDH": [110.0] * 500, "PDL": [90.0] * 500, "PreviousDayClose": [100.0] * 500,
    })
    engine.sector_map = pd.DataFrame({"Symbol": [f"S{i}" for i in range(500)], "Sector": ["A"] * 250 + ["B"] * 250})
    ...
    monkeypatch.setattr(me, "map_nifty500", lambda symbols: pd.DataFrame({"Symbol": symbols[:490], "SecurityId": [str(i) for i in range(490)]}))
    quotes = pd.DataFrame({
        "Symbol": [f"S{i}" for i in range(490)],
        "LTP": [101.0] * 250 + [99.0] * 240,
        "PreviousClose": [100.0] * 490,
        "change_pct": [1.0] * 250 + [-1.0] * 240,
        "TodayOpen": [100.0] * 490, "TodayHigh": [102.0] * 490, "TodayLow": [98.0] * 490,
    })
    monkeypatch.setattr(me, "market_quote_partial", lambda mapping: quotes)
    monkeypatch.setattr(me, "index_quote", lambda symbol: {"LTP": 101.0, "PreviousClose": 100.0, "NetChange": 1.0})
