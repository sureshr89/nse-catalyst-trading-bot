"""Trade and signal journal for the NIFTY 500 paper strategy."""
...
class TradeJournal:
    TRADE_COLUMNS = [
        ...
        "entry_time", "trigger_entry_time", "market_entry_time", "entry", "stop_loss", "target",
        "quantity", "exit_time", "exit_price", "exit_reason", "risk", "reward", "rr", "pnl",
        "risk_per_share", "actual_risk", "position_value", "open_cross_level", "pdh", "pdl",
        "today_open", "today_low", "today_high", "previous_day_close", "gap", "gap_percent", "gap_type",
        "market_direction", "sector_direction", "stock_direction", "stock_today_direction", "setup_type",
        "trigger_candle_open", "trigger_candle_close", "trigger_close", "pdh_pdl_reached", "liquidity_qualified",
        "nifty500_universe", "signal_quality_score", "why_this_trade", "mae", "mfe", "status"
    ]
    SIGNAL_COLUMNS = [
        "timestamp", "symbol", "industry", "sector", "signal", "market_direction", "sector_direction",
        "stock_direction", "stock_today_direction", "pdh", "pdl", "today_open", "today_low", "today_high",
        "previous_day_close", "gap", "gap_percent", "gap_type", "entry", "stop_loss", "target", "quantity",
        "risk_reward", "risk_per_share", "actual_risk", "position_value", "open_cross_level", "setup_type",
        "trigger_candle_open", "trigger_candle_close", "trigger_close", "pdh_pdl_reached", "liquidity_qualified",
        "nifty500_universe", "approved", "reason"
    ]
    ...
    def _daily_setup_key(self, signal):
        return (
            ...
            self._normalise(signal.get("symbol", "")),
            self._normalise(signal.get("signal", "")),
            self._normalise(signal.get("setup_type", "")),
        )
    ...
    def upsert_trade(self, trade):
        ...
        if not df.empty and bool(mask.any()):
            idx = df.index[mask][0]
            for column in self.TRADE_COLUMNS:
                if row[column] != "" or column in self.EXIT_FIELDS:
    ...
    def log_signal(self, signal):
        ...
        if self.signal_exists(signal):
            return {"saved": False, "duplicate": True, "reason": "Duplicate daily setup"}
