...
class TradeJournal:
    ...
    def _prepare_files(self):
        for path, columns in ((self.trade_file, self.TRADE_COLUMNS), (self.signal_file, self.SIGNAL_COLUMNS)):
            ...
            if not os.path.exists(path):
                self._write_header(path, columns); continue
            try:
                df = pd.read_csv(path)
                if self.LEGACY_COLUMNS.intersection(set(df.columns)):
                    print("Legacy strategy journal detected; clearing:", path); self._write_header(path, columns); continue
    ...
    def _daily_setup_key(self, signal):
        return (self._signal_date(signal), self._normalise(signal.get("symbol", "")), self._normalise(signal.get("signal", "")), self._normalise(signal.get("setup_type", "")))
    ...
    def _research_context(trade):
        score = 0; reasons = []
        side = str(trade.get("signal", trade.get("buy_sell", ""))).upper()
        ...
        for field, points, label in (("market_direction", 30, "NIFTY 500 aligned"), ("sector_direction", 30, "Sector aligned"), ("stock_direction", 20, "Stock aligned")):
            if required and str(trade.get(field, "")).upper() == required:
    ...
    def upsert_trade(self, trade):
        ...
        trade = dict(trade)
        score, explanation = self._research_context(trade)
