...
from config.settings import (
    ...
    MARKET_CLOSE, TOTAL_CAPITAL, MAX_OPEN_POSITIONS, MIN_REQUIRED_RISK,
    MAX_RISK_PER_TRADE, MIN_RR_RATIO, TRADE_LOG_FILE,
...
class PaperTradeEngine:
    ...
    @staticmethod
    def _trade_number(t):
        m=re.search(r"PAPER-(\d+)$",str(t).strip().upper())
        return int(m.group(1)) if m else 0

    @staticmethod
    def _strategy_name(value):
        value=str(value or "").strip().upper()
        return value if value in {"S1","S2","S3","S4","S5"} else ""

    def _valid_open_position(self,k,p):
        if not isinstance(p,dict):return False
        symbol=str(p.get("symbol",k)).strip().upper()
        sig=str(p.get("signal","")).strip().upper()
        e=self._number(p.get("entry"));sl=self._number(p.get("stop_loss"));tg=self._number(p.get("target"));q=self._number(p.get("quantity"))
        setup=self._strategy_name(p.get("setup_type"))
        return bool(symbol==str(k).strip().upper() and sig in {"BUY","SELL"} and setup and e and e>0 and sl and sl>0 and tg and tg>0 and q and q>0 and int(q)==q and p.get("trade_id"))

    def _migrate_state(self,state,version):
        s=dict(state)
        s.setdefault("open_positions",{})
        s.setdefault("closed_positions",[])
        s.setdefault("trade_counter",0)
        s.setdefault("total_capital",TOTAL_CAPITAL)
        s.setdefault("available_capital",s.get("total_capital",TOTAL_CAPITAL))
        s.setdefault("used_capital",0.0)
        s.setdefault("session_date",s.get("saved_at"))
        if not isinstance(s["open_positions"],dict) or not isinstance(s["closed_positions"],list):
    ...
    def _restore_state(self):
        ...
        try:
            ...
            session_date=self._session_date(state.get("session_date") or state.get("saved_at"))
            if session_date is not None and session_date!=datetime.now(INDIA_TZ).date():
            ...
            self.closed_positions=[v for v in state.get("closed_positions",[]) if isinstance(v,dict) and self._strategy_name(v.get("setup_type"))]
            for p in self.open_positions.values():
                p.setdefault("mae",0.0);p.setdefault("mfe",0.0);p.setdefault("last_processed_candle",self._candle_key(p.get("entry_time")));p.setdefault("last_live_price",None)
    ...
    def _save_state(self):
        ...
        try:
            with open(temporary,"w",encoding="utf-8") as f:
                json.dump(state,f,ensure_ascii=False,indent=2,default=str);f.flush();os.fsync(f.fileno())
            ...
            print(f"Paper state sync skipped: {type(e).__name__}: {e}")

    def has_open_position(self,symbol):return str(symbol).strip().upper() in self.open_positions

    def _daily_strategy_guard(self, strategy, entry_time):
        strategy=self._strategy_name(strategy)
        if not strategy:return False,"Missing or invalid setup_type; expected S1-S5"
        day=self._session_date(entry_time) or datetime.now(INDIA_TZ).date()
        trades=0;realised=0.0
        for p in self.closed_positions:
            if self._strategy_name(p.get("setup_type"))!=strategy:continue
            if self._session_date(p.get("entry_time"))!=day:continue
            trades+=1
            realised+=self._number(p.get("pnl")) or 0.0
        for p in self.open_positions.values():
            if self._strategy_name(p.get("setup_type"))==strategy and self._session_date(p.get("entry_time"))==day:
                trades+=1
        if trades>=int(MAX_TRADES_PER_STRATEGY_PER_DAY):return False,f"{strategy} daily trade limit reached ({int(MAX_TRADES_PER_STRATEGY_PER_DAY)})"
        if realised<=-float(DAILY_MAX_LOSS_PER_STRATEGY):return False,f"{strategy} daily loss limit reached (Rs {float(DAILY_MAX_LOSS_PER_STRATEGY):.2f})"
        return True,None
    ...
    def _validate_trade(self,t):
        ...
        symbol=str(t.get("symbol","")).strip().upper();sig=str(t.get("signal","")).strip().upper();setup=self._strategy_name(t.get("setup_type"))
        e=self._number(t.get("entry"));sl=self._number(t.get("stop_loss"));tg=self._number(t.get("target"));qv=self._number(t.get("quantity"));ar=self._number(t.get("actual_risk"))
        if not symbol:return None,"Missing symbol"
        if sig not in {"BUY","SELL"}:return None,"Invalid signal"
        if not setup:return None,"Missing or invalid setup_type; expected S1-S5"
        ...
        entry_time=t.get("entry_time") or datetime.now(INDIA_TZ)
        hh=self._time_string(entry_time)
        if hh is None or hh<self.trading_start or hh>self.last_entry_time:return None,"Entry time is outside the allowed window"
        entry_date=self._session_date(entry_time)
        if entry_date!=datetime.now(INDIA_TZ).date():return None,"Entry time is not in the current IST session"
        allowed,guard_reason=self._daily_strategy_guard(setup,entry_time)
        if not allowed:return None,guard_reason
        value=round(e*q,2)
        ...
        return {"symbol":symbol,"signal":sig,"setup_type":setup,"entry_time":entry_time,"entry":round(e,4),"stop_loss":round(sl,4),"target":round(tg,4),"quantity":q,"risk":risk,"reward":reward,"rr":round(rr,4),"position_value":value},None

    def open_trade(self,t):
        v,reason=self._validate_trade(t)
        if v is None:return {"opened":False,"reason":reason}
        self.trade_counter+=1;tid=f"PAPER-{self.trade_counter:04d}"
        p={"trade_id":tid,"symbol":v["symbol"],"stock":v["symbol"],"signal":v["signal"],"buy_sell":v["signal"],"setup_type":v["setup_type"],"strategy_version":CURRENT_STRATEGY,"entry_time":v["entry_time"],"entry":v["entry"],"stop_loss":v["stop_loss"],"target":v["target"],"quantity":v["quantity"],"risk":v["risk"],"reward":v["reward"],"rr":v["rr"],"mae":0.0,"mfe":0.0,"last_processed_candle":self._candle_key(v["entry_time"]),"last_live_price":v["entry"],"status":"OPEN","exit_time":None,"exit_price":None,"exit_reason":None,"pnl":None}
        ...
        p["risk_per_share"]=round(abs(v["entry"]-v["stop_loss"]),4);p["actual_risk"]=v["risk"];p["position_value"]=v["position_value"]
        self.open_positions[v["symbol"]]=p;self.used_capital=round(self.used_capital+v["position_value"],2);self.available_capital=round(self.total_capital-self.used_capital,2);self._save_state()
        return {"opened":True,"trade_id":tid,"position":p.copy()}
    ...
    def square_off_all(self,timestamp=None,reason="PAPER_SQUARE_OFF_15:00"):
        now=datetime.now(INDIA_TZ);exit_time=timestamp or now
        if self._session_date(exit_time)!=now.date():return []
        hh=self._time_string(exit_time)
        if hh is None:return []
        if hh<self.square_off_time:return []
        results=[]
        for symbol in list(self.open_positions):
            p=self.open_positions.get(symbol)
            if not p:continue
            price=self._number(p.get("last_live_price"))
            if price is None:
                try:
                    live=self.price_data.get_latest_live_price(symbol,max_age_seconds=2)
                    price=self._number(live.get("Close")) if live else None
                except Exception:price=None
            if price is None:price=self._number(p.get("entry"))
            closed=self.close_position(symbol,price,exit_time,reason)
            if closed is not None:results.append(closed)
        return results
    ...
    def process_live_price(self,symbol,price,timestamp=None,high=None,low=None):
        ...
        if self._time_string(timestamp or now) >= self.square_off_time:
            self.square_off_all(timestamp or now)
            return None
    ...
    def summary(self):
        vals=[self._number(t.get("pnl")) or 0 for t in self.closed_positions]
