...
class PaperTradeEngine:
    ...
    def _restore_state(self):
        ...
        try:
            ...
            # Recover the counter from the journal too, preventing duplicate IDs
            # if a container restarts before the JSON state was synchronized.
            try:
            ...
            # Recalculate capital from the actual restored open positions rather
            # than trusting potentially stale persisted arithmetic.
    ...
    def _validate_trade(self, trade):
        if not isinstance(trade, dict):
            return None, "Trade must be a dictionary"
        if not self.paper_trading:
            return None, "Paper trading is disabled"
        if self.live_trading:
            return None, "Live trading must remain disabled while using PaperTradeEngine"
        if not trade.get("approved", False):
            return None, "Trade has not been approved by RiskEngine"
        ...
        quantity_number = self._number(trade.get("quantity"))
        actual_risk = self._number(trade.get("actual_risk"))

        if not symbol:
            return None, "Missing symbol"
        if signal not in {"BUY", "SELL"}:
            return None, "Invalid signal"
        if entry is None or stop is None or target is None or quantity_number is None:
            return None, "Invalid trade values"
        if entry <= 0 or stop <= 0 or target <= 0:
            return None, "Prices must be positive"
        if quantity_number <= 0 or int(quantity_number) != quantity_number:
            return None, "Quantity must be a positive whole number"
        quantity = int(quantity_number)

        if signal == "BUY" and (stop >= entry or target <= entry):
            return None, "Invalid BUY stop/target"
        if signal == "SELL" and (stop <= entry or target >= entry):
            return None, "Invalid SELL stop/target"

        risk_per_share = abs(entry - stop)
        risk = round(risk_per_share * quantity, 2)
        reward = round(abs(target - entry) * quantity, 2)
        rr = reward / risk if risk > 0 else 0.0
        if risk < float(MIN_REQUIRED_RISK):
            return None, f"Actual risk ₹{risk:.2f} is below minimum ₹{float(MIN_REQUIRED_RISK):.2f}"
        if risk > float(MAX_RISK_PER_TRADE):
            return None, f"Actual risk ₹{risk:.2f} exceeds maximum ₹{float(MAX_RISK_PER_TRADE):.2f}"
        if rr < float(MIN_RR_RATIO):
            return None, f"Risk:Reward {rr:.2f} is below minimum 1:{float(MIN_RR_RATIO):.1f}"
        if actual_risk is not None and abs(actual_risk - risk) > 1.0:
            return None, f"Approved risk ₹{actual_risk:.2f} does not match calculated risk ₹{risk:.2f}"

        entry_time = trade.get("entry_time") or datetime.now()
        entry_hhmm = self._time_string(entry_time)
        if entry_hhmm is None:
            return None, "Unable to determine entry time"
        if entry_hhmm < self.trading_start or entry_hhmm > self.last_entry_time:
            return None, f"Entry time {entry_hhmm} is outside {self.trading_start}-{self.last_entry_time} IST"

        position_value = round(entry * quantity, 2)
        if position_value > self.available_capital:
            return None, "Insufficient available capital"
        if self.has_open_position(symbol):
            return None, f"{symbol} already has an open position"
        if len(self.open_positions) >= MAX_OPEN_POSITIONS:
            return None, "Maximum open positions reached"

        return {
            "symbol": symbol, "signal": signal, "entry_time": entry_time,
            "entry": round(entry, 4), "stop_loss": round(stop, 4),
            "target": round(target, 4), "quantity": quantity,
            "risk": risk, "reward": reward, "rr": round(rr, 4),
    ...
    def open_trade(self, trade):
        ...
        position = {
            ...
            "stock": validated["symbol"],
            "signal": validated["signal"],
            "buy_sell": validated["signal"],
        ...
        # Preserve every useful strategy/risk field in runtime state.
    ...
    def close_position(self, symbol, exit_price, exit_time, reason):
        ...
        position.update({
            "status": "CLOSED", "exit_time": exit_time,
    ...
    def process_candle(self, symbol, candle):
        ...
        high = self._number(candle.get("High", candle.get("high")))
        low = self._number(candle.get("Low", candle.get("low")))
        close = self._number(candle.get("Close", candle.get("close")))
        candle_time = candle.get("Datetime", candle.get("datetime"))
        ...
        # With only OHLC data the intrabar order is unknowable. Use the
        # conservative rule: if both levels are touched, count the stop first.
        if sl_hit:
            return self.close_position(symbol, stop, candle_time, "STOP_LOSS")
        if target_hit:
            return self.close_position(symbol, target, candle_time, "TARGET")

        candle_hhmm = self._time_string(candle_time)
        if candle_hhmm is not None and candle_hhmm >= self.square_off_time:
            return self.close_position(symbol, close, candle_time, "SQUARE_OFF")
        return None
