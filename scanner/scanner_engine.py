...
class ScannerEngine:
    def __init__(self):
        ...
        self.references = pd.DataFrame(); self.sectors = pd.DataFrame(); self.opening_candidates = pd.DataFrame(); self.gap_analysis = pd.DataFrame()
    ...
    def prepare_reference_data(self, force=False):
        ...
        self.universe = self.universe_engine.get_dataframe(refresh=True); self.references = ReferenceStore(self.universe).prepare(); self.sectors = SectorStore(self.universe).prepare(force=force); self._prepared_date = today
    ...
    def prepare_opening_candidates(self, force=False):
        ...
        if references.empty: self.diagnostics["rejections"]["missing_data"] = len(self.universe); self._write_diagnostics(); return pd.DataFrame()
    ...
    def _direction_asof(df, as_of=None):
        ...
        if as_of is not None:
            ...
            if getattr(data["Datetime"].dt,"tz",None) is not None and stamp.tzinfo is None: stamp=stamp.tz_localize("Asia/Kolkata")
            elif getattr(data["Datetime"].dt,"tz",None) is not None: stamp=stamp.tz_convert("Asia/Kolkata")
