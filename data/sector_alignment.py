...
def build_sector_map(universe):
    ...
    if sector_col is None:
        raise ValueError("NIFTY 500 sector/industry column unavailable")
...
def load_sector_map(universe,refresh=False):
    ...
    if not refresh and CACHE.exists():
        try:
            cached=pd.read_csv(CACHE);cached["Symbol"]=cached["Symbol"].map(_norm)
            actual=set(cached["Symbol"])
            if len(cached)>=REQUIRED and {"Symbol","Sector"}.issubset(cached.columns) and expected and expected.issubset(actual):
                return cached.drop_duplicates("Symbol")
...
def calculate_sector_alignment(prices,sector_map,price_col="change_pct"):
    ...
    m=sector_map[["Symbol","Sector"]].copy();m["Symbol"]=m["Symbol"].map(_norm)
    ...
    return {"available":alignment is not None,"alignment_pct":alignment,"mapped":mapped,"priced":priced,"sectors":total,"positive_sectors":pos,"negative_sectors":neg,"coverage":f"{priced}/500"}
