...
class ReferenceStore:
    ...
    def _download_batch(self,tickers):
        ...
        except Exception as error: print(f"Reference batch download failed ({len(tickers)}):",error); return pd.DataFrame()
    def _coverage_ok(self,df):
        if df is None or df.empty or self.universe.empty:return False
        required=max(1,int(len(self.universe)*self.minimum_coverage)); universe_symbols=set(self.universe["Symbol"].astype(str).str.upper()); saved_symbols=set(df["Symbol"].astype(str).str.upper()) if "Symbol" in df.columns else set(); return len(saved_symbols&universe_symbols)>=required
    def _cached_file_is_valid(self,saved):
        ...
        try:
            ...
            return not prepared_ist.dt.date.ne(datetime.now(INDIA_TZ).date()).any()
    ...
    def prepare(self):
        if self.path.exists():
            ...
            except Exception as error:print("Saved NIFTY 500 references could not be loaded:",error)
        symbols=self.universe["Symbol"].astype(str).str.upper().tolist(); tickers=[self._ticker(s) for s in symbols]; today=datetime.now(INDIA_TZ).date(); batches=[tickers[i:i+self.batch_size] for i in range(0,len(tickers),self.batch_size)]; rows=[]
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_map={executor.submit(self._download_batch,b):b for b in batches}
            for future in as_completed(future_map):
                batch=future_map[future]
                try:raw=future.result()
                except Exception as error:print("Reference batch worker failed:",error);continue
                ...
                for symbol,ticker in zip(symbols,tickers):
                    ...
                    try:
                        ...
                        completed=data[[d.date()<today for d in index_dates]]
                        if completed.empty:continue
                        prev=completed.iloc[-1]; close=float(prev["Close"]);volume=float(prev.get("Volume",0) or 0); rows.append({"Symbol":symbol,"PDH":round(float(prev["High"]),4),"PDL":round(float(prev["Low"]),4),"PreviousDayClose":round(close,4),"PreviousDayVolume":volume,"PreviousDayTurnover":round(close*volume,2)})
        ...
        if result.empty or not self._coverage_ok(result):print("NIFTY 500 reference coverage incomplete:",len(result) if not result.empty else 0,"of",len(self.universe));return pd.DataFrame()
        result=result.merge(self.universe[["Symbol","Industry"]],on="Symbol",how="left"); result["PreparedAtIST"]=datetime.now(INDIA_TZ).isoformat(timespec="seconds"); result.to_csv(self.path,index=False); return result
